# high-level-advisor Agent Eval Spike

A/B eval spike for the `high-level-advisor` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Priority ruthlessness, blind-spot exposure, paralysis resolution.
...
```text
evals/high-level-advisor-agent-spike/
...
1. Read `.claude/agents/high-level-advisor.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent high-level-advisor
