# orchestrator Agent Eval Spike

A/B eval spike for the `orchestrator` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Routing correctness, handoff fidelity, synthesis quality.
...
```text
evals/orchestrator-agent-spike/
...
1. Read `.claude/agents/orchestrator.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent orchestrator
