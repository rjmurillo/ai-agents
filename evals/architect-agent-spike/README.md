# architect Agent Eval Spike

A/B eval spike for the `architect` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Design coherence, pattern enforcement, boundary discipline.
...
1. Read `.claude/agents/architect.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent architect
