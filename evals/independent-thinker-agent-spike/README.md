# independent-thinker Agent Eval Spike

A/B eval spike for the `independent-thinker` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Counter-evidence quality, contrarian-not-contrarian-for-its-own-sake calibration.
...
```text
evals/independent-thinker-agent-spike/
...
1. Read `.claude/agents/independent-thinker.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent independent-thinker
