# backlog-generator Agent Eval Spike

A/B eval spike for the `backlog-generator` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Task quality (sized, actionable, evidence-linked) when generating from project state.
...
```text
evals/backlog-generator-agent-spike/
...
1. Read `.claude/agents/backlog-generator.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent backlog-generator
