...
Held-out agent-vs-baseline corpus driving `scripts/eval/eval-agent-vs-baseline.py` against the `reflect` skill.

## Skill under test

`.claude/skills/reflect/SKILL.md`

> CRITICAL learning capture. Extracts HIGH/MED/LOW confidence patterns from conversations to prevent repeating mistakes and preserve what works. Use PROACTIVELY after user corrections ("no", "wrong"), after praise ("perfect", "exactly"), when discovering edge cases, or when skills are heavily used. Without reflection, valuable learnings are LOST forever. Acts as continuous improvement engine for all

## Why eval-worthy

Extracts HIGH/MED/LOW confidence patterns. Pattern recognition is judgment.
