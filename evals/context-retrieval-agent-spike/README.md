# context-retrieval Agent Eval Spike

A/B eval spike for the `context-retrieval` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Coverage breadth, memory/doc relevance, latency.
...
```text
evals/context-retrieval-agent-spike/
...
1. Read `.claude/agents/context-retrieval.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent context-retrieval
