# milestone-planner Agent Eval Spike

A/B eval spike for the `milestone-planner` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Milestone decomposition quality, dependency capture, AC clarity.
...
```text
evals/milestone-planner-agent-spike/
...
1. Read `.claude/agents/milestone-planner.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent milestone-planner
