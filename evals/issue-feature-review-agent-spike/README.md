# issue-feature-review Agent Eval Spike

A/B eval spike for the `issue-feature-review` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Cost/value verdict accuracy, unknown-flagging recall.
...
```text
evals/issue-feature-review-agent-spike/
...
1. Read `.claude/agents/issue-feature-review.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent issue-feature-review

# Live A/B (10 trials per fixture per condition)
python3 scripts/eval/eval-reviewer-asymmetry.py \
    --agent issue-feature-review \
    --trials 10 \
    --output evals/issue-feature-review-agent-spike/runs/"<RUN_ID>"/runs.jsonl
```
