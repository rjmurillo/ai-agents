# retrospective Agent Eval Spike

A/B eval spike for the `retrospective` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.

## Agent under test

`.claude/agents/retrospective.md`

> Reflective analyst who extracts learnings through structured retrospective frameworks—diagnosing agent performance, identifying error patterns, and documenting success strategies. Uses Five Whys, timeline analysis, and learning matrices. Use when you need root-cause analysis, ato

## Eval target

Pattern extraction quality, atomicity scoring, learning-matrix coverage.
...
```text
evals/retrospective-agent-spike/
...
1. Read `.claude/agents/retrospective.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent retrospective

# Live A/B (10 trials per fixture per condition)
python3 scripts/eval/eval-reviewer-asymmetry.py \
    --agent retrospective \
    --trials 10 \
    --output evals/retrospective-agent-spike/runs/"<run-id>".json
```
