# spec-generator Agent Eval Spike

A/B eval spike for the `spec-generator` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
EARS adherence, three-tier completeness, traceability.
...
```text
evals/spec-generator-agent-spike/
...
1. Read `.claude/agents/spec-generator.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent spec-generator
