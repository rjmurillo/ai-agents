# task-decomposer Agent Eval Spike

A/B eval spike for the `task-decomposer` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Atomicity, AC presence, DoD clarity, dependency sequencing.
...
```text
evals/task-decomposer-agent-spike/
...
1. Read `.claude/agents/task-decomposer.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent task-decomposer
