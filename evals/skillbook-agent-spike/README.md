# skillbook Agent Eval Spike

A/B eval spike for the `skillbook` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Skill quality gates, dedup detection, atomicity rejection.
...
1. Read `.claude/agents/skillbook.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent skillbook
