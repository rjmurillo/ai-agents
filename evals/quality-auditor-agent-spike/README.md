# quality-auditor Agent Eval Spike

A/B eval spike for the `quality-auditor` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Grade accuracy across domains, gap-tracking recall.
...
```text
evals/quality-auditor-agent-spike/
...
1. Read `.claude/agents/quality-auditor.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent quality-auditor
