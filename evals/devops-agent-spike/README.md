# devops Agent Eval Spike

A/B eval spike for the `devops` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
Pipeline correctness, security defaults, cache/concurrency hygiene.
...
1. Read `.claude/agents/devops.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent devops
