# context-gather Eval Spike

Held-out agent-vs-baseline corpus driving `scripts/eval/eval-agent-vs-baseline.py` against the `context-gather` skill.

## Skill under test

`.claude/skills/context-gather/SKILL.md`

> Gather comprehensive context from Forgetful Memory, Context7 docs, DeepWiki, and web sources before planning or implementation. Delegates to the context-retrieval subagent to search across all knowledge tiers and returns a focused summary with a parseable CONTEXT_LOADED marker for downstream skip detection.

## Why eval-worthy

Delegates to context-retrieval. Coverage breadth is the eval target.
