# adr-generator Agent Eval Spike

A/B eval spike for the `adr-generator` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.

## Agent under test

`.claude/agents/adr-generator.md`
...
ADR quality: format adherence, completeness, template detection accuracy.
...
```text
evals/adr-generator-agent-spike/
...
1. Read `.claude/agents/adr-generator.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent adr-generator

# Live A/B (10 trials per fixture per condition)
python3 scripts/eval/eval-reviewer-asymmetry.py \
    --agent adr-generator \
    --trials 10 \
    --output evals/adr-generator-agent-spike/runs/"<RUN_ID>"/runs.jsonl
```
