# explainer Agent Eval Spike

A/B eval spike for the `explainer` agent. Default framing is **prompt-change control vs treatment** per the reviewer-asymmetry-spike template.
...
PRD clarity, INVEST adherence, ambiguity reduction (junior-reviewer simulation).
...
1. Read `.claude/agents/explainer.md` and identify the agent's bounded judgment output (verdict, finding, recommendation, design, plan).
...
python3 scripts/eval/eval-reviewer-asymmetry.py --dry-run --agent explainer
