# pr-comment-responder Eval Spike

Held-out agent-vs-baseline corpus driving `scripts/eval/eval-agent-vs-baseline.py` against the `pr-comment-responder` skill.
...
Comment triage and response. Acknowledgment quality varies.
...
```text
evals/pr-comment-responder-spike/
