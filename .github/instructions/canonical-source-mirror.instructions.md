...
The two destination trees now agree, measured on this branch after this PR's Copilot re-review fix (issue #5404, PR #5433) tightened `builder-ethos` and `voice` prose to restore the `.py` bucket's required 600-byte reserve:

| Tree | Consumer | Always-on |
|---|---|---|
| `.github/instructions` | Copilot working in this repository | 7 rules, 70,392 bytes |
| `src/copilot-cli/instructions` | the shipped plugin, installed elsewhere | 7 rules, 70,392 bytes |
