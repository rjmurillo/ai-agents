...
10. **Never attribute an action to a human from the author field alone.** Agents commit under the operator's identity, so `%an` says whose credentials ran, never who decided. Before writing or saying that a person made a change, read the trailer: `git show -s --format=%B <sha>` and look for `Copilot-Session:` or an equivalent agent marker. If one is present, an agent authored it, and the operator may have had no involvement. Resolve the session before naming anyone: query the local store (`~/.copilot/session-store.db`, or the `sessions` table by id prefix) or search `~/.copilot/session-state/*/events.jsonl` for the session id. The same blindness applies to GitHub audit events: a label added by an agent records under the operator's login and is indistinguishable from a human decision by inspection, which is why an authorization has to be stated in writing rather than inferred from the actor. Measured 2026-08-09: commits `45f07f346`, `8565eb6d3`, and `bdd4908b0` all carry `Copilot-Session: 500a9a25-a9ab-4688-8521-76a6a132fd40`. All three read as the operator's work in `git log`. One of them renamed two branch-protection contexts and stalled `main` for five hours, and the author field was used to argue the operator had done it. Refs #4782.
