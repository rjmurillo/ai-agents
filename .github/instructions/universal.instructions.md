...
3. **Verify every closing keyword against the diff before merging.** `Fixes #<n>` closes an issue when merged, so confirm each claim against a named hunk. Downgrade unsupported claims to `Refs #<n>` with reason. The inverse also matters: no closing keyword is not automatically a defect. Agent-authored PR bodies SHOULD use `These references do not close their linked items:` for bare `Refs`. Read the body before adding a closing keyword to a PR you did not write. After merge, editing the body is inert; close any orphaned issue with a comment citing the merge commit. Evidence: on 2026-08-03, 15 claimed closing keywords across five PRs were unsupported, and four of seven keyword-free PRs repaired from title matches explicitly should not close the issue.
...
6. MUST NOT add auto-generated headers, generation timestamps, or "do not edit"
   ...
   calling it.
7. MUST NOT nest git worktrees in the checkout.
