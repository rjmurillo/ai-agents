...
4. **A local run clears a red remote check only when it is the same checker with the same configuration.** A different tool, ruleset, flag set, or version produces evidence about a different check, and reporting it as "resolved" tells a reader to disregard a finding that is still live. Before writing that a red check is cleared, state which command you ran and why it is the same check; if you cannot, say the check is red and you could not reproduce it. Measured 2026-08-06 on PR #4701: `semgrep --config=auto` reported zero findings while `semgrep-cloud-platform/scan` reported two blocking ones, because the CI scan is the Semgrep Cloud App with a server-side ruleset and this repository ships no semgrep config for the CLI to read. Both were real, and one hid a URL parse that silently derived `github.com/owner` from a remote naming no repository.
5. **When one pull request is red and its siblings are green, the cause is in your diff.** A theory blaming shared or pre-existing code predicts that every pull request touching it is red, which is one query to falsify. Run it before publishing the theory. Measured on the same pull request: a sweep of 40 open pull requests found exactly one red, and #4723 modified the same file at the same lines and passed.
6. **Conventional commits**. Commit messages MUST follow `<type>(<scope>): <desc>` and include a `Co-Authored-By:` trailer when authored with an AI agent.
7. **Atomic commits**. Each commit MUST touch five or fewer authored files (see `AGENTS.md` boundaries). Hook-generated companions (session episodes, MCP config, agent catalog, memory index) are exempt and do not count toward the limit.
8. **No secrets**. MUST NOT commit credentials, tokens, or API keys. Secrets live in environment variables or the secrets manager.
9. **Pin Actions to SHA**. New GitHub Actions references MUST pin to a commit SHA, never a floating tag.
10. **Session log**. Long-running work MUST have a session log under `.agents/sessions/` per `.agents/SESSION-PROTOCOL.md`.
