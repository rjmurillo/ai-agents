...
5. **Isolate process-global state in a subprocess**. A regression guard that asserts on process-global state (`sys.path`, `sys.modules`, `os.environ`, `cwd`) SHOULD run in a clean subprocess when the property under test can be masked by another test module's import-time mutation. Sibling modules under `tests/` insert directories on `sys.path` at collection, so an in-process guard for the *absence* of such an entry passes for the wrong reason and stops catching the regression it was written for. Exemplar: `tests/validation/test_pre_pr_model_pin_wiring.py::test_lazy_version_pin_import_survives_sys_path_restore_in_isolation` (#3073), whose earlier in-process form passed with the bug reinstated because sibling portability tests kept `scripts/validation` on `sys.path`.
