---
applyTo: scripts/validation/**,scripts/**,.github/workflows/**,.github/actions/**,build/**,.claude/skills/**/scripts/**,.claude/skills/**/tests/**,src/copilot-cli/skills/**/scripts/**,src/copilot-cli/skills/**/tests/**,lefthook.yml
---
...
12. **Size a pre-push job's timeout for a loaded machine, not an idle one.** A script's standalone wall clock does not predict its wall clock as a lefthook job. Measured on `worktree-gc-report` with identical input every time, 288 registered worktrees and 0 removal candidates: 6.83s and 11.47s standalone, against 92.87s, 94.45s, 99.48s and 101.33s across four consecutive real pushes. Roughly ten times, and tightly clustered across four runs, so it is a property of the environment rather than noise. The mechanism is in `lefthook.yml`: that job sits in a `parallel: true` group next to `python-tests`, which carries a 30m cap and is the heaviest job in the hook, so the two contend for the same cores. Any job you add to a parallel group inherits that contention, so budget for it when you pick the cap. A cap sized on an idle run is a cap a real push exceeds, at which point a job that only reports is deciding whether code can ship.
13. **Read `lefthook.yml` for a group's scheduling. Do not infer it from the run summary.** The summary reports a group's duration as the sum of its jobs' durations, which for four consecutive pushes matched the sum to the hundredth (1017.69, 950.28, 930.85, 926.49) while the longest single job was well below it (818.73, 765.34, 738.28, 733.33). That arithmetic reads as proof of serial execution and is not: the group in question declares `parallel: true`. Timeout and contention reasoning that rests on the summary's arithmetic instead of the config will be backwards.
