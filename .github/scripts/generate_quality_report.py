...
Replaces the inline PowerShell block in the 'Generate Report'
step (id: report) of ai-pr-quality-gate.yml.
...
def main() -> None:
    try:
        ...
        if not os.path.isfile(report_file):
            print(f"::error::Failed to write report file: {report_file}")
            sys.exit(1)

        write_output("report_file", report_file)

    except Exception:
        print(f"::error::Failed to generate report: {traceback.format_exc()}")
        sys.exit(1)
