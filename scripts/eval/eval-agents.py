...
def _call_api_for_agents(
    api_key: str, messages: list[dict[str, str]], system: str = "", model: str = DEFAULT_MODEL
) -> str:
    """Call the Anthropic API with agent-specific max_tokens."""
    result: str = _call_api(
        api_key, messages, system=system, model=model, max_tokens=_AGENT_MAX_TOKENS
    )
    return result
...
def run_assessment(
    ...
    for agent_name in agents:
        ...
        print(f"\n{'=' * 60}", file=sys.stderr)
        print(
            f"  Agent: {agent_name} (model: {meta.get('model', 'unspecified')}, "
...
def main() -> None:
    ...
    parser.add_argument(
        "--model", type=str, default=DEFAULT_MODEL, help="Model to use for assessment"
    )
    parser.add_argument(
    ...
    api_calls = (
        prompt_count * 2 * args.runs if not args.dry_run else 0
    )  # (1 run + 1 score) * runs per prompt
    ...
    for agent_name in agents:
        if agent_name in results:
            ...
            overall = (
                round(sum(non_zero_values) / len(non_zero_values), 2) if non_zero_values else 0.0
            )
    ...
    print(
        f"  {'Agent':<22} {'Model':<8} {'Role':>6} {'Action':>7} {'Qual':>6} {'Approp':>7} {'Overall':>8}",
        file=sys.stderr,
    )
    ...
    weak = [(name, data) for name, data in sorted_agents if data["overall"] < 3.5]
    if weak:
