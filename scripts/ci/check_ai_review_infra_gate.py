...
skip the module writes the verdict file the parse step reads (the literal
``/tmp/ai-review-output.txt``,
overridable via ``AI_REVIEW_OUTPUT_FILE`` for tests), a ``retry_count=0`` output,
and a ``skip=true`` output the workflow's ``if:`` guard consumes to skip the
Invoke step. Both the skip and run decisions exit 0 so the DID_NOT_RUN verdict
reaches the unconditional parse step (#2821). Every GitHub sink is optional so
the module runs and is testable outside GitHub Actions.
...
# Literal path the action's parse step reads (``cat /tmp/ai-review-output.txt``).
# The gate must write this exact path on skip; a test override keeps the suite
# off the shared /tmp file.
DEFAULT_OUTPUT_FILE = "/tmp/ai-review-output.txt"
...
def emit(decision: GateDecision, output_file: str) -> None:
    ...
    The ``skip`` output is always written so the workflow ``if:`` guard has a
    value. On skip the verdict file and ``retry_count`` output mirror the inline
    block; the ``::warning::`` annotation is printed to stdout.
    """
...
def main() -> int:
    ...
    output_file = os.environ.get("AI_REVIEW_OUTPUT_FILE") or DEFAULT_OUTPUT_FILE
