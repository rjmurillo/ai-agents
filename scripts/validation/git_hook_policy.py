...
def run_pytest(repo_root: Path) -> int:
    ...
    for command in _pytest_commands(repo_root):
        ...
        if result.returncode == 3 and remaining < TEST_SUITE_TIMEOUT_SECONDS:
            # returncode 3 is the timeout sentinel from _run_command.
            # When remaining < the full budget, the command was started with
            # budget scraps, not a configured limit.  The generic timeout
            # message in stderr already says "timed out after <remaining>s",
            # which reads like a hung test.  Append a budget-exhaustion
            # clarification so the cause is unambiguous.
            elapsed = TEST_SUITE_TIMEOUT_SECONDS - remaining
            print(
                f"ERROR: pytest suite exhausted its {TEST_SUITE_TIMEOUT_SECONDS}s budget "
                f"({elapsed:.0f}s elapsed). The command above was started with only "
