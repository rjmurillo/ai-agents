...
def load_baseline() -> dict[str, Any]:
    ...
    try:
        result: dict[str, Any] = json.loads(BASELINE_PATH.read_text(encoding="utf-8"))
        return result
...
def fetch_live_params(ruleset_id: int) -> dict[str, Any]:
    ...
    if result.returncode != 0:
        print(
            f"ERROR: gh api failed (exit {result.returncode}): {result.stderr.strip()}",
            file=sys.stderr,
        )
...
def check_drift(baseline: dict[str, Any], live: dict[str, Any]) -> list[str]:
    """Compare baseline parameters against live values. Return drift messages."""
    drifts: list[str] = []
    for key, expected in baseline.get("parameters", {}).items():
        actual = live.get(key)
        if actual is None:
            drifts.append(f"  {key}: expected={expected!r}, not found in live ruleset")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if drifts:
        ...
        print("\n".join(drifts))
        print(
            f"\nUpdate {BASELINE_PATH.name} after confirming the change is intentional."
        )
        return EXIT_DRIFT
