...
Scope: ``SKILL.md`` under ``.claude/skills/`` and ``src/copilot-cli/skills/``
(both shipped trees). Roots that do not exist are skipped.
...
def _iter_skill_files(root: Path) -> list[Path]:
    return [
        p
        for p in sorted(root.rglob(SKILL_FILE_NAME))
        if p.is_file() and "__pycache__" not in p.parts
    ]


def scan_skill_execs(repo_root: Path) -> dict[str, int]:
    """Return {relative_posix_path: count} for SKILL.md files with >0 invocations.
        ...
        json.dumps(
            {
                "_comment": (
                    ...
                    "SKILL.md files (issue #2838). Counts of bare "
