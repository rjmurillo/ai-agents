...
def _deleted_paths_since_base(repo_root: Path, warn_label: str) -> list[str] | None:
    ...
    Provider deletion and rename must invalidate unchanged workflow consumers.
    The shared changed-path helper excludes deletions because ordinary lint
    targets cannot inspect absent files.
    """
    base_ref = _resolve_branch_base_ref(repo_root)
    if base_ref is None:
        return None
...
def _workflow_yaml_targets(repo_root: Path) -> list[str] | None:
    ...
    Composite actions never reach actionlint directly (#2346). Their metadata
    defines inputs consumed by workflows, so changing one invalidates every
    workflow contract. A changed reusable workflow also invalidates each
    workflow that references it through a local ``uses:`` path.
    """
