...
.NOTES
  ...
  Error Codes:
  - E_PATH_SYMLINK: Session log path or parent directory contains a symlink
  - E_PATH_ESCAPE: Session log path is not under .agents/sessions/ (path traversal/escape attempt)
  - E_SESSION_NOT_FOUND: Session log file does not exist
  - E_GIT_ROOT: Could not find git repository root
  - E_TOOL_MISSING: Required command not found
  - E_PROTOCOL_MISSING: SESSION-PROTOCOL.md not found
  - E_MUST_INCOMPLETE: Required checklist item not complete
  - E_QA_REQUIRED: QA validation required but not completed
  - E_DIRTY_WORKTREE: Uncommitted changes detected
  - E_MARKDOWNLINT_FAIL: Markdown linting failed
#>
...
# Security: Reject symlinks (similar to Sync-McpConfig.ps1 pattern)
# Use Get-ChildItem with -Force to handle hidden files
$sessionItem = Get-ChildItem -Path $sessionFullPath -Force | Select-Object -First 1
if ($sessionItem.LinkType) {
  $linkTarget = if ($sessionItem.Target) { " -> $($sessionItem.Target)" } else { "" }
  Fail 'E_PATH_SYMLINK' "Security: Session log path is a symlink, which is not allowed: $sessionFullPath$linkTarget"
}
...
Starts with check: $startsWithCheck
Original path: $sessionFullPath
