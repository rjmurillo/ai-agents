...
def run(repo_root: Path, *, check: bool = False) -> int:
    """Update token counts and verify root index coverage for repo_root.

    This function is the shared entry point used by both the CLI and
    the memory_index_token_ratchet gate. It does not automatically
    create rows for unindexed files; the legacy backlog is tracked via
