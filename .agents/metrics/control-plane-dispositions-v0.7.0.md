...
### Always-on rule subtraction: `search-before-building.md`

- Class: `MERGE` (folded into the `programming-advisor`, `memory-search`, and `memory-gate` skills as `references/*.md` files; the always-on copy is gone; the owner waived the eval for text that survives in a skill)
- Owner: not assigned
- Consumers: every Claude Code session in this repository (always-loaded
  rule content)
- Evidence: `search-before-building.md`'s "When To Apply", "The Three
  Layers", and "Quick Self-Review" sections moved to
  `.claude/skills/programming-advisor/references/search-before-building.md`;
  "Where To Search" moved to
  `.claude/skills/memory-search/references/where-to-search.md`; "The
  Contradiction Log" moved to
  `.claude/skills/memory-gate/references/contradiction-log.md`. Each
  destination `SKILL.md` gained one pointer line to its new reference file.
- Bar to clear: the same eight-run eval bar named in the
  `claude-model-patches.md` row above
  (`.claude/skills/context-optimizer/references/rule-audit-procedure.md`
  Steps 0a and 1); not run this session.
- Blocker: no scenario file exists yet under `tests/evals/rule-scenarios/`
  for this candidate, and the eight paid runs the procedure requires have
  not been run.
- Status: moved in PR (M4 PR2, this branch): text lives in
  `programming-advisor`, `memory-search`, and `memory-gate`. That bar still
  gates cutting the text outright; it does not gate relocating it out of
  the always-on set into the skills that search before building.
  `scripts/validation/instruction_budget.py` measured the `.md` always-on
  mirror at 50,912 bytes before and 43,895 bytes after (4 rules to 3).

### Always-on rule subtraction: `voice.md` Writing Style, Completeness Principle, Confusion Protocol

- Class: `MERGE` (folded into the `spec`, `plan`, `review`, and `autoplan` skills; the always-on copy is gone; the owner waived the eval for text that survives in a skill)
- Owner: not assigned
- Consumers: every Claude Code session in this repository (always-loaded
  rule content)
- Evidence: `voice.md`'s "Writing Style" (with "User-Turn Override"),
  "Completeness Principle: Boil the Lake" (with "Completeness Scores"),
  and "Confusion Protocol" (with "Unattended runs") sections moved to
  three ADR-108 partials
  (`templates/skills/partials/writing-style.mustache`,
  `completeness-scores.mustache`, `confusion-protocol.mustache`),
  rendered into `spec` and `plan`. `review` and `autoplan` are not
  template-owned, so the same text is hand-copied into
  `.claude/skills/review/resources/writing-style.md` (Writing Style and
  Completeness Scores only; a reviewer does not run the Confusion
  Protocol) and
  `.claude/skills/autoplan/references/writing-style-and-confusion-protocol.md`
  (all three sections). Each destination `SKILL.md` gained one pointer
  line to its new reference file.
- Bar to clear: the same eight-run eval bar named in the
  `claude-model-patches.md` row above
  (`.claude/skills/context-optimizer/references/rule-audit-procedure.md`
  Steps 0a and 1); not run this session.
- Blocker: no scenario file exists yet under `tests/evals/rule-scenarios/`
  for this candidate, and the eight paid runs the procedure requires have
  not been run.
- Status: moved in PR (M4 PR3, this branch): text lives in `spec`, `plan`,
  `review`, and `autoplan`. That bar still gates cutting the text
...
### Always-on rule subtraction: `builder-ethos.md` Decision Procedure, Task Completion Contract Forming/Reactivation

- Class: `MERGE` (folded into the `autoplan` and `avoiding-manufactured-work` skills; the always-on copy is gone; the owner waived the eval for text that survives in a skill)
- Owner: not assigned
- Consumers: every Claude Code session in this repository (always-loaded
  rule content)
- Evidence: `builder-ethos.md`'s `## Decision Procedure` section (six
  numbered steps plus closing sentence) moved verbatim to
  `.claude/skills/autoplan/references/decision-procedure.md`, with a new
  read instruction next to autoplan's existing Confusion Protocol
  pointer. `builder-ethos.md`'s `### Forming the contract` and
  `### Reactivation` subsection bodies (under `## 4. Task Completion
  Contract`) moved verbatim into
  `templates/skills/avoiding-manufactured-work.SKILL.md.tmpl` as two new
  `## Forming The Contract` and `## Reactivation` sections placed right
  after its existing Disposition Procedure section; the template's two
  internal references to "that same builder-ethos rule" were reworded to
  point at those sections instead. `builder-ethos.md` keeps the `## 4.
  Task Completion Contract` heading and its `### Precedence`, `###
  Finding disposition`, and `### Terminal predicate` subsections
  unchanged, and keeps the `### Reactivation` heading (test-pinned) and
  the phrase "frozen task contract" elsewhere in the file; both cut
  subsections were replaced with a one-sentence pointer to their new
  home.
- Bar to clear: the same eight-run eval bar named in the
  `claude-model-patches.md` row above
  (`.claude/skills/context-optimizer/references/rule-audit-procedure.md`
...
- Blocker: no scenario file exists yet under `tests/evals/rule-scenarios/`
  for this candidate, and the eight paid runs the procedure requires have
  not been run.
- Status: moved in PR (M4 PR4, this branch): text lives in `autoplan`
  and `avoiding-manufactured-work`. That bar still gates cutting the
  text outright; it does not gate relocating it out of the always-on set
  into the skills that walk the procedure and classify the finding.
  `scripts/validation/instruction_budget.py` measured the `.md`
  always-on mirror at 38,542 bytes before and 36,847 bytes after (still
  3 rules: `builder-ethos.md` shrank from 14,199 to 12,504 bytes at
  source, and `voice.md` stays the largest always-on rule, now 1,825
  bytes ahead of `builder-ethos.md` instead of 130).
