...
- **Cost**: 54 generated files
  ...
  30` plus the `copilot_cli_instructions: 24` sibling in the same
  baseline object) is the maintenance and portability cost measured; no
...
| 2 | Always-loaded instruction tokens decrease for every supported harness | Unchecked | No token-reducing change lands in this PR; this PR adds one new committed markdown file plus a memory correction, neither of which is always-loaded. |
