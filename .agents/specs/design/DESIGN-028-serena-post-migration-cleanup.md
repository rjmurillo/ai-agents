...
`.agents/specs/requirements/REQ-030-serena-post-migration-cleanup.md`.
...
`.agents/analysis/5394-serena-cleanup-disposition.md`.
...
2. Remove the index when it has no rows left, or when every row pointed at a
   deleted file.
3. Leave an index that still routes to surviving memories.

Removing an index whose targets survive would make those files unreachable from
`memory-index.md` and would raise the orphan count, so this design never does
it.
...
For each deleted path, run a repository-wide grep and update only the
references that this change makes semantically stale. A citation inside a
retrospective or an ADR that records history stays.
...
| A deleted path breaks a script or a workflow | Low | Step 6 greps `*.md`, `*.py`, and `*.yml` for each deleted basename |
