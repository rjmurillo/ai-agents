...
- **Metrics**: audit emits one row to `.agents/metrics/agent-skill-audit-runs.tsv` with 7 tab-separated columns: `timestamp | agent_count | verdict_skill | verdict_context_fork_skill | verdict_keep | verdict_merge | fm_bytes_saved_pct`. The 7th column quantifies load-budget impact (% of always-loaded frontmatter bytes saved if all skill+context-fork-skill rows migrate). Re-runs append rows; old rows preserved for diff.
