...
WHEN a contributor or a build changes any artifact under an ADR-109-enumerated class (agents, rules, hooks and settings; skills already covered by ADR-108; lib excepted to `scripts/`), THE SYSTEM SHALL treat the class's `templates/` tree (or `scripts/` for lib) as the sole canonical source, render it into both plugin trees (`src/claude/`, `src/copilot-cli/`) and bin-place those trees into both install trees (`.claude/`, `.github/`) in one atomic `build_all.py` run, and fail closed under `--check` or the pre-PR drift gate whenever any of the four trees would not match, SO THAT one edit to one template reaches every consumer instead of forking across up to four hand-maintained copies.
