...
- [x] `find templates/hooks -type f | wc -l` matches the class scope: 13 executables, `hooks.json`, `dispatch_groups.json`, and `markdownlint-safe-config.yaml` (16), plus one settings template (17 total under `templates/hooks/`); `git ls-files .claude/hooks | wc -l` (23) minus the templated 16 leaves the 7 doc files hand-maintained and unchanged by this task.
- [x] `find src/claude/hooks -type f | wc -l` returns 15 (13 executables, `dispatch_groups.json`, `markdownlint-safe-config.yaml`); `test -f src/claude/hooks.json` exits 0; both render for the first time in this tree.
- [x] `git diff --exit-code -- .claude/hooks .claude/settings.json` exits 0 after `uv run python build/scripts/build_all.py` runs on a clean checkout, confirming the binplace copy from `src/claude/hooks/` reproduces `.claude/hooks/`'s class members exactly.
- [x] `grep -n "OWNED_PREFIXES" -A 8 build/scripts/build_all.py` lists `.claude/hooks/` and `.github/hooks/` in the tuple (`.claude/settings.json` added too, since it is also a binplace-manifest target). Symlink-refusal is covered by `build_all.py`'s existing generic unreachable-owned-path test suite (`tests/build_scripts/test_build_all.py`, parametrized over `OWNED_PREFIXES`, not per-entry); a live symlink probe against the checked-out `.claude/hooks/` was not run because the harness's own deny rule refuses `mv`/`ln` targeting `.claude/` paths.
- [x] `ls .github/hooks/*.json` lists at least one file where it previously failed with "No such file or directory".
- [x] A new runtime-contract test generates a synthetic `.github/hooks/*.json` entry and runs the emitted command under the Copilot cloud-agent contract (cwd set to the working directory, `bash` as the only honored interpreter, no plugin-root environment variables), asserting the vendored script is found, with a negative control proving a bare relative command fails the same harness. It is a NEW sibling file, `tests/build_scripts/test_hook_templates_github_runtime_contract.py`, not an addition to the existing 831-line `test_generate_hooks_runtime_contract.py` (500-line taste-lint ceiling). It also documents, rather than fixes, a known gap: today's plugin-hooks.json commands are `$COPILOT_PLUGIN_ROOT`-anchored, which would not resolve under the cloud-agent's no-plugin-vars contract once any `.github/hooks/*.json` entry stops being empty; ADR-097 keeps entries empty today and this task does not reopen what a plugin registers.
- [x] `uv run python build/scripts/build_all.py --check` exits 2 after a hand edit to any file under `.claude/hooks/`, `.claude/settings.json`, or `.github/hooks/`, and the hand edit is still present afterward.
- [x] `templates/platforms/binplace.yaml` gains the `hooks` row (scoped to `src/claude/hooks/` directory), the `hooks-github` row (`src/copilot-cli/hooks` to `.github/hooks`), the `hooks-json` row (scoped to `src/claude/hooks.json` root FILE, not the `plugin_tree: null` shape DESIGN-025's excerpt showed; see PR body deviation note), and the `settings` row (`plugin_tree: null`, since `.claude/settings.json` ships in no plugin per ADR-109 section 3). `binplace_manifest.py` is generalized to binplace a file-shaped `plugin_tree` and to allowlist a direct-render (`plugin_tree: null` plus `compile`) row; `tests/build_scripts/test_binplace_manifest.py` asserts the root `hooks.json` file maps to `.claude/hooks/hooks.json`.
- [x] `_Gate("Hook Template Drift", ...)` appears in `scripts/validation/pre_pr_sequence.py`'s registry and in `tests/validation/test_pre_pr_sequence_registry.py`'s `EXPECTED_ORDER`.
...
- [x] `.github/CODEOWNERS` gains an entry for `/templates/hooks/`.
