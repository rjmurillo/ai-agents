...
- [ ] `templates/platforms/binplace.yaml` gains the `hooks` and `settings` rows (the latter with `plugin_tree: null`, since `.claude/settings.json` ships in no plugin per ADR-109 section 3).
