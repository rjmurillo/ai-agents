...
- [ ] **BLOCKED, not satisfied.** A fresh plugin install from the updated marketplace resolves `project-toolkit` at `./src/claude` with no missing file. The skill-loading smoke (`claude --plugin-dir src/claude plugin details project-toolkit`) passes: every skill name is listed. A deeper completeness check does not pass: `src/claude/skills/<name>/` carries only `SKILL.md` for all 111 skills; `scripts/`, `references/`, and `tests/` subdirectories that 84 of 111 skills carry under `.claude/skills/<name>/` (43 with `scripts/`, 61 with `references/`, 36 with `tests/`) are entirely absent from `src/claude/skills/<name>/`. No generator or binplace row copies them; `generate_skills.py`'s `_copy_skill_tree` performs that copy only for the Copilot CLI platform (`src/copilot-cli/skills/`), which is unaffected by this task. Example: `.claude/skills/merge-resolver/SKILL.md` instructs the reader to run `python3 .claude/skills/merge-resolver/scripts/resolve_pr_conflicts.py`, a script that does not exist under `src/claude/`. This predates this task (introduced when B3 moved the skills render target, TASK-033) but was invisible until this task's own marketplace repoint made `src/claude/` the sole Claude-side source; before this task, `.claude/` was still the live `project-toolkit` source and carried the full trees. Closing this gap needs a new generator step (mirroring `_copy_skill_tree`'s Copilot-side behavior for `src/claude/skills/`, always excluding `SKILL.md`, whose canonical source stays the template render) and is out of this task's scope: it is a B3-shaped fix, not a B6 one. That fix is landing on `feat/adr-109-b3-skill-support-files`; this task's status stays `todo` and B6 stays open until that branch merges and `feat/adr-109-b6-marketplace` merges `main` again to pick it up. Recorded here rather than silently worked around.
