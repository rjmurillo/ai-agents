...
- [ ] `tests/build_scripts/test_skill_templates_pilot_scope.py`'s `PILOT` constant is deleted (and the file itself deleted, since its sole purpose was pinning the pilot boundary) once `discover(repo_root)` on the real tree returns all 111 names with no further owner-approval step required for a new one. Deferred: batch 3 (PR #5781, 20 skills) had not merged when the render-target-move PR landed. `PILOT` stays as the 91-name set matching `discover()` on that branch; retiring the pin is a follow-up commit once batch 3 merges and `discover()` returns all 111.
