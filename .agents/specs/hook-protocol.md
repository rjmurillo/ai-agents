...
- `scripts/memory_enhancement/hooks/post_tool_call_memory.py` (fact capture): **deleted by ADR-097**, which retired the `PostToolUseFailure` wrapper that was its only caller. Re-adding automatic fact capture means writing a new carrier and clearing `.claude/rules/tool-use-hook-bar.md` first.
