...
**Finding 1** (recurring em/en-dash self-violation): **FM #3 (Ambiguous Instruction Inversion), partial fit.** The rule prohibiting em/en dashes was present in always-on context but was not mechanically verified before staging. FM #3's core pattern—a soft requirement decays from active enforcement to passive presence—matches this shape: the rule was loaded, but "loaded" was treated as "applied" without a blocking self-check. The fit is partial, not exact: FM #3 typically describes ambiguous wording that the agent interprets permissively, whereas here the rule was unambiguous but simply not actively self-checked against generated prose. This finding's single-shot catch shape (violation caught at the first pre-commit gate, no multi-round correction) does not fit FM #9's multi-round-correction trigger. FM #3 is the nearest existing class under `.agents/governance/FAILURE-MODES.md`; a new class covering "unambiguous rule loaded but not mechanically verified" may be warranted given two documented occurrences (2026-08-21, 2026-08-25), but is not proposed in this retrospective.
