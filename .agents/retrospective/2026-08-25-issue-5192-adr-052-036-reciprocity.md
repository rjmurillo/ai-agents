...
**What happened**: `git push` failed twice before succeeding. First, `push-ref-policy` reported `push validation requires complete Git history. .git/shallow pins history...`, fixed with `git fetch --unshallow origin`, the exact fix and exact gate the 2026-08-21 retrospective (Finding 1, Learning 1) already documented for this remote-session container class. Second, after unshallowing, `retrospective-policy` reported `git push requires retrospective evidence for this session`, which the 2026-08-21 retrospective's own session apparently did not trigger (its retrospective was written and, per its own Phase 5, never committed in that session: "not yet written to Serena memory (left for a follow-up session or explicit persistence pass)"), so there was no precedent in this repo's retrospective corpus for what satisfies this specific gate. Reading `scripts/validation/git_hook_policy.py:check_retrospective_evidence` directly (rather than guessing at an env-var bypass) showed the actual satisfying conditions: a documentation-only push, a trivial single-file session under 10 minutes old, or a retrospective file dated today under `.agents/retrospective/`. This file is that retrospective.

**Root cause**: The shallow-clone gap is a genuine environment default in this container class (confirmed present again, three sessions after the first documented instance), not a session error. The retrospective-gate gap is a genuine first-encounter: the prior session's own retrospective (which would have been the reference example) was itself never pushed, so there was no committed example to follow, and the session had to read the enforcing script's source directly to learn the contract instead of finding a documented recipe.
...
**Classification**: Environment/process gap, not an agent failure mode. Recorded per Learning 2 below since the shallow-clone half is now confirmed present in at least three independent sessions in this container class and still has no automated fix (only a documented manual one), and the retrospective-gate half had no committed example in `.agents/retrospective/` for a session to learn from before this one.
