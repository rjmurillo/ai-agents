...
| Start doing | Fixing the invariant rather than the reported instance. `expand_home` owes one property, that the result stays under `home`, and three separate mechanisms can violate it: a rooted suffix, a drive-anchored suffix, a drive-relative suffix. I patched them one review round at a time because each round I asked "what is broken" instead of "what does this function guarantee, and what can break that guarantee". |
...
| Then fixed THAT by stripping separators, which still left the suffix anchorable | Second regression on the same fix, reached commit `10b1fa2c1`, corrected in `d21631e3d` | Stripping separators does not make a suffix relative on Windows, where a drive anchors independently: `~/D:/x` leaves `D:/x` and joining it onto home yields `D:\x`, dropping home exactly as the rooted case did. Three consecutive rounds found a real bug in one nine-line function, each in my fix for the previous, because each fix addressed the instance in front of me rather than the invariant the function owes: the result must be under `home` | Fix the invariant, not the instance. When a helper exists to guarantee a property, enumerate the mechanisms that can violate it (rooted, drive-anchored, drive-relative) instead of patching the one that was reported | 92% |
