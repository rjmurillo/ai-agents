...
| Start doing | Asking the runtime what a platform does rather than encoding a belief about it, and pinning the derivation itself so a hardcoded answer fails a test. Both platform defects in this PR (the doubled separator, then the backslash overcorrection) came from writing down what I thought path handling does instead of reading `os.sep` and `os.altsep`. |
