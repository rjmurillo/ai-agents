...
| A0-T3 spec artifacts | S | REQ-021, DESIGN-020, TASK-024 to 026, ontology, this plan; frontmatter validator passes |
...
| Task | Size | Done when |
|------|------|-----------|
| A1-T1 `skill_templates.py` | M | `discover`, `owned_targets`, `check_grammar`, `render`, `compile_all` per DESIGN-020 |
...
Exit criteria: REQ-021 acceptance criteria 1, 2, 3, 8, 9, 10, 12, 13 hold; `build_all.py --check` exits 0; `check_skill_md_portability.py` reports no new offender; PR body quotes the byte report and uses `Fixes #5706`.

| Task | Size | Done when |
|------|------|-----------|
| A2-T1 six partials | S | Each opens with a `rule-source` line; parity test passes with a negative control |
| A2-T2 eight templates | M | `@CLAUDE.md` deleted; `{{> slug}}` lines at the DESIGN-020 steps; grammar check passes |
