...
**Plan format note:** This document does not follow the repo's canonical `.agents/plans/TEMPLATE.md` layout (Metadata / Objectives / Decision Log / Progress Log / Blockers). It is generated by `/autoplan` and uses a richer structure with inline Problem, Premises, Acceptance Criteria (REQs), CVA analysis, phase review outputs, and risk register. The Metadata block above satisfies the template's discoverability fields; the rest of the document is the working plan as /autoplan produced it.
