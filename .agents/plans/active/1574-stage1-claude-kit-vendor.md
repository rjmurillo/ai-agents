...
> **Note on versioning:** "v1.1" refers to the GitHub **milestone** for scheduling purposes. The npm package released by this plan uses semver `v0.1.0` as the initial shipped artifact version, and `v0.2+` for follow-up features explicitly deferred in this plan. Milestones and package versions are distinct axes.
