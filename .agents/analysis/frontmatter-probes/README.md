...
`.agents/analysis/frontmatter-parser-build-vs-buy.md` (issue #5275).

Run each from the repository root:

```bash
uv run python .agents/analysis/frontmatter-probes/probe_divergence_matrix.py
uv run python .agents/analysis/frontmatter-probes/probe_library_semantics.py
uv run python .agents/analysis/frontmatter-probes/probe_corpus_scan.py
```
...
| `probe_corpus_scan.py` | Parser agreement across every markdown file under `.agents/architecture`, `.serena/memories`, `.claude/skills`, `src`, and `templates` |
