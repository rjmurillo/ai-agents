...
| MUST | Security review export | [x] | No security-sensitive changes; documented as not applicable |
| MUST | Complete session log | [x] | All sections filled |
| MUST | Update Serena memory | [x] | Serena MCP unavailable; recorded in session log for follow-up |
| MUST | Run markdown lint | [x] | Running now |
| MUST | Route to qa agent | [x] | Investigation-only session; QA not required |
