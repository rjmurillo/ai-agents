# Execution Plan: ai-agents init, Stage 1 Claude Kit Vendor
...
| **Priority** | P1 |
| **Issue** | [#1574](https://github.com/rjmurillo/ai-agents/issues/1574) |
| **Epic** | #1574 → Stage 1 (#1619) → M0-M9 (#1623-#1632) |
| **Related PR** | [#1575](https://github.com/rjmurillo/ai-agents/pull/1575) (merged, being deprecated by M6) |

> **Note on versioning:** "v0.5.0" refers to the **GitHub milestone** ([#9](https://github.com/rjmurillo/ai-agents/milestone/9)) that groups Epic #1574, 4 Stage issues (#1619-#1622), and M0-M9 sub-issues (#1623-#1632). It is a repo-level scheduling bucket, not a package version. The npm package released by this plan uses its own semver starting at `@rjmurillo/ai-agents@0.1.0`, with `v0.2+` reserved for follow-up features explicitly deferred in this plan. GitHub milestone and npm package version are independent axes.

**Plan format note:** This document is generated by `/autoplan` and uses a richer structure than `.agents/plans/TEMPLATE.md` (with inline Problem, Premises, Acceptance Criteria/REQs, CVA analysis, phase review outputs, and risk register). For compatibility with the `execution-plans` skill and plan-management tooling, the canonical template sections are summarized below and link into the richer content further down.

> **Closeout note:** Completed on 2026-07-27. Evidence: PR #1633 and Stage issues #1574, #1619, #1623 to #1632 closed; packages/ai-agents-cli/ shipped @rjmurillo/ai-agents 0.1.0.

## Objectives

- [ ] **Stage 1 M0**: Commit plan file to main for session resumability (#1623) ✓ in progress
- [ ] **Stage 1 M1**: Package scaffold + interfaces (REQ-1.7) (#1624)
- [ ] **Stage 1 M2**: Bundle pipeline with blocking lint + cleanup pass (REQ-2.1-2.5) (#1625)
- [ ] **Stage 1 M3**: `init` core + CLAUDE.md merge + version pin (REQ-1.1-1.14) (#1626)
- [ ] **Stage 1 M4**: DX affordances: list, update, --only, GETTING-STARTED.md (REQ-1.13-1.17) (#1627)
- [ ] **Stage 1 M5**: Docs: README Terminal Start + positioning (REQ-6.1-6.6) (#1628)
- [ ] **Stage 1 M6**: Python deprecation: remove legacy init_project.py entrypoint (REQ-5.1-5.3) (#1629)
- [ ] **Stage 1 M7**: CI smoke matrix + first-turn verification (REQ-4.1-4.5) (#1630)
- [ ] **Stage 1 M8**: npm publish: provenance + OIDC + release docs (REQ-3.1-3.4) (#1631)
- [ ] **Stage 1 M9**: Review gates + ADR-042 amendment (REQ-7.1-7.3) (#1632). MERGE PREREQUISITE

## Decision Log

| Date | Decision | Rationale | Alternatives Considered |
|------|----------|-----------|------------------------|
| 2026-04-12 | Stage 1 CLI in TypeScript + bun + `npx`, not Python + `pipx` | Target audience lives in npm ecosystem; ADR-042 amended to scope Python-first to internal automation | Python+pipx (rejected: friction for Squad/Copilot audience) |
| 2026-04-12 | Full `.claude/` vendor minus github skill, hooks, lib, settings.json | Q9 resolved "full scope" for user-facing content; Eng phase excluded dev-private runtime infra | Curated kit (rejected: regresses vs existing `/install-plugin`) |
| 2026-04-12 | CLI is top-of-funnel for pre-Claude-Code devs, not a replacement for `/install-plugin` | /devex-review live baseline showed `/install-plugin` wins for inside-tool users | Universal "Fastest Start" (rejected: dishonest positioning) |
| 2026-04-12 | REQ-1.7 ships interfaces with single impl, strategy pattern deferred to Stage 2 | Eng phase both voices CRITICAL on locking the seam now; 20 LOC cost, rewrite if skipped | Defer interfaces entirely (rejected: Stage 2 rewrite risk) |
| 2026-04-12 | REQ-2.3 lint is BLOCKING, not warning | CEO + Eng phases both CRITICAL; shipping known-broken refs makes onramp an offramp | Warning-only (rejected: Phase 1 consensus) |
| 2026-04-12 | Add `list`, `--only`, `update` DX affordances | User accepted all 3 DX taste decisions at /autoplan final gate | Single-verb init (rejected: DX scorecard 4.8→7.5) |
| 2026-04-13 | Milestone v0.5.0 (#9) created, replaces placeholder v1.1 | User directive; aligns with repo milestone sequence (v0.4.0 → v0.5.0) | v1.1 (rejected: inconsistent with repo milestone numbering) |
| 2026-04-13 | REQ-6.1 "Fastest Start" → "Terminal Start (no AI tool needed yet)" | /devex-review finding; existing `/install-plugin` is faster inside Claude/Copilot | Keep "Fastest Start" (rejected: misleading) |

## Progress Log

| Date | Update | Agent |
|------|--------|-------|
| 2026-04-12 | /autoplan full pipeline: CEO (6/6 consensus), Eng (5/5 consensus), DX (7/7 consensus), competitive analysis | orchestrator (Claude Code) |
| 2026-04-12 | /plan decomposition: 9 milestones M1-M9 + dependency graph + risk register | orchestrator |
| 2026-04-12 | 15 GitHub issues created (Epic + 4 Stages + M0-M9) all labeled `Refining` | github skill |
| 2026-04-12 | M0 committed plan file on branch `feat/1574-stage1-claude-kit-vendor` | implementer |
| 2026-04-12 | /review pre-landing pass: 1 auto-fix (line 20 scope contradiction) | review skill |
| 2026-04-12 | /devex-review baseline audit: existing `/install-plugin` 8/10 getting-started | devex-review skill |
| 2026-04-12 | PR #1633 opened to `main`, initial CI validation triggered | devops |
| 2026-04-13 | 2 CI failures fixed: PR body init_project.py reference, line 81 regex literal | implementer |
| 2026-04-13 | /land-and-deploy readiness: 9 bot review threads addressed (scoped npx, CVA reconcile, metadata table) | implementer |
| 2026-04-13 | Milestone v0.5.0 (#9) created, 16 items attached | github skill |
| 2026-04-13 | 5 follow-up bot review threads addressed (smoke regex broadened, VERDICT token, template sections) | implementer |

## Blockers

- None currently. Stage 1 M0 merge is the active work. M1 is queued pending human review of `Refining` label on #1624.

## Related

- [#1574 Epic](https://github.com/rjmurillo/ai-agents/issues/1574)
- [#1619 Stage 1](https://github.com/rjmurillo/ai-agents/issues/1619)
- [#1620 Stage 2: Copilot Infrastructure](https://github.com/rjmurillo/ai-agents/issues/1620)
- [#1621 Stage 3: Squad-compete + --from squad](https://github.com/rjmurillo/ai-agents/issues/1621)
- [#1622 Stage 4: Evangelize](https://github.com/rjmurillo/ai-agents/issues/1622)
- [#1633 PR](https://github.com/rjmurillo/ai-agents/pull/1633)
- [Milestone v0.5.0 (#9)](https://github.com/rjmurillo/ai-agents/milestone/9)
- [PRD: ai-agents Platform](/.agents/plans/PRD-ai-agents-platform.md)
- [ENG Review: ai-agents Platform](/.agents/plans/ENG-REVIEW-ai-agents-platform.md)
- [GTM: ai-agents Platform](/.agents/plans/GTM-ai-agents-platform.md)

---

The remainder of this document is the /autoplan working plan as produced: full Problem statement, Premises, Acceptance Criteria (43 REQs), CVA analysis, phase review outputs, and risk register.

**Stage:** 1 of 4 (Claude-only, targeted for GitHub milestone [v0.5.0 (#9)](https://github.com/rjmurillo/ai-agents/milestone/9) tracked under Stage 1 issue #1619)

---

## Problem

ai-agents has 26MB of structured institutional memory, governance, session protocols, 25+ agents, 62 skills, and 57+ ADRs. It works. It compounds. But the onramp is a cliff: read SESSION-PROTOCOL, configure Serena, understand skill routing, learn the agent roster, then finally do something useful. Squad's onramp is a 3-step quickstart (their CLI actually exposes 17 commands, but the new-user path is 3). We need the same 3-step feel, reusing everything we already have.

The strategic reframe (per Richard, 2026-04-12): the "working team" already exists in `.claude/commands/*.md` + `.claude/agents/*` + `.claude/skills/*`. We do not build a new team. We package the existing one into `npx @rjmurillo/ai-agents init`.
...
### REQ-1: `npx @rjmurillo/ai-agents init [path]` vendors the full Claude kit
...
- `.claude/settings.json`: Registers hook paths; wires session-init, lint hooks, scheduled tasks. Every entry is repo-specific. Emit `settings.json.sample` in v0.2+ after preflight design.
...
- **REQ-4.4 (Eng phase finding, first-turn verification):** After `init` completes, smoke test runs `claude --non-interactive "/spec hello world"` in the target dir and asserts zero unresolved-path errors in output. This catches bundled content that references any missing dependency at agent runtime. **Pass/fail:** zero errors matching `(file not found\|no such file\|cannot resolve\|ENOENT)` whose file target matches `(\.agents/\|\.serena/\|\.github/copilot\|ADR-0\d+\|HANDOFF\|SESSION-PROTOCOL)` in claude stdout or stderr. The regex deliberately covers `.agents/`, `.serena/`, governance-adjacent paths, ADR citations, and session-protocol references, all known portability risks identified in the Eng phase. This is the decisive test: does the bundle actually work in a fresh repo?
...
- **REQ-7.1:** Security agent reviews bundled content for embedded secrets, credentials, unsafe commands, shell injection risk. Verdict: PROCEED required. **Pass/fail:** security agent verdict comment on the M9 tracking issue includes the literal token `VERDICT: PASS` (machine-parseable by the review-dashboard workflow).
- **REQ-7.2:** DevOps agent reviews npm publish flow (auth, provenance), CI matrix, bun build reproducibility. Verdict: PROCEED required. **Pass/fail:** devops agent verdict comment includes the literal token `VERDICT: PASS`.
- **REQ-7.3:** ADR-042 amendment drafted + adr-review cleared before S1 merges. **Pass/fail:** ADR file exists under `.agents/architecture/`, adr-review comment on the ADR PR includes the literal token `VERDICT: PASS`.
...
- **Relationship:** Stage 1 declares `BundleSource`, `TargetEmitter`, and `Transform` interfaces (REQ-1.7) with a single concrete implementation each. The interfaces are the architectural seam where Stage 2's Copilot emitter and Stage 3's Squad importer plug in additively. This is NOT a strategy pattern yet (no runtime dispatch, no variant selection), it's just the contract. Declaring the contract now costs ~20 LOC; skipping it forces a Stage 2 rewrite of `init.ts`. That's the smallest useful abstraction: interfaces before strategy, strategy when a second variant actually exists.
...
8. README "Terminal Start" section (REQ-6.1 canonical name), day 2
...
- **Comment 2 (PRD bot):** Milestone v0.5.0, P1, complexity 10/12, escalated to PRD. Aligned to "Master Product Objective: minimal friction adoption."
...
7. **The "magic" is the product.** `npx @rjmurillo/ai-agents init` sells the same emotion that drew users to Squad: one command, working team, no thinking. Template repos and curated starters lose that emotion. The magic is non-negotiable because it's what converts.
