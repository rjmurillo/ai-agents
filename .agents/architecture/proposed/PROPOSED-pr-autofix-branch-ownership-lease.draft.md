...
- **Enforce MAX_TTL during parsing.** After parsing timestamps, the parser rejects any marker whose `expires_at > acquired_at + MAX_TTL` (15 minutes) as malformed. This prevents a forged lease from setting `expires_at` arbitrarily far in the future: even if an attacker can post a comment, the parser will treat an over-extended marker as "no live lease" and the loop will proceed. Without this check, the TTL bound on forgery DoS is stated but not enforced.
