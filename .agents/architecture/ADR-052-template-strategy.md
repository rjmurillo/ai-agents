...
**Supersession pending: ADR-109 (proposed 2026-09-11) supersedes this record on acceptance.** ADR-109 inverts this record's direction: `templates/` is canonical for every plugin artifact class and `src/claude/` becomes a generated tree. The Migration Plan below is not carried forward once ADR-109 is accepted; issue #5282 is re-scoped by ADR-109's per-class PRs. This record stays `accepted` until then, because `scripts/validation/check_adr_lifecycle.py` refuses a proposed record that declares `supersedes`; the acceptance edit to ADR-109 flips this frontmatter to `status: superseded`, `superseded-by: ADR-109`.
