...
| Agent-runtime credentials, including MCP server tokens | Direct | Inventory alongside the workflow secrets, record granted scopes, and pin a negative mutation probe against rulesets and bypass actors. The loop reached this branch through one such credential, and it appears in no workflow file | High |
