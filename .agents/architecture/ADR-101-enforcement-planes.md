...
What stops that being exploitable today is worth stating exactly, because a panel reviewer reported it as a one-step exploit and it is not. The filter list at `pytest.yml:69-100` is head-defined, so a pull request can edit it, but the list covers `**/*.yml` and also names `.github/workflows/pytest.yml` by path. Any edit to the filter therefore marks Python inputs as changed and the real test job runs. The single-pull-request version of the attack closes on itself, by design rather than by accident.

The residual is narrower and still worth naming. Narrowing the filter takes a first pull request that does run the tests and shows the change in its diff, after which a second pull request touching only unfiltered file types receives a green `Run Python Tests` having run none. What prevents that is the breadth and self-coverage of a head-defined list, maintained by hand. That is a P0 artifact standing in for a P1 control, which is this decision's own invariant in miniature: the protection is real, and it is one careless filter edit from not being. Requirement 1 below is what replaces it with structure.
