...
- **Contain every secret reachable from a `pull_request` run**, not only the PATs. This is more than adding an `environment:` stanza, and the obvious version of it does not work.
  ...
  `BOT_PAT` is the sharpest case because classic `repo` scope reaches the Rulesets API, but it is not the only one. `COPILOT_GITHUB_TOKEN` sits in the same `pull_request`-triggered `ai-spec-validation.yml`; `ANTHROPIC_API_KEY` is reachable from `slash-command-quality.yml` and `software-engineering-library-activation.yml`; `CLAUDE_CODE_OAUTH_TOKEN` from `post-pr-retrospective.yml` and, separately, from `.github/workflows/claude.yml`, which lists `pull_request` among its triggers at `:19` and passes `secrets.CLAUDE_CODE_OAUTH_TOKEN` into the action at `:101`. All five workflows trigger on `pull_request`, so their `if:` guards are head-defined and a pull request can delete them. Four of the five are not PATs, so a scope worded around PATs misses them. `claude.yml` is the sharpest of those four because the same workflow also grants `checks: write` and `statuses: write`, so one head-defined edit reaches both the credential and the publishing permission the anchor below depends on.
...
- Fork pull requests are unaffected by the design's own mechanics, and an earlier revision claimed otherwise. A `workflow_run` job runs the trusted base definition and publishes against `github.event.workflow_run.head_sha`, which is a fork head as readily as a branch head, so nothing here deadlocks a fork. The real constraint is the one stated above: that job must reconstruct its inputs from the API rather than consume artifacts the fork run produced. GitHub's own first-time-contributor approval step still applies and still costs a maintainer action, but it applies today and is not caused by this decision.
