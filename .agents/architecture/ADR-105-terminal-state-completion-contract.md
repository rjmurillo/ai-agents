...
- Generated instruction mirrors (`.github/instructions/builder-ethos.instructions.md`,
  ...
  `src/copilot-cli/skills/avoiding-manufactured-work/SKILL.md`) are not
  regenerated by this ADR. They follow the existing canonical-source-mirror
  sync cadence, not an emergency follow-up.
- The 12 critic/qa agent surfaces carrying "zero findings is a valid
  APPROVED" wording (for example, `.claude/agents/critic.md`: "zero findings
  is a valid APPROVED when the review names at least one inspected file and
  line and states why nothing blocks") are conceptually adjacent, since a
  zero-finding APPROVED verdict is itself a terminal response, but this ADR
  does not edit them. Reconciling their wording with the Completion-Tail
  Audit is a follow-up, not a defect in this decision.
...
| 12 critic/qa agent surfaces carrying "zero findings is a valid APPROVED" wording (e.g. `.claude/agents/critic.md`) | Indirect | None required by this ADR; a follow-up should confirm each surface's approval-verdict language does not itself grow a continuation tail | Low |
| Generated instruction mirrors (`.github/instructions/builder-ethos.instructions.md`, `.github/instructions/voice.instructions.md`, `src/copilot-cli/skills/avoiding-manufactured-work/SKILL.md`) | Indirect | Regenerate or sync on the existing canonical-source-mirror cadence; not required before this ADR merges | Low |
...
Full multi-role consensus per `CONSENSUS.md` (the six-role `adr-review`
panel: architect, critic, independent-thinker, security, analyst,
high-level-advisor) did not run for this ADR. This is a documented
reduction, not a silent omission. The doctrine is prose-only across three
always-on instruction files plus a catalog entry: it carries no enforcement
code, no CI gate, and no schema change, so its blast radius is narrower than
the ADRs that ran the full panel (ADR-099, ADR-100, ADR-101). In place of
the panel, the orchestrating session is routing this same diff to
independent critic and security agent review before merge, as a scoped
substitute proportional to that narrower blast radius. `.claude/rules/
governance.md` rule 3 ("Consensus for cross-role rules... MUST follow the
consensus protocol in CONSENSUS.md") is satisfied at this reduced scope for
