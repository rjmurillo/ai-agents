...
5. **Fix the rebind churn at its source, `post_qa_code_changes`.** #5103 and #5107 show 18 to 20 commits with zero open review threads because the QA and session evidence rebind fires on every `main` merge into a long-lived branch. An earlier revision of this item said "fix the rebind generator" and named no component anywhere, while ordering it first, so the stated order could not be begun.
   ...
   The component is `post_qa_code_changes` in `.claude/lib/qa_report.py:244-253` and its `src/copilot-cli/lib/qa_report.py` mirror. It runs `git log --format= --name-only --no-renames -m -z <qaCommit>..<head>`, and `-m` splits each merge and diffs against **both** parents, so every path `main` touched inside a base merge is returned as a post-QA code change. The QA report then reads stale and needs a rebind commit, once per base merge, on a branch that changed none of those paths. PR #4954's QA record states the mechanism in its own words: "Because `post_qa_code_changes()` walks `git log -m` (both parents of a merge), every path `origin/main` touched appears as changed relative to the prior binding."
   ...
   **The obvious fix is wrong, and a reviewer caught it before it shipped.** An earlier revision of this item offered "compare against the first parent for merge commits" as one of two options. For a base merge the branch is the first parent and `main` is the second, so diffing the merge against its first parent returns exactly the paths `main` imported, which is the false-positive set item 5 exists to remove. That option is deleted rather than reworded.
   ...
   Replacing `-m` with `-c` therefore drops the clean incoming delta and keeps the authored resolution edits, which is the behaviour this item wants and which neither the first-parent option nor a blanket `--no-merges` gives. `--no-merges` is the cruder alternative and it is a real loss: it would stop counting conflict resolutions as authored change at all. State the choice in the implementation rather than leaving it to the implementer, and pin it with a test: a branch that merges `main` cleanly keeps its QA binding, and a branch whose merge resolved a conflict in a QA-relevant path does not. This decision's own pull request hit it four separate times while being written, and the fourth is the sharpest: once the local history was pushed, its reconciliation merges joined the remote lineage and the same `-m` walk flagged the ADRs again from the pull request head, so no binding short of the head was stable.
