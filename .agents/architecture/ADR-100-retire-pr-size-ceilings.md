...
5. **Fix the rebind churn at its source, `post_qa_code_changes`.** #5103 and #5107 show 18 to 20 commits with zero open review threads because the QA and session evidence rebind fires on every `main` merge into a long-lived branch. An earlier revision of this item said "fix the rebind generator" and named no component anywhere, while ordering it first, so the stated order could not be begun.
   ...
   **State the composite in the implementation rather than leaving it to the implementer, and note that the first attempt at stating it was wrong in a way worth keeping.** That attempt defined the second term as `git diff --name-only <merge>^1 <merge>` intersected with the paths the merge changed, and claimed it recovers the resolved-to-incoming case "without readmitting the clean import, because a clean import differs from the first parent too and must be excluded by the same test that a conflict resolution passes." A reviewer showed the two halves of that sentence contradict each other. A clean import and an incoming-side resolution are **indistinguishable** by comparing the merge result to its parents: both leave the result equal to the second parent and different from the first. The proposed term therefore readmits exactly the paths item 5 exists to exclude, which is the defect it was written to fix, reintroduced one revision later.

   What separates them is not the result but the **base**. Let `B = git merge-base <merge>^1 <merge>^2`. A path was conflicted only if both sides moved it away from `B`; a clean import is a path only the incoming side moved. So the second term is: paths changed in `B..<merge>^1` **and** in `B..<merge>^2`, whose merge result equals the second parent **and differs from the first**. The base condition is what the first attempt lacked; the differs-from-the-first-parent clause is what the second attempt lacked, and a reviewer supplied the case that needs it. If the incoming side already carries the receiving side's change verbatim and nothing more, both ranges touch the path, git merges it cleanly, and the result equals **both** parents, so the branch's validated content did not move and a rebind would be spurious. Requiring the result to differ from the first parent excludes exactly that case and nothing else: a genuine incoming-side resolution differs from the first parent by construction, and an incoming side that carries the receiving change **plus** further edits differs from it too, which is correct, because the branch's content did move.

   **Prefer `git merge-tree` conflict output where the surrounding code can reach it**, rather than treating the two range computations as equivalent. They are not: the ranges are a conservative approximation that the clause above narrows to the point of correctness on the cases this decision has examined, while `merge-tree` reports the conflicted set directly. Use the ranges only as the fallback, and pin the equivalence with the tests below rather than assuming it.
...
**This decision now picks, and an earlier revision's refusal to pick was the defect.** That revision offered two options and left the choice to the owner: define a counterfactual per retired gate, or split items 3 and 4 into their own decision. A reviewer objected that deferring permits implementation while two of the three retirements have no test that can reach them, which is correct. The split is the cleaner of the two on its own terms and this decision has offered it twice, but it costs the thing the decision exists for: the two size gates contradict each other, as session 3991 records above, so retiring the commit ceiling while keeping the five-file cap leaves an author at the ceiling with squashing as the only move and the atomic cap waiting on the other side. That is the trap the decision is built to remove, and a split reinstates it for the length of the second decision. So the first option is taken. The two missing replays are stated here at the same specificity as the one above, before the measurement, so neither can be fitted after.

**Atomic-cap counterfactual, for item 2.** Over the same merged population, replay `check_atomic_commit`'s contract commit by commit: authored files per commit against five, counting the way the gate counts, which excludes the hook-generated companions `.claude/rules/universal.md` MUST-6 exempts. A pull request enters the atomic would-have-been-blocked partition if any single commit in it exceeds five authored files. Note the squash bias runs opposite to the commit-count replay: where that one understates (fewer commits survive than the gate saw), this one overstates, because a branch that squashed several compliant commits into one presents a single commit exceeding five files when the original commits each stayed under. #5178 and session 3991 are the rewritten histories the commit ceiling caused; on this replay they falsely enter the blocked partition. Record that partition as an upper bound rather than a count. Item 2's retirement fails, and the cap returns, if a pull request in that partition satisfies predicate 1 or 2 above **and** the reviewer's record names commit granularity, rather than overall pull request size, as what a split would have caught. That last clause is the whole difference between this replay and the commit-count one; without it the same regression scores against both gates and the decision would reinstate a cap on evidence belonging to its neighbor.

**Scope-cap counterfactual, for item 3's threshold.** Replay `detect_scope_explosion.py`'s `BLOCK_THRESHOLD = 50` over each merged pull request's net authored file surface, computed after `_partition_generated` and after the process-record exclusions item 3 adds. Computing it before those exclusions would test a gate this decision is not retiring, and would put every long agent session over the line on its own session log. A pull request enters the scope would-have-been-blocked partition above 50. Item 3's threshold demotion fails, and the block returns, if a pull request in that partition satisfies predicate 1 or 2 above. This is the weakest-evidenced item in the decision, so it is also the one most likely to fail its own test, and that is the intended property rather than a concession.

Three partitions over one population, one 90 day window, three separate verdicts. Each gate returns on its own evidence and a failure in one does not reinstate the others, because the three measure different things and the sample already shows them disagreeing: #4846 passes a file-surface test at 16 files and fails a commit-count test at 235. The cost is two additional replays over a population assembled once for the first, which is what not deferring is worth.
