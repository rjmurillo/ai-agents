...
`markdownlint_guard` and `require_subagent_model`. Issue #4917 (2026-08-18)
then added `serena_worktree_scope`, which is not excluded: the vendored
source now has five registrations across two events, and the Copilot-side
PreToolUse manifest carries three shims, `markdownlint_guard`,
`require_subagent_model`, and `serena_worktree_scope`. This repository also
