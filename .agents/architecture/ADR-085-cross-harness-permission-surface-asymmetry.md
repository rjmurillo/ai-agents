...
PreToolUse, and Copilot generation emitted one host registration. Issue #4917
(2026-08-18), merged onto that #5154 baseline, then added
`serena_worktree_scope`, which is not excluded: the vendored source now has
two registrations on one event, PreToolUse, and Copilot generation emits one
host registration carrying both shims, `require_subagent_model` and
