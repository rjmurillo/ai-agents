...
The equality raise ties `endingCommit` and `comparison.head` together, and the tie is visible in history, at a scale smaller than it first looks. **Correction**: an earlier version of this measurement walked `git log --all -- .agents/sessions/`, which reads every ref this clone happens to hold rather than the branch under review, a violation of `.claude/rules/ci-scripts.md` MUST 9 ("a claim about what the repository contains MUST come from a named ref"); that version reported 50 edits across 38 commits. Re-measured from `HEAD`: across the 1026 single-parent commits touching `.agents/sessions/` reachable from `HEAD`, `comparison.head` is edited 44 times (a session log can be edited more than once), across 32 distinct commits. Of those 44 edits, 38 land in a commit where `endingCommit` is also present; 36 of those 38 agree (SHA-prefix-normalized, since an abbreviated `endingCommit` can name the same commit as a full `comparison.head`). Reproduce by walking `git log HEAD -- .agents/sessions/` and diffing `episodeMetrics.comparison.head` against `endingCommit` per file, per single-parent commit.
...
- `.agents/critique/ADR-102-debate-log.md`. Round 3: the six-independent-agent re-review, the overruled Block, and the corpus and citation corrections this ADR incorporates.
