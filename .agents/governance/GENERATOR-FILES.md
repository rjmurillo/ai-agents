...
(the new row above): a `.claude/skills/<name>/SKILL.md` whose
`templates/skills/<name>.SKILL.md.tmpl` exists at run time is generated output,
not a hand-maintained source, and the exception is scoped to exactly that set.
Every other file below is kept in sync by hand and guarded by the
