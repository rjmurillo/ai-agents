...
Measured on `agent_signal` in `scripts/quality_gate/external_signal_gate.py` (issue #4487). `DID_NOT_RUN` maps to `UNKNOWN`, and the fallback also yields `UNKNOWN`. With both paths silent, deleting the alias entry left all tests green. After splitting them so the alias returns quietly and the fallback warns, deleting the entry fails exactly one test, `test_did_not_run_is_aliased_not_merely_caught_by_the_fallback`, on the presence of the warning. Same code path, same return value, now falsifiable.
