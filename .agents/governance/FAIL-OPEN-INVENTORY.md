...
| `.github/scripts/check_spec_failures.py:82-87,108-112` | `main` | `TRACE_INFRA_FAILURE` or `COMPLETENESS_INFRA_FAILURE`, either or both | `return 0`, "Not blocking merge" | ADVISORY | N/A | yes, `::warning::`, and since issue #5738 an honest `INFRA_FAILURE` label on the affected side's summary-table cell and the overall verdict (`generate_spec_report.py`); the raw `CRITICAL_FAIL` verdict text is still shown, but labeled as unevaluated, in that side's collapsible findings details | DELIBERATE for the not-blocking decision, rationale recorded here per issue #5738: this check is a best-effort quality signal, not a security control, so it is not held to the `.claude/rules/security.md:55-58` "gate availability is gate correctness" policy for security review (itself a response to issue #4777's missing-review-admits-risk failure). Flipping this required check to block on every Copilot CLI hiccup would stop all merges on a credential outage with no attacker in the loop, a materially different risk than an unreviewed security change reaching `main`. Still UNDOCUMENTED: the infra-failure flag itself (`is_infrastructure_failure` at `scripts/ci/invoke_copilot_cli.py:220-225`, the production classifier) is trusted without independent verification that Copilot CLI actually could not run, rather than a disguised real failure; that gap is unchanged by this PR. It returns `True` for exit codes 124 or 137, `True` for ANY empty stdout regardless of cause (a genuine outage and an unrelated crash both classify as infrastructure), and `False` whenever stdout already contains `VERDICT:`. (`get_failure_category` at `scripts/ai_review_common/verdict.py:464-486` returns the same two category strings but is not this flag's production path: `scripts/ai_review_common/quality_gate.py:11` re-exports `get_failure_category` and it has no caller outside `tests/test_quality_gate.py`.) `COPILOT_GITHUB_TOKEN` itself was still failing as of 2026-09-17T04:00Z (this credential rotation is an operational task, not a code fix, and is unresolved by this PR) |
