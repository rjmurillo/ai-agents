...
**Superseded in shape by the billing matrix (2026-09-16).** Provider names are now cells of a 3x2 table in `scripts/eval/_billing_matrix.py`: harness (`claude`, `codex`, `copilot`) crossed with billing (`api`, `subscription`). Select with `--harness`/`--billing`, `EVAL_HARNESS`/`EVAL_BILLING`, or any provider name below. Every name in this memory still resolves to the same transport it always did. See `.serena/memories/eval/eval-billing-matrix.md` for the table.
...
Provider names: `openai`, `codex`, `codex-api`, `codex-cli`, `codex-subscription`, `claude-api`, `claude-cli`, `claude-code`, `claude-subscription`, `copilot`, `copilot-cli`, `copilot-subscription`, `copilot-api`, `anthropic-sdk`, and the retired `github`, `github-models`. OpenAI and GitHub Models share the OpenAI-compatible provider. GitHub Models uses `https://models.github.ai/inference` and token lookup order `GITHUB_MODELS_TOKEN`, `GITHUB_TOKEN`, `GH_TOKEN`. The Copilot rows shell out to the installed Copilot CLI.
