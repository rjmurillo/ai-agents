...
`extract` writes an envelope, not a bare mapping: `{"schema": "optimizer-results/1", "corpus": "26136df314d6c7b57fb85b8557440ac61d49c8728529f3040b930c8ff7ca02ef", "results": {"A001": true, "A002": false}}`, the same document the README shows. `corpus` is the identity of the task set the score was taken against, `fixture_set_sha` on the agent path and `null` on the rule and hook paths, which publish no equivalent. The reader still accepts a bare mapping; the discriminator is a string-valued `schema` key rather than the key alone, because bare mappings are all-boolean by construction and a legacy file with a task literally named `schema` must still parse as legacy.

## The corpus guard, and why it needed two attempts

`gate` refuses when the split's pin, the incumbent file and the candidate file name more than one corpus between them, counting a known corpus beside an unknown one as a disagreement. Unknown everywhere is not a disagreement and is not refused; it is reported as `corpus_verified: false`, because the rule and hook paths publish no corpus identity and refusing there would disable the gate for two of the three artifact classes. This exists because the omission had already cost something: on 2026-07-27 two architect-spike runs were gated as a null control, published across four files, and then found to disagree on `fixture_set_sha` with all eight fixture files changed. The report format carried the falsifier the whole time. `_fixture_set_sha`'s own docstring says the field exists so a consumer can verify two runs hit the same set; `extract` never read it.
...
5. The conflict rule runs twice: once on headers before the lock, once on the loaded `ResultsFile` values before the charge. Only the second pair is what the comparison is scored from. Both call sites emit one shared refusal payload so the two cannot drift. A shared builder does not enforce that on its own: the gate's copy shipped as `_corpus_refusal() | {"sel_consultations": spent}`, so the key set alone said which read caught the mismatch, and a seventeenth review found it. Assert the two call sites produce equal output, or the language will not.
...
8. The error contract is a claim like any other. The README says every failure prints one JSON document, and until round seventeen nothing tested a write. `_write_atomic` created its temp file above the `try` that wraps `OSError`, so `split --out` into a missing directory printed a traceback. `_read_json` never gained the `UnicodeDecodeError` arm that `_read_text` got in an earlier round, so a binary input reported the decoder's class instead of `ConfigError`. When two readers consume the same bytes, grep for the sibling before assuming a handler is complete.
...
- `mcnemar_exact` is reported always and enforced only with `--max-p`, which defaults to absent. A single discordant gain yields `p_value: 0.5` and still ACCEPTs by default, because a three-task held-out group cannot reach 0.05 and enforcing a conventional floor would make the common case unpassable. When supplied, `--max-p` is the FAMILY bar and is Bonferroni-divided by `--max-consultations`: five looks at 0.05 each bound the family at 5 * 0.05 = 0.25 without assuming anything about dependence, and at about 0.226 only under independence, which five looks at one selection group are not. Bonferroni is used because it holds under arbitrary dependence between the comparisons. That is conditional on each per-comparison p-value being valid on its own, which the exact McNemar tail earns only if discordant pairs are independent fair coin flips under the null; the rule-path null control reproduced both gains under a byte-identical no-op, so outcomes here are correlated and that half is not guaranteed. So raising the budget buys more looks at a stricter bar, never a cheaper one. The bar is pinned in the ledger like the cap, absence included, so a candidate refused at 0.05 cannot be re-gated at 0.1.
- Every verdict and refusal names `consultations`, this run's charge, so the one key answering "what did this cost me" is never absent where the answer is nonzero. `sel_consultations` is the running total including that charge and appears only where the total is both known and this selection group's: on a ledger key mismatch the count belongs to another group, so it is withheld rather than reported as a false zero.
