...
the number. The two destination trees agree today, measured on this branch after this PR's Copilot re-review fix (issue #5404, PR #5433) tightened `builder-ethos` and `voice` prose to restore the `.py` bucket's required 600-byte reserve:

| Tree | Consumer | Always-on |
|---|---|---|
| `.github/instructions` | Copilot in this repository | 7 rules, 70,392 bytes |
...
rules measure 70,508 bytes at `.claude/rules/`, 116 more, because the generator
