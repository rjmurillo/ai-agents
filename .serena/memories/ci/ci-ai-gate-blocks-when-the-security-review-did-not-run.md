...
trigger and recovery. This one covers what it does not: the summary line names
a cause nothing measured, the block is deliberate so repairing it removes a
security requirement, and which verdict token the failing agent wrote decides
...
CLI. The same claim is hardcoded in `generate_quality_report.py`,
`check_spec_failures.py`, and a comment in `agent-review/action.yml`, and is
...
On the measured incident the CLI installed and answered `--version` 10 lines
earlier. That is the whole proof; no reviewer call succeeded, so it does not
show the CLI was healthy, only that it was present:
...
you get decides which branch runs. One combination blocks nothing.
...
So "an infra failure always blocks" is wrong. The security-specific logic keys
off a category only some failure tokens ever receive.
...
Commit `426c1aa28` (#2846) made the behavior fail closed across the gates and
hooks. #2821 is load-bearing, not incidental: commit `230bb7cfc` says it and
#2818 exist to "preserve security review DID_NOT_RUN instead of downgrading it
to WARN". Widening the `WARN` downgrade to cover security is therefore a
straight revert of that fix, not a new idea. #2821 also added the runtime
annotation at `aggregate_quality_verdicts.py:120`.

The remedy is the one the warning names: re-run the gate, or review security
manually. Do not change the gate logic. The two edits below look like fixes,
turn the gate green, and remove the requirement that the security review
happen. The warning still prints, because it is driven by the aggregate's own
`security_review_ran`, which neither edit touches. So the PR merges green with
the reason sitting unread in the log. Never apply them:
...
infra flag, and wrote the artifacts at lines 72 to 75, and `action.yml` uploads
them at line 157 before the verdict check runs at line 187. The aggregate reads
those artifacts. That set still governs whether the agent step fails on a real
blocking verdict with no infra flag, so it is not dead code.
...
only adds it and never removes it, so it may be stale from an earlier run.
...
Three caveats on this evidence. Identical verdicts pointed at one shared cause
here, but `NEEDS_REVIEW` is the empty-verdict default, so identical tokens are
equally consistent with ten unrelated failures; the `HTTP 403` is what proves
infrastructure, not the agreement. QA passing among nine infra failures is
inferred from job ordering, not measured. Both PRs went green on rerun with no
diff change, which is consistent with the diagnosis but cannot confirm it,
since the reviewers are not deterministic.
...
produced any. Equally wrong: debugging the CLI on the strength of
