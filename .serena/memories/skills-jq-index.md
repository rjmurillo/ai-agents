...
| jq raw output quotes string shell variable -r | jq-raw-output |
| jq object construct select rename flatten transform | jq-object-construction |
| jq filter select condition any label array match | jq-filtering |
| jq array map sort group unique flatten length count | jq-array-operations |
| jq string interpolate template format tab csv tsv | jq-string-formatting |
