...
extracts the current session's episode to `.project-toolkit/memory/episodes/episode-<session>.json`,
then stages it. Until ADR-089 it also wrote and staged
`.project-toolkit/memory/causality/causal-graph.json`; that graph and its writer job are
