# ADR-017 Split Execution (Session 90)

**Date**: 2025-12-23  
**Commits**: c8ada09 (ADR-018), 0698b2e (ADR-017 split)  
**Branch**: docs/adr-017

## Summary

Successfully split ADR-017 from bundled document (~550 lines) into focused architecture + governance documents following ADR-018 criteria.

## What Was Split

**Original**: ADR-017-model-routing-low-false-pass.md (bundled 7 decisions, violated "single AD" criterion)

**New Architecture Document**: ADR-017-model-routing-strategy.md (~200 lines)
...
- Cross-references ADR-017 for architectural rationale

## ADR-018 Split Criteria Applied
...
**Created**:
- `.agents/architecture/ADR-017-model-routing-strategy.md`
- `.agents/governance/AI-REVIEW-MODEL-POLICY.md`

**Updated**:
- `.agents/architecture/ADR-017-debate-log.md` (added "Post-Debate: ADR-017 Split" section)
- `.agents/sessions/2025-12-23-session-90-adr-debate-clarification.md` (documented split)

**Removed**:
- `.agents/architecture/ADR-017-model-routing-low-false-pass.md` (preserved in git history)
...
- ADR-018: Architecture vs Governance Split Criteria (meta-ADR defining when to split)
