...
- Updated `.github/actions/ai-review/action.yml` so infrastructure failures (timeouts/rate limits, no stdout/stderr) no longer exit the job after retries.
- When retries are exhausted, the action now emits a fallback output with `VERDICT: CRITICAL_FAIL`, a clear infrastructure message, and preserves `infrastructure_failure=true`. The job continues so artifacts are produced.
- Added attempt counting before breaking out of the retry loop to keep `retry_count` non-negative and accurate.
- Non-infrastructure errors still fail fast; infrastructure failures are now downgraded by aggregation to WARN instead of blocking the workflow.
