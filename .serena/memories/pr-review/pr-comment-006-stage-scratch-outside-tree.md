# Skill: Stage Scratch Artifacts Outside the Working Tree
...
- 2026-05-02: rule landed in `AGENTS.md ## Boundaries > Never` (`Stage scratch artifacts in working tree (use $TMPDIR / mktemp; reply drafts, intermediate bodies, agent rough notes have no enduring value once posted)`), and `.gitignore` gained safety-net entries for `.agents/audit/pr-*-replies/` and `.agents/audit/pr*-reply-*.md` so unintended writes do not enter git even if guidance is ignored.
