...
# --- Claude-side dispatch-group expansion (#3075) --------------------------
#
# .claude/settings.json registers ONE dispatch_claude.py process per
# (event, matcher) group to cut per-hook spawn cost on the Claude Code
# side. Copilot CLI has its own consolidation (ADR-068 dispatcher), so the
# generator expands each dispatch registration back to the per-hook
# entries recorded in .claude/hooks/dispatch_groups.json before emission.
# The expanded entries reproduce the pre-consolidation command, timeout,
# and statusMessage values, so the generated Copilot tree is unchanged by
# the Claude-side consolidation.
