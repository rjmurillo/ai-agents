...
class TestAdapterNonPositiveMaxRetries:
    """`max_retries <= 0` is a caller mistake, not a provider result."""

    @pytest.mark.parametrize("max_retries", [0, -1, False])
    def test_non_positive_max_retries_names_the_parameter(
        ...
        assert repr(max_retries) in str(exc_info.value)
        assert calls == []
