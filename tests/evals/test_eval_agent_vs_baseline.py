...
class TestAdapterNonPositiveMaxRetries:
    """Fixed behavior: `max_retries < 1` raises ValueError immediately.

    Before the fix for #4121, these values silently returned an ERR_UNKNOWN
    record with attempts==0, making a caller configuration mistake
    indistinguishable from a real provider failure. The fix raises ValueError
    so the misconfiguration is surfaced at call time.
    """

    @pytest.mark.parametrize("max_retries", [0, -1, False])
    def test_non_positive_max_retries_raises(
