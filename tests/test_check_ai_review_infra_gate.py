...
def test_emit_skip_writes_file_outputs_and_warning(
    ...
    assert "skip=true" in out_text
    assert "retry_count=0" in out_text
...
def test_emit_run_writes_only_skip_false_and_no_file(
    ...
    out_text = out.read_text(encoding="utf-8")
    assert "skip=false" in out_text
    assert "retry_count" not in out_text
...
def test_main_skip_returns_zero_and_writes_file(
...
def test_main_run_returns_zero_and_skips_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    out = tmp_path / "out.txt"
    output_file = tmp_path / "ai-review-output.txt"
...
def test_main_missing_context_env_defaults_to_run(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    out = tmp_path / "out.txt"
