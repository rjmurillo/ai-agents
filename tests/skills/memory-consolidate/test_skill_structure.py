...
def test_consolidation_contract() -> None:
    ...
    for phrase in (
        ...
        "has no verified finite timeout",
        "a serena timeout or error falls back to direct directory access",
        "source stamp",
        ...
        "every path from serena, memory-maintenance, direct inventory, or an index",
        ...
        "record each target's content hash",
        "before each write, deletion, or rollback",
        "track a deleted target as absent",
        "if another process changed a target, stop",
        'starting_commit="$(git rev-parse head)"',
        'memory_git_targets.py" restore',
...
def test_discovery_index_and_output_contract() -> None:
    for phrase in (
        ...
        "memory-maintenance",
        "canonical `test_memory_size.py` script",
        "do not reimplement size validation here",
        "200 lines",
        "25,600 bytes",
        '"Success": true',
        '"Data": {',
