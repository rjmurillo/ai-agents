...
class TestFindCurrentSessionLog:
    """Tests for find_current_session_log function."""

    def test_returns_none_when_no_dir(self, tmp_path):
        assert complete_session_log.find_current_session_log(tmp_path / "missing") is None

    def test_returns_none_when_empty(self, tmp_path):
        assert complete_session_log.find_current_session_log(tmp_path) is None
    ...
    def test_finds_most_recent(self, tmp_path):
        ...
        f1.write_text("{}")
        f2.write_text("{}")
        result = complete_session_log.find_current_session_log(tmp_path)
        assert result is not None


class TestGetEndingCommit:
    """Tests for get_ending_commit function."""

    @patch("complete_session_log.subprocess.run")
    def test_returns_commit(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="abc1234\n")
        assert complete_session_log.get_ending_commit() == "abc1234"

    @patch("complete_session_log.subprocess.run")
    def test_returns_none_on_failure(self, mock_run):
        mock_run.return_value = MagicMock(returncode=1, stdout="")
        assert complete_session_log.get_ending_commit() is None


class TestHandoffModified:
    """Tests for test_handoff_modified function."""

    @patch("complete_session_log.subprocess.run")
    def test_detects_modified(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="HANDOFF.md\n")
        assert complete_session_log.test_handoff_modified() is True

    @patch("complete_session_log.subprocess.run")
    def test_not_modified(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="other-file.md\n")
        assert complete_session_log.test_handoff_modified() is False


class TestSerenaMemoryUpdated:
    """Tests for test_serena_memory_updated function."""
    ...
    def test_detects_memory_changes(self, mock_run):
        ...
        assert complete_session_log.test_serena_memory_updated() is True

    @patch("complete_session_log.subprocess.run")
    def test_no_changes(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="src/app.py\n")
        assert complete_session_log.test_serena_memory_updated() is False


class TestUncommittedChanges:
    """Tests for test_uncommitted_changes function."""

    @patch("complete_session_log.subprocess.run")
    def test_clean_repo(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="")
        assert complete_session_log.test_uncommitted_changes() is False

    @patch("complete_session_log.subprocess.run")
    def test_dirty_repo(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="M file.py\n")
        assert complete_session_log.test_uncommitted_changes() is True
...
class TestPathContainment:
    ...
    def test_valid_path(self, tmp_path):
        ...
        assert complete_session_log.validate_path_containment(
    ...
    def test_rejects_traversal(self, tmp_path):
        ...
        assert complete_session_log.validate_path_containment(
            evil_path, sessions_dir
        ) is False


class TestRunMarkdownLint:
    """Tests for run_markdown_lint function."""

    @patch("complete_session_log.subprocess.run")
    def test_no_markdown_files(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="src/app.py\n")
        result = complete_session_log.run_markdown_lint()
