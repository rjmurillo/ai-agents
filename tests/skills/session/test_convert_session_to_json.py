...
class TestFindChecklistItem:
    """Tests for find_checklist_item function."""

    def test_finds_checked_item(self):
        content = "| MUST | activate_project | [x] | Tool called |"
        result = convert_session_to_json.find_checklist_item(content, "activate_project")
        assert result["Complete"] is True
        assert "Tool called" in result["Evidence"]

    def test_not_found(self):
        content = "| MUST | something_else | [x] | Done |"
        result = convert_session_to_json.find_checklist_item(content, "activate_project")
        assert result["Complete"] is False

    def test_unchecked_item(self):
        content = "| MUST | activate_project | [ ] |  |"
        result = convert_session_to_json.find_checklist_item(content, "activate_project")
        assert result["Complete"] is False


class TestParseWorkLog:
    """Tests for parse_work_log function."""
    ...
    def test_extracts_work_log_entries(self):
        ...
        entries = convert_session_to_json.parse_work_log(content)
...
class TestConvertFromMarkdown:
    """Tests for convert_from_markdown function."""

    def test_extracts_session_number(self):
        result = convert_session_to_json.convert_from_markdown(
