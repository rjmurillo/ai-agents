...
class TestCheckFileSize:
    ...
    def test_memory_data_file_exempt_despite_size(self) -> None:
        # .project-toolkit/memory/ holds append-only generated data (issue #2785).
        lines = ["{}\n"] * 9000
        result = check_file_size(".project-toolkit/memory/episodes/episode-batch.json", lines)
        assert result == []
    ...
    def test_memory_data_absolute_path_under_cwd_exempt(self, tmp_path: Path) -> None:
        ...
        target = tmp_path / ".project-toolkit" / "memory" / "episodes" / "episode-1.json"
        lines = ["{}\n"] * 9000
    ...
    def test_memory_segment_in_parent_dir_not_exempt(self, tmp_path: Path) -> None:
        ...
        # contain .project-toolkit/memory must not leak the exemption to repo files.
        repo = tmp_path / ".project-toolkit" / "memory" / "repo"
    ...
    def test_absolute_path_outside_repo_not_exempt(self, tmp_path: Path) -> None:
        lines = ["{}\n"] * 9000
        result = check_file_size("/elsewhere/.project-toolkit/memory/data.json", lines)
        assert len(result) == 1
        assert result[0].severity == "error"

    def test_absolute_path_with_dotdot_escape_not_exempt(self, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        target = repo / ".project-toolkit" / "memory" / ".." / ".." / "src" / "big.py"
        lines = ["line\n"] * 600
        with patch.object(mod.Path, "cwd", return_value=repo):
            result = check_file_size(str(target), lines)
        assert len(result) == 1
    ...
    def test_non_memory_large_file_still_fails(self) -> None:
        # A look-alike path that is not under .project-toolkit/memory must still fail.
        lines = ["line\n"] * 600
    ...
    def test_memory_segment_mid_relative_path_not_exempt(self) -> None:
        # .project-toolkit/memory not anchored at the start of the relative path: not exempt.
        lines = ["line\n"] * 600
        result = check_file_size("src/.project-toolkit/memory/x.json", lines)
        assert len(result) == 1
        assert result[0].severity == "error"
