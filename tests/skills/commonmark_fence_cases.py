...
sees them, and that second cause is DESTRUCTIVE where the rest of this entry
is not. Because the expansion runs to four-column stops before the grammar
looks, how many spaces the permissive rule eats depends on the column, and so
on the LABEL'S LENGTH: over eleven single-line label lengths, nine turn a
balanced document into an unclosed one under `--write` and the two that do not
are exactly the two that land on a tab stop. The same escape through the
next-line destination path corrupts none of eleven, and the escaped-SPACE
sibling corrupts none of eleven, so this is specifically a tab reaching the
grammar already expanded. That makes four entries on this list filed as
disagreements that turned out to write, which is the pattern the top of this
docstring is about. So the permissive rule stays, with two known-wrong shapes, because
