...
BeforeAll {
    ...
    function New-MockDiffOutput {
        ...
        for ($i = 1; $i -le $FileCount; $i++) {
            # Each segment is a complete diff block for one file
            # Using explicit newline separator to ensure (?m)^diff --git matches correctly
            $segments += @"
...
"@
        ...
        # Join with newline to ensure each "diff --git" is at the start of a line
