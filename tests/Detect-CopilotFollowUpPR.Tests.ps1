...
BeforeAll {
    $scriptPath = Join-Path $PSScriptRoot ".." ".claude" "skills" "github" "scripts" "pr" "Detect-CopilotFollowUpPR.ps1"

    # Source the script to get access to functions (dot-source approach for testing)
    # Note: The script runs detection on source, so we'll test via mocking
}
