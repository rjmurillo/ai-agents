...
class TestUncommittedChangesExcludesSessionLog:
    ...
    def _run(
        ...
        *,
        ...
        with mock.patch("subprocess.run", return_value=fake):
            return mod._test_uncommitted_changes(
    ...
    def test_unrelated_dirty_file_returns_true(self) -> None:
        assert self._run(" M some/other/file.py\0") is True

    def test_session_log_only_returns_false_when_excluded(self) -> None:
        path = ".agents/sessions/2026-08-03-session-9999.json"
        porcelain = f" M {path}\0"
        assert self._run(porcelain, exclude_path=path) is False

    def test_session_log_without_exclude_returns_true(self) -> None:
        porcelain = " M .agents/sessions/2026-08-03-session-9999.json\0"
        assert self._run(porcelain, exclude_path=None) is True

    def test_session_log_plus_other_dirty_file_returns_true(self) -> None:
        path = ".agents/sessions/2026-08-03-session-9999.json"
        porcelain = f" M {path}\0 M scripts/some_script.py\0"
        assert self._run(porcelain, exclude_path=path) is True

    def test_rename_source_outside_owned_evidence_returns_true(self) -> None:
        porcelain = "R  .agents/qa/report.md\0scripts/main.py\0"

        assert self._run(
            porcelain,
            exclude_paths=(".agents/qa/report.md",),
        ) is True

    def test_rename_between_owned_evidence_paths_returns_false(self) -> None:
        porcelain = "R  .agents/qa/report.md\0.agents/qa/old-report.md\0"

        assert self._run(
            porcelain,
            exclude_paths=(
                ".agents/qa/report.md",
                ".agents/qa/old-report.md",
            ),
        ) is False
    ...
    def test_exclude_path_none_is_backward_compatible(self) -> None:
        assert self._run(" M foo.py\0", exclude_path=None) is True
