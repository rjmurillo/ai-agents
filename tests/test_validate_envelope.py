"""Tests for validate_skill_output's validate_envelope, in-process.

Split out of test_skill_output.py once that file crossed the 500-line
taste-lint gate (ADR-103 Round 5). test_skill_output.py keeps the
producer-side tests (get_output_format, write_skill_output,
write_skill_error); this file covers only the standalone validator.

CLI subprocess integration tests (invalid JSON, path traversal, symlink
attacks) live in test_skill_output_cli.py. Independent JSON-Schema-level
tests that run the schema itself via the jsonschema library, rather than
validate_envelope's hand-written checks, live in
test_skill_output_schema.py.
"""
