...
def test_session_policy_uses_existing_log_mode_for_a_historical_record(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-1-historical.json"
    commands: list[list[str]] = []

    def record(command: list[str], _root: Path) -> subprocess.CompletedProcess[str]:
    ...
    monkeypatch.setattr(
        ...
        lambda _sessions, _root: tmp_path / ".agents/sessions/current.json",
    )
    monkeypatch.setattr(policy, "_path_has_committed_source", lambda _path, _root: True)
    monkeypatch.setattr(policy, "_run_command", record)

    assert policy.check_sessions([path], tmp_path) == 0
    assert "--existing-log" in commands[0]
    assert "--creation-mode" not in commands[0]


def test_session_policy_fully_validates_the_current_branch_log(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-1-current.json"
    commands: list[list[str]] = []
    ...
    monkeypatch.setattr(
        policy,
        "_session_log_for_current_branch",
        lambda _sessions, _root: tmp_path / path,
    )
    monkeypatch.setattr(policy, "_path_has_committed_source", lambda _path, _root: True)
    monkeypatch.setattr(policy, "_run_command", record)
...
def test_session_policy_uses_creation_mode_for_an_uncommitted_new_log(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-1-new.json"
    commands: list[list[str]] = []
    ...
    monkeypatch.setattr(policy, "_path_has_committed_source", lambda _path, _root: False)
    monkeypatch.setattr(policy, "_run_command", record)
...
def test_session_policy_fully_validates_a_new_log_after_its_first_commit(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-1-new.json"
...
def test_session_policy_fully_validates_when_head_presence_is_unknown(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-1-historical.json"
    ...
    monkeypatch.setattr(
        policy,
        "_session_log_for_current_branch",
        lambda _sessions, _root: None,
    )
    monkeypatch.setattr(policy, "_path_has_committed_source", lambda _path, _root: None)
    monkeypatch.setattr(policy, "_run_command", record)
...
def test_session_policy_fully_validates_a_staged_rename_of_a_committed_log(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-1-renamed.json"
    commands: list[list[str]] = []
    ...
    monkeypatch.setattr(
        policy,
        "_path_has_committed_source",
        lambda _path, _root: True,
    )
    monkeypatch.setattr(policy, "_run_command", record)
...
def test_session_policy_fully_validates_a_low_similarity_session_replacement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    path = ".agents/sessions/2026-07-19-session-2-rewritten.json"
    old_path = ".agents/sessions/2026-07-19-session-1-original.json"
    commands: list[list[str]] = []
    ...
    def git_result(
        ...
        output = f"D\0{old_path}\0A\0{path}\0"
    ...
    monkeypatch.setattr(policy, "_path_exists_at_head", lambda _path, _root: False)
    monkeypatch.setattr(policy, "_run_git", git_result)
    monkeypatch.setattr(policy, "_run_command", record)
