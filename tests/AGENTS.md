...
- `tests/conftest.py` autouse fixtures force `GIT_CONFIG_COUNT=1` + `commit.gpgsign=false` (issue #2548), strip a leaked `core.hooksPath` (issue #2996), clear `CI` (issue #4380), default `AI_AGENTS_PROJECT_REPO=1` (issue #2610), and sanitize every `GIT_*` pointer var so `tmp_path` git commands cannot reach the real checkout (issue #4717).
- `git_hook_policy.py pytest` runs 4 partitions, not one: bulk + `tests/mutation/` under xdist (`-n`, `AI_AGENTS_PYTEST_WORKER_CAP`), then `tests/test_safe_push_pr_branch.py` + `test_mutation_workspace_signals.py` and `tests/test_pr_autofix_late_live_state_gate.py` serially -- those two mutate process-global state and would corrupt a shared worker pool.
...
- `.agents/governance/test-location-standards.md` is stale: it documents a Pester-only `*.Tests.ps1` layout for a repo with zero PowerShell files. The live rule is `.claude/rules/testing.md` (`paths: tests/**`, `**/tests/**`, `.claude/skills/**/tests/**`, `.agents/security/benchmarks/**`).

## Entry points

- `uv run pytest tests/ -x` -- direct run, root `AGENTS.md` Standards.
- `uv run python scripts/validation/git_hook_policy.py pytest` -- the exact partitioned invocation lefthook and pre-push run.
...
| `scripts/validation/git_hook_policy.py` (`_pytest_commands`, `~7170-7300`) | exact partitioned pytest invocations lefthook/CI run |
...
| `tests/mutation/` | mutation harnesses; each file name is a numbered incident |
...
- `tests/fixtures/guard_corpus_baseline.json` and similar ratchet/baseline JSON under `tests/ci/fixtures/`: read-only floors, regenerated by their own script's `--update`, never hand-edited.
...
- "`tests/*.test.ts` runs via `cli-smoke.yml`'s `bun test`" -- `docs/project-structure.md:51` says so; it is false. `bun test` at `cli-smoke.yml:179` runs with `working-directory: packages/ai-agents-cli`, a sibling tree; the two repo-root `.test.ts` files import repo-root `src/transforms/` and `src/copilot-target-emitter.ts`, and no root `package.json`/`tsconfig.json`/`bunfig.toml` wires either to any runner. Confirmed orphaned in `.agents/audit/2026-09-04-ponytail-audit-over-engineering.md` finding R3.
- "`check_nested_tests.py` scans every test file" -- its `git ls-files` patterns (`tests/*/test_*.py`, `tests/test_*.py`) reach only one directory level under `tests/`. Verified: `git ls-files "tests/skills/*/test_*.py"` returns real tracked files that neither pattern matches, so a nested-function bug two levels deep (`tests/skills/<name>/test_*.py`) is invisible to this gate whenever git is available, which in this repo is always.
- "`checks_coverage.py` enforces the 100/80/60 coverage table" -- it does not; see Matters.
- "`test-location-standards.md` is the placement authority" -- it describes a Pester layout the repo no longer has; `testing.md` plus the `check_*` gates above are what actually run.
- "the root HEAD guard only fires on git-heavy tests" -- it is autouse for the whole suite and fires on ANY real-repo HEAD movement in a test's window, including one from a concurrent human commit in the same worktree.
