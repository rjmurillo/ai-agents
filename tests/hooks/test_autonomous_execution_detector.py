"""Tests for UserPromptSubmit autonomous_execution_detector hook.
...
from autonomous_execution_detector import (
    AUTONOMY_PATTERNS,
    extract_user_prompt,
    format_autonomous_warning,
...
class TestHasAutonomyKeywords:
    ...
    def test_partial_word_no_match(self) -> None:
        ...
        # But 'full autonomy' should match
...
class TestExtractUserPrompt:
    ...
    def test_prompt_key(self) -> None:
        result = extract_user_prompt({"prompt": "hello"})
        assert result == "hello"

    def test_user_message_text_key(self) -> None:
        result = extract_user_prompt({"user_message_text": "hello"})
        assert result == "hello"

    def test_message_key(self) -> None:
        result = extract_user_prompt({"message": "hello"})
        assert result == "hello"

    def test_priority_order(self) -> None:
        """prompt takes priority over user_message_text."""
        result = extract_user_prompt({"prompt": "first", "user_message_text": "second"})
        assert result == "first"

    def test_empty_prompt_falls_through(self) -> None:
        result = extract_user_prompt({"prompt": "", "user_message_text": "fallback"})
        assert result == "fallback"

    def test_whitespace_prompt_falls_through(self) -> None:
        result = extract_user_prompt({"prompt": "   ", "message": "fallback"})
        assert result == "fallback"

    def test_no_known_keys(self) -> None:
        assert extract_user_prompt({"other": "data"}) is None

    def test_non_string_value(self) -> None:
        assert extract_user_prompt({"prompt": 123}) is None

    def test_empty_dict(self) -> None:
        assert extract_user_prompt({}) is None


class TestFormatAutonomousWarning:
    """Tests for warning message formatting."""

    def test_contains_key_sections(self) -> None:
        output = format_autonomous_warning()
        assert "AUTONOMOUS EXECUTION DETECTED" in output
        assert "Session Log Requirement" in output
        assert "Multi-Agent Consensus Gates" in output
        assert "Audit Trail Requirements" in output
        assert "Blocked Operations" in output
        assert "Proceed Only If" in output

    def test_contains_today_date(self) -> None:
        output = format_autonomous_warning()
        # Should contain a date pattern
        assert "session-NN.json" in output

    def test_contains_specific_blocked_commands(self) -> None:
        output = format_autonomous_warning()
        assert "git push --force" in output
        assert "git branch -D" in output
...
class TestMainAllow:
    ...
    def test_tty_stdin(self) -> None:
        with patch("autonomous_execution_detector.sys.stdin") as mock_stdin:
...
class TestMainDetect:
    ...
    def test_autonomous_keyword_injects_warning(
        ...
        assert "AUTONOMOUS EXECUTION DETECTED" in captured.out
...
class TestModuleAsScript:
    ...
    def test_autonomous_execution_detector_as_script(self) -> None:
        ...
        hook_path = str(
            HOOK_DIR / "autonomous_execution_detector.py"
        )
    ...
    def test_main_guard_via_runpy(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        hook_path = str(HOOK_DIR / "autonomous_execution_detector.py")
