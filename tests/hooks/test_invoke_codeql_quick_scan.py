"""Tests for invoke_codeql_quick_scan.py Claude Code PostToolUse hook."""
...
class TestGetFilePathFromInput:
    ...
    def test_extracts_file_path(self):
        ...
        assert invoke_codeql_quick_scan._get_file_path_from_input(hook_input) == "/tmp/test.py"

    def test_returns_none_for_missing_tool_input(self):
        assert invoke_codeql_quick_scan._get_file_path_from_input({}) is None

    def test_returns_none_for_non_dict_tool_input(self):
        assert invoke_codeql_quick_scan._get_file_path_from_input(
            {"tool_input": "string"}
        ) is None
...
class TestGetProjectDirectory:
    ...
    def test_uses_env_var(self, monkeypatch):
        ...
        result = invoke_codeql_quick_scan._get_project_directory({"cwd": "/other"})
        assert result == "/custom"

    def test_uses_cwd_from_input(self, monkeypatch):
        monkeypatch.delenv("CLAUDE_PROJECT_DIR", raising=False)
        result = invoke_codeql_quick_scan._get_project_directory({"cwd": "/from/input"})
        assert result == "/from/input"

    def test_falls_back_to_os_getcwd(self, monkeypatch):
        monkeypatch.delenv("CLAUDE_PROJECT_DIR", raising=False)
        result = invoke_codeql_quick_scan._get_project_directory({})
...
class TestShouldScanFile:
    ...
    def test_python_file(self, tmp_path):
        ...
        assert invoke_codeql_quick_scan._should_scan_file(str(py_file)) is True
    ...
    def test_workflow_yml(self, tmp_path):
        ...
        assert invoke_codeql_quick_scan._should_scan_file(str(yml_file)) is True
    ...
    def test_workflow_yaml(self, tmp_path):
        ...
        assert invoke_codeql_quick_scan._should_scan_file(str(yaml_file)) is True

    def test_non_workflow_yml(self, tmp_path):
        yml_file = tmp_path / "config.yml"
        yml_file.write_text("key: value")
        assert invoke_codeql_quick_scan._should_scan_file(str(yml_file)) is False

    def test_empty_path(self):
        assert invoke_codeql_quick_scan._should_scan_file("") is False

    def test_none_path(self):
        assert invoke_codeql_quick_scan._should_scan_file(None) is False

    def test_nonexistent_file(self):
        assert invoke_codeql_quick_scan._should_scan_file("/nonexistent/file.py") is False

    def test_non_scannable_file(self, tmp_path):
        txt_file = tmp_path / "readme.txt"
        txt_file.write_text("hello")
        assert invoke_codeql_quick_scan._should_scan_file(str(txt_file)) is False
...
class TestIsCodeQLInstalled:
    ...
    def test_found_in_path(self, tmp_path):
        with patch("shutil.which", return_value="/usr/bin/codeql"):
            assert invoke_codeql_quick_scan._is_codeql_installed(str(tmp_path)) is True
    ...
    def test_not_installed(self, tmp_path):
        with patch("shutil.which", return_value=None):
            assert invoke_codeql_quick_scan._is_codeql_installed(str(tmp_path)) is False
...
class TestGetLanguageFromFile:
    ...
    def test_python_file(self):
        assert invoke_codeql_quick_scan._get_language_from_file("test.py") == "python"

    def test_yml_file(self):
        assert invoke_codeql_quick_scan._get_language_from_file("ci.yml") == "actions"

    def test_yaml_file(self):
        assert invoke_codeql_quick_scan._get_language_from_file("ci.yaml") == "actions"

    def test_unknown_file(self):
        assert invoke_codeql_quick_scan._get_language_from_file("test.rs") is None
...
class TestMain:
    ...
    def test_codeql_not_installed_skips(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("sys.stdin", io.StringIO(input_data))
        with patch.object(
            invoke_codeql_quick_scan, "_is_codeql_installed", return_value=False
        ):
            assert invoke_codeql_quick_scan.main() == 0
    ...
    def test_language_returns_none_for_scannable_file(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("sys.stdin", io.StringIO(input_data))
        with patch.object(
            invoke_codeql_quick_scan, "_should_scan_file", return_value=True
        ):
            with patch.object(
                invoke_codeql_quick_scan, "_is_codeql_installed", return_value=True
            ):
    ...
    def test_win32_appends_exe(self, tmp_path):
        ...
        with patch("shutil.which", return_value=None):
            with patch.object(sys, "platform", "win32"):
                ...
                result = invoke_codeql_quick_scan._is_codeql_installed(str(tmp_path))
...
class TestModuleAsScript:
    ...
    def test_invoke_codeql_quick_scan_as_script(self):
        hook_path = str(
