...
Structure your response exactly as follows. The output format block below is synchronized with `.github/prompts/issue-feature-review.md` so on-demand reviewers return the same shape. Keep the output format block in sync when changing it. The rest of this agent file (Core Behavior, Decision tables, Evaluation Criteria, Constructive Skepticism) is Claude Code-specific and is intentionally not mirrored.
...
**Use repository and GitHub issue context only.** Do not use general web browsing or web search. The canonical prompt constrains the reviewer to the issue body and repo contents, while allowing GitHub API access through the approved tools listed above when available. This agent enforces the same constraint across on-demand agent surfaces. When data beyond repo contents and approved GitHub tooling is needed, state `UNKNOWN - requires manual research by maintainer`.
...
> **Canonical sync**: Evaluation framework mirrors `.github/prompts/issue-feature-review.md` for on-demand review. Keep both in sync when changing review logic.
