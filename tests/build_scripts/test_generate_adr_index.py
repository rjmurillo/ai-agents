...
def test_the_documented_recipe_agrees_with_the_generator_on_a_padded_closing_fence(tmp_path):
    """Both ACCEPT a padded closing fence now. The invariant is agreement.

    This test previously asserted both RAISE. That was correct for the contract
    of the day: `_FRONTMATTER_RE` required the closing fence to be three dashes
    followed immediately by `\\r?\\n`, so `"--- \\n"` matched nothing and
    `parse_frontmatter` reported it as unterminated.

    Issue #5275 measured what that cost. `check_adr_lifecycle.py` and
    `detect_adr_changes.py` both parsed the very same file cleanly and ran every
    lifecycle check against real values, so one trailing space, which no author
    would notice, passed the governance gate and crashed the index build. Three
    parsers, three answers.

    Both sides now delegate to `scripts/validation/frontmatter_contract.py` and
    therefore to `python-frontmatter`'s boundary, `^-{3,}\\s*$`, which treats a
    padded fence as a close. The expected value flipped deliberately, in the
    commit that migrated the parsers, and not incidentally: what this test
    asserts is still that the shipped recipe and the generator agree, which is
    the property that was broken. `test_the_recipe_and_generator_agree_on_an_
    unterminated_block` below pins the case that must still raise, so the flip
    cannot be read as "the recipe stopped checking".
    """
    adr_dir = tmp_path / "architecture"
...
def test_the_recipe_and_generator_agree_on_an_unterminated_block(tmp_path):
    """The fence contract loosened; the loud failure did not.

    `frontmatter.loads` alone would regress this: it returns empty metadata for
    an absent block and for an unterminated one alike, so a record with a broken
    schema would land silently in Needs backfill. The recipe calls `detect` and
    `split` precisely so the two stay distinguishable.
    """
    from generate_adr_index import AdrIndexError

    adr_dir = tmp_path / "architecture"
    adr_dir.mkdir(parents=True)
    (adr_dir / "ADR-001-unterminated.md").write_text(
        "---\nid: ADR-001\nstatus: accepted\n# ADR-001: Open\n\n## Decision\n\nDo it.\n",
        encoding="utf-8",
    )
...
def test_the_recipe_and_generator_agree_on_trailing_text_after_the_fence(tmp_path):
    """`--- nope` is the slip the contract still refuses to read as a close."""
    from generate_adr_index import AdrIndexError

    adr_dir = tmp_path / "architecture"
    adr_dir.mkdir(parents=True)
    (adr_dir / "ADR-001-trailing.md").write_text(
        "---\nid: ADR-001\nstatus: accepted\n--- nope\n"
        "# ADR-001: Trailing\n\n## Decision\n\nDo it.\n",
        encoding="utf-8",
    )
