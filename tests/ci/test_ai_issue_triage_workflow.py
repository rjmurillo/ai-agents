...
def test_prd_main_invalid_complexity_score_returns_config_error(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
):
    monkeypatch.setenv("ISSUE_NUMBER", "42")
    ...
    assert prd_comment_mod.main() == 2
    assert "::error::COMPLEXITY_SCORE must be an integer" in capsys.readouterr().err


def test_prd_main_missing_issue_returns_config_error(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
):
    ...
    assert prd_comment_mod.main() == 2
    assert "::error::ISSUE_NUMBER is required" in capsys.readouterr().err
...
def test_summary_main_missing_issue_returns_config_error(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
):
    monkeypatch.delenv("ISSUE_NUMBER", raising=False)
