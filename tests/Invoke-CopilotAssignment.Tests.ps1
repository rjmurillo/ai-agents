...
Describe "Get-MaintainerGuidance Function" {
    ...
    Context "RFC 2119 Keyword Extraction" {
        It "Extracts sentences with MUST keyword when no bullets exist" {
            $comments = @(
                @{
                    user = @{ login = "rjmurillo" }
                    body = "The skill files MUST NOT be created directly. They SHOULD be generated by the agent."
                }
            )
            $result = Get-MaintainerGuidance -Comments $comments -Maintainers @("rjmurillo")
            $result | Should -Not -BeNullOrEmpty
            $result.Count | Should -Be 2
            $result[0] | Should -Match "MUST NOT"
            $result[1] | Should -Match "SHOULD"
        }

        It "Prefers bullet points over RFC 2119 extraction" {
            $comments = @(
                @{
                    user = @{ login = "rjmurillo" }
                    body = "- This is a bullet point item for guidance`nFiles MUST be committed."
                }
            )
            $result = @(Get-MaintainerGuidance -Comments $comments -Maintainers @("rjmurillo"))
            # Should extract bullet point, not the MUST sentence
