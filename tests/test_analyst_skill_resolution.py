...
# Regression: prior versions returned BLOCKED immediately on PR URL/number
# input without attempting retrieval via declared read-only tools.
...
def test_pull_request_read_declared_on_every_surface(path: Path) -> None:
    """pull_request_read must be in every analyst surface (precondition for retrieval)."""
    text = path.read_text()
...
def test_blocked_requires_retrieval_failure(path: Path) -> None:
    ...
    assert blocked_idx != -1, f"{path.relative_to(REPO_ROOT)}: no [BLOCKED] directive"
    context = text[max(0, blocked_idx - 300):blocked_idx + 300].lower()
    has_conditional = any(
        word in context
        for word in ("unavailable", "missing", "fails", "after", "if required")
    )
    assert has_conditional, (
        f"{path.relative_to(REPO_ROOT)}: [BLOCKED] not conditioned on retrieval failure"
    )
...
def test_local_identity_conditional_in_template() -> None:
    """Local identity columns must be conditional for remote-only analysis."""
    text = SHARED_TEMPLATE.read_text()
    assert "when present" in text.lower() or "only when" in text.lower(), (
        "template: local identity must be conditional"
    )


@pytest.mark.parametrize("path", ALL_ANALYST_SURFACES, ids=lambda p: str(p.relative_to(REPO_ROOT)))
def test_no_web_access_stated(path: Path) -> None:
    """Every analyst surface must state no web access."""
    text = path.read_text().lower()
    assert "no web access" in text or "no shell or unrestricted web" in text or \
           "analyst has no web access" in text, (
        f"{path.relative_to(REPO_ROOT)}: no-web-access statement missing"
    )
