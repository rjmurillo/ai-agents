    ...
    for segment in _shell_command_segments(tokens):
        ...
        for option_index, token in enumerate(command_tokens):
            if token.startswith("--prefix="):
                ...
                checks_out_workspace = prefix in {
                    "$GITHUB_WORKSPACE/",
                    "${GITHUB_WORKSPACE}/",
                } or (effective_workspace and prefix in {"", "./"})
            ...
            if token == "--prefix" and option_index + 1 < len(command_tokens):
                ...
                return checks_out_workspace, in_workspace
        return effective_workspace, in_workspace
...
class TestFirstUnmetRepoDependency:
    ...
    def test_checkout_index_after_changing_directory_does_not_satisfy_dependency(
        self,
    ) -> None:
        job = {
            "steps": [
