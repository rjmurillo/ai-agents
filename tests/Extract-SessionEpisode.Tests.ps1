...
Describe "ConvertFrom-Events" {
    ...
    Context "Milestone events" {
        ...
        It "Does not match bold markdown status headers" {
    ...
    Context "Edge cases" {
        ...
        It "Returns single-element array for one event" {
            $lines = @("- ✅ Task complete")
            $result = ConvertFrom-Events -Lines $lines
            $result | Should -BeOfType [System.Array]
            $result.Count | Should -Be 1
...
Describe "ConvertFrom-Lessons" {
    ...
    Context "Edge cases" {
        ...
        It "Returns empty array for zero lessons" {
            $lines = @("# Title", "Just text")
            $result = ConvertFrom-Lessons -Lines $lines
            $result | Should -BeOfType [System.Array]
            $result.Count | Should -Be 0
        ...
        It "Returns single-element array for one lesson" {
