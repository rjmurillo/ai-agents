"""Test utilities for the gate_latency modules: captured output and stubs.

Separated from ``conftest.py`` so each file holds one concern. ``conftest.py``
is pytest's fixture surface and now defines only the ``repo`` fixture; this
module holds the captured lefthook output, the subprocess fakes, and the
shared invocation helpers, which are ordinary functions any test can import.

The code-qualities axis scored the combined file 5.6 cohesion against a floor
of 7, which is the same complaint in test form: a fixture surface and a
utility library are two things.
...
from scripts.metrics import gate_latency_sampler
...
def run_one_repetition(
    repo: Path,
    *,
    lefthook_cmd: list[str] | None = None,
    hook: str = "pre-commit",
    files: tuple[str, ...] = (),
) -> HookRun:
    """Drive one repetition with this suite's defaults.

    Five tests issued the same call and differed only in what they asserted
    about the result. Sharing the invocation keeps each test's assertions
    distinct, which is what testing.md asks for (one behavior per test), while
    removing the repetition the code-qualities non-redundancy score counts.
    """
    return gate_latency_sampler._run_repetition(
        repo, lefthook_cmd or ["lefthook"], hook, files, 0
    )
