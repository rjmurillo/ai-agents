...
@pytest.mark.parametrize(
    "text",
    [
        "Save to: `.project-toolkit/qa/report.md`\n",
        "Generate the map at `.project-toolkit/pr-comments/PR-1/comments.md`.\n",
        "## Output\n\nWrite report to `.project-toolkit/retrospective/today.md`.\n",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "The old instruction said save to `.project-toolkit/qa/x.md`. "
...
def test_historical_marker_without_reason_does_not_suppress() -> None:
    text = "Save to `.project-toolkit/qa/x.md`. agents-write-target: historical\n"
...
def test_findings_are_located_and_sorted(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    (tmp_path / "docs").mkdir()
    (tmp_path / "docs" / "b.md").write_text("Save to `.project-toolkit/b`.\n", encoding="utf-8")
    (tmp_path / "docs" / "a.md").write_text("Save to `.project-toolkit/a`.\n", encoding="utf-8")
