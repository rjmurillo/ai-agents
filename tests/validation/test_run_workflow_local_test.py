...
# --- issue #3948: process-group kill on timeout, #3949: timeout budget ----


def test_run_timeout_returns_124_and_kills_process_group():
    """_run kills the whole process group and returns 124 on TimeoutExpired (#3948)."""
    mock_proc = MagicMock()
    mock_proc.pid = 42
    mock_proc.communicate.side_effect = [
        subprocess.TimeoutExpired(cmd=["gh", "act"], timeout=1),
        ("out", "err"),
    ]
    ...
    with (
        ...
        rc, stdout, stderr = w._run(["gh", "act"], timeout=1)

    assert rc == w._TIMEOUT_RC
    assert killed == [(42, __import__("signal").SIGTERM)]
    assert stdout == "out"
    assert stderr == "err"


def test_run_timeout_survives_process_already_gone():
    """_run does not raise if the process exits between timeout and killpg."""
    mock_proc = MagicMock()
    mock_proc.pid = 99
    mock_proc.communicate.side_effect = [
        subprocess.TimeoutExpired(cmd=["gh", "act"], timeout=1),
    ...
    with (
        ...
        rc, _stdout, _stderr = w._run(["gh", "act"], timeout=1)

    assert rc == w._TIMEOUT_RC


def test_run_happy_path_returns_subprocess_rc():
    """_run returns the real subprocess exit code on success."""
    mock_proc = MagicMock()
    mock_proc.pid = 7
    ...
    with patch("subprocess.Popen", return_value=mock_proc):
        rc, stdout, _stderr = w._run(["echo", "hello"], timeout=10)

    assert rc == 0
    assert stdout == "hello"


def test_act_dryrun_timeout_is_a_warning_not_a_block(monkeypatch, tmp_path):
    """rc=124 from dry-run produces a [WARN] and stage.ok=True (#3949)."""
    wf = _write_wf(tmp_path, "name: x\non: push\njobs: {}\n")
    monkeypatch.setattr(
        w,
        "_run",
        lambda cmd, *, timeout, cwd=None, env=None: (w._TIMEOUT_RC, "", ""),
    )
    res = w._act_dryrun_stage([wf.name], tmp_path)
    assert res.ok is True
    assert "[WARN]" in res.detail
    assert "cold action cache" in res.detail


def test_act_full_timeout_is_a_warning_not_a_block(monkeypatch, tmp_path):
    """rc=124 from full run is also a [WARN] so CI cold-cache does not hard-fail."""
    wf = _write_wf(tmp_path, "name: x\non: push\njobs: {}\n")
    monkeypatch.setattr(
        w,
        "_run",
...
def test_act_dryrun_non_timeout_failure_still_blocks(monkeypatch, tmp_path):
    """rc != 0 and rc != 124 keeps the gate hard-red (regression guard)."""
    wf = _write_wf(tmp_path, "name: x\non: push\njobs: {}\n")
    monkeypatch.setattr(
        w,
        "_run",
        lambda cmd, *, timeout, cwd=None, env=None: (1, "", "something went wrong"),
    )
    res = w._act_dryrun_stage([wf.name], tmp_path)
    assert res.ok is False


def test_timeout_rc_constant_is_124():
    """_TIMEOUT_RC must equal 124 to match GNU timeout(1) convention."""
    assert w._TIMEOUT_RC == 124


def test_act_dryrun_timeout_is_300_seconds():
    """Dryrun budget raised to 300 s to survive cold action cache (#3949)."""
    assert w._ACT_DRYRUN_TIMEOUT == 300
