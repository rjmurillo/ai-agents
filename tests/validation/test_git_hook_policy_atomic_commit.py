...
def test_atomic_commit_counts_deleted_files_over_limit(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Advisory since ADR-100 item 2: over-limit still reports, never blocks."""
    repo = tmp_path / "repo"
    _init_repo(repo)
...
def test_atomic_commit_counts_renamed_files_over_limit(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Advisory since ADR-100 item 2: over-limit still reports, never blocks."""
    repo = tmp_path / "repo"
...
def test_atomic_commit_message_names_only_local_remedy(
    ...
    assert "Consider splitting this commit." in captured.err
    assert "commit-limit-bypass" not in captured.err


def test_atomic_commit_never_blocks_regardless_of_file_count(
    tmp_path: Path,
) -> None:
    """Negative control for ADR-100 item 2: no authored-file count blocks.

    A blocking implementation would return 1 well before 50 files. The
    advisory implementation must return 0 no matter how large the count.
    """
    repo = tmp_path / "repo"
    _init_repo(repo)
    _commit_file(repo, "README.md", "init\n")
    _stage_files(repo, [f"file_{index}.txt" for index in range(50)])
