...
class TestAdrPolicyUsesBranchLog:
    """check_adr_review_policy judges evidence against the current branch's log."""
    ...
    def test_uses_current_branch_log_not_mtime_winner(
        ...
        branch_log.write_text(
            json.dumps(
                {
                    "session": {"branch": "feature/adr-fix"},
                    "protocolCompliance": {
                        "adrReview": {"Complete": True, "Evidence": "reviewed ADR-099"}
                    },
                }
            )
        )
    ...
    def test_fails_when_no_session_log_for_branch(
        ...
        """Gate fails when _session_log_for_current_branch returns None."""
        monkeypatch.setattr(policy, "_gated_adr_review_paths", lambda paths, root: list(paths))
        monkeypatch.setattr(policy, "_merge_in_progress", lambda root: False)
        monkeypatch.setattr(
            policy, "_session_log_for_current_branch", lambda sessions_dir, root: None
        )

        result = policy.check_adr_review_policy(
            [".agents/architecture/ADR-999-test.md"], tmp_path
        )
    ...
    def test_gate_calls_session_log_for_current_branch_not_today(
        ...
        """check_adr_review_policy calls _session_log_for_current_branch, not _today_session_log."""
        monkeypatch.setattr(policy, "_gated_adr_review_paths", lambda paths, root: list(paths))
        monkeypatch.setattr(policy, "_merge_in_progress", lambda root: False)
        ...
        assert called_branch, "check_adr_review_policy must call _session_log_for_current_branch"
        assert not called_today, (
            "check_adr_review_policy must NOT call _today_session_log directly"
        )
