...
4c. **Always-on correctness pass.** Load `resources/technical-review.md`, resolved like the axis prompts with `resources/` in place of `references/`. Invoke `Task(subagent_type="code-reviewer")`, falling back to `general-purpose`, with that contract as the response contract and the step 1 diff as input. Scope-check its output as in 4a, then `extract_verdict`; any failure is `UNKNOWN`.
