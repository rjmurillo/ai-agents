...
<!-- vendor-portability: contributor-facing pre-push gate for the rjmurillo/ai-agents
     repo itself. It intentionally reads .github/prompts/pr-quality-gate-qa.md
...
Run the qa axis of the quality gate against your working changes, before you
push, using the same criteria CI will apply.

Migrated from the pr-quality/qa command under ADR-064, which makes skills the
single user-invocable surface and renames the namespaced sub-command
pr-quality/qa to pr-quality-qa. The command file is gone, so its path is named
...
Then emit a fenced JSON block conforming to `.project-toolkit/schemas/pr-quality-gate-output.schema.json` with `"agent": "qa"`.
...
- [ ] The JSON block validates against the schema and carries `"agent": "qa"`
...
| Judging from memory of the axis | The criteria change and the recalled version is the one that was true once | Read `@.github/prompts/pr-quality-gate-qa.md` every run |
...
- **New criterion.** Add it to `.github/prompts/pr-quality-gate-qa.md`, which
