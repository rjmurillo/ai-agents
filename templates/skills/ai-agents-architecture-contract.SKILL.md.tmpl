...
<!-- vendor-portability: contributor-facing knowledge pack for the rjmurillo/ai-agents repo itself. It intentionally references .agents/governance, .claude/lib, scripts/hook_utilities, scripts/github_core, scripts/ai_review_common, build/scripts/lib_mirror.py, scripts/validation, build/generate_agents.py, build/generate_agent_catalog.py, build/scripts, templates/agents, and templates/platforms because its audience is repo contributors, not plugin consumers. Issue #2050. -->
...
| Shared Python libs | `scripts/hook_utilities`, `scripts/github_core`, `scripts/ai_review_common` | `src/claude/lib/{hook_utilities,github_core,ai_review_common}` and `src/copilot-cli/lib/{hook_utilities,github_core,ai_review_common}` direct (imports rewritten to relative); `.claude/lib/{hook_utilities,github_core,ai_review_common}` arrives second, binplaced from `src/claude/lib/` | `build_all.py`'s lib step (`build/scripts/lib_mirror.py`, ADR-109 B5); `--check` is the CI dry-run |
...
Direction rule: generators read canonical, write mirrors. They NEVER write `.claude/`. That invariant is enforced in code: `assert_no_claude_writes` (`build/scripts/build_all.py:888`), applied at lines 2221 to 2230 of the same file under the comment "REQ-003-010: enforce .claude/ no-write invariant"; any generator write under `.claude/` prints `REQ-003-010 VIOLATION` and exits 2. ADR-108 carves out one class, the template-owned skill files under `.claude/skills/`, passed to the guard as an allowlist. When a drift check goes red, the output shows a difference, not a direction. Always answer "which side is canonical?" from the table above before editing either side. An agent once "fixed" drift by editing the source to match the generated tree (2025-12-15 incident, commit reverted); the archaeology lives in `ai-agents-failure-archaeology`.
...
|---|---|---|
| Generators never write `.claude/` (except the ADR-108 template-owned skill files) | `assert_no_claude_writes` at `build_all.py:888`, applied at lines 2221 to 2230, exit 2 | Canonical tree gets silently overwritten by its own mirror; source of truth inverts |
...
| `.claude/lib/` matches `scripts/` packages | `build_all.py --check` | Plugin-distributed hooks import different code than the tested originals |
