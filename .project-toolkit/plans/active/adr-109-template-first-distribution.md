...
- [x] B3: the remaining 93 skills templated in batches; skill_templates.py's render target moved to `src/claude/skills/`, giving skills the `src/claude/` plugin tree B1 did not deliver; the ADR-108 pilot-scope pin retired once the full 111-skill set matched `discover()` (TASK-033). Five batches merged (`696280ab7` #5773, `b7ced340e` #5780, `55fc554ff` #5781, `5c87a5558` #5783, `8bd3d8ad5` #5782); render-target move, `OWNED_PREFIXES` widening, and the `prompts` manifest row merged `23c3d5dd3` (PR #5785). Skill support files (`scripts/`, `references/`, `tests/`) mirrored into `src/claude/skills/` too, closing the gap B6's install verification found: `build/scripts/skill_support_mirror.py`, merged `77a44751b` (#5794).
...
- [x] B6: the marketplace switch; `claude-agents` retired, `project-toolkit` repointed to `./src/claude` (TASK-036). PR #5791. Install-parity criterion, initially BLOCKED (`src/claude/skills/<name>/` shipped only `SKILL.md` for 84 of 111 skills), closed once `feat/adr-109-b3-skill-support-files` merged (`77a44751b`, #5794) and this branch merged `main` again: `build_all.py --check` exits 0, `tests/e2e/test_claude_skill_support_mirror_parity.py` passes, and a direct file-by-file walk found zero missing twins. ADR-109's migration order (section 7) is now complete: B0 through B6 all landed.

## Milestones

### B0: the record itself

Exit criteria: ADR-109 accepted (lifecycle gate passes, `adr-review` round recorded if the owner requires one before B1 starts); the four staged amendment sites (ADR-052 Status note, ADR-107 property 1 and Related Decisions, REQ-003 D4/D9/REQ-003-010) turn from conditional to unconditional text; `.project-toolkit/architecture/README.md` lists ADR-109.
...
| B0-T3 index | S | `.project-toolkit/architecture/README.md` regenerated; ADR-052's row moves from Accepted to Retired |
...
| 2026-09-15 | `feat/adr-109-b3-skill-support-files` merged to `main` (`77a44751b`, #5794): `build/scripts/skill_support_mirror.py` mirrors every skill's `scripts/`, `references/`, and `tests/` into `src/claude/skills/<name>/`. Merged `main` into `feat/adr-109-b6-marketplace`; rebuilt (`build_all.py --check` exit 0); switched `test_resolve_lib_dir.py`'s CLI fixtures to source the script from `src/claude/skills/merge-resolver/scripts/`, the marketplace-installed artifact, and added `test_marketplace_script_exists` as the negative control CodeRabbit asked for; verified zero missing support-file twins by direct file walk and `tests/e2e/test_claude_skill_support_mirror_parity.py`. TASK-036's install-verification criterion and B6's checkbox both flipped to done. ADR-109's migration order is complete: B0 through B6 all landed. | claude |
...
- No blockers remain. B6's install-parity gate closed once `feat/adr-109-b3-skill-support-files` merged (`77a44751b`, #5794) and `main` was merged into `feat/adr-109-b6-marketplace`. ADR-109's migration order is complete: B0 through B6 all landed.
...
- Prior art: ADR-108, REQ-025, DESIGN-024, TASK-030, TASK-025, and the completed plan `.project-toolkit/plans/completed/5706-skill-guidance-excerpts.md`, whose compile-and-drift-gate shape this plan generalizes
