...
|------|-----------------|------------------|----------------|-----------|
| analyst.shared.md | 2 | 0 | Hybrid | Line 822: `Save to: .project-toolkit/analysis/` |
| architect.shared.md | 7 | 0 | Hybrid | Multiple `.project-toolkit/architecture/` references |
| critic.shared.md | 12 | 0 | Hybrid | Extensive `.project-toolkit/critique/` output paths |
| devops.shared.md | 3 | 0 | Hybrid | `.project-toolkit/devops/` references |
| explainer.shared.md | 2 | 0 | Hybrid | `.project-toolkit/planning/` PRD output |
| high-level-advisor.shared.md | 0 | 0 | Hybrid | Generic but part of output path convention |
| implementer.shared.md | 4 | 0 | Hybrid | References `.project-toolkit/planning/` for input |
...
| planner.shared.md | 6 | 0 | Hybrid | `.project-toolkit/planning/` output paths |
| pr-comment-responder.shared.md | 22 | 0 | Hybrid | Extensive `.agents/` references |
| qa.shared.md | 8 | 0 | Hybrid | `.project-toolkit/qa/` output paths |
| retrospective.shared.md | 2 | 0 | Hybrid | `.project-toolkit/retrospective/` output |
| roadmap.shared.md | 5 | 0 | Hybrid | `.project-toolkit/roadmap/` references |
| security.shared.md | 5 | 0 | Hybrid | `.project-toolkit/security/` output paths |
| skillbook.shared.md | 0 | 0 | Hybrid | Generic but part of output path convention |
| task-generator.shared.md | 4 | 0 | Hybrid | `.project-toolkit/planning/` references |
...
|-------|-----------|-------------|--------------|----------------|-----------|
| adr-review | 5 | 0 | 0 | Hybrid | Hard-codes `.project-toolkit/architecture/ADR-*.md` |
...
| memory | 5 | 1 | 0 | Hybrid | `.project-toolkit/sessions/` hard-coded |
| merge-resolver | 8 | 0 | 1 | Hybrid | `.agents/` conflict resolution paths |
| metrics | 4 | 0 | 0 | Hybrid | `.agents/` metrics output |
| session-init | 13 | 0 | 0 | Hybrid | Heavy `.project-toolkit/sessions/` coupling |
| session-log-fixer | 4 | 0 | 0 | Hybrid | `.project-toolkit/sessions/` validation |
| session-migration | 13 | 0 | 0 | Hybrid | `.project-toolkit/sessions/` migration logic |
| session-qa-eligibility | 26 | 1 | 0 | Hybrid | Heavy `.project-toolkit/qa/` references |
...
```python
SESSION_DIR = os.environ.get("AWESOME_AI_SESSIONS_DIR", ".project-toolkit/sessions")
ANALYSIS_DIR = os.environ.get("AWESOME_AI_ANALYSIS_DIR", ".project-toolkit/analysis")
```
...
| Invoke-QAAgentValidator.ps1 | 3 | Hybrid | `.project-toolkit/qa/` validation |
| Invoke-SessionStartMemoryFirst.ps1 | 5 | Hybrid | `.agents/` memory checks |
| Invoke-SessionLogGuard.ps1 | 4 | Hybrid | `.project-toolkit/sessions/` validation |
| Invoke-ADRReviewGuard.ps1 | 4 | Hybrid | `.project-toolkit/architecture/` enforcement |
...
| Invoke-SessionValidator.ps1 | 3 | Hybrid | `.project-toolkit/sessions/` validation |
...
| Invoke-ADRChangeDetection.ps1 | 1 | Hybrid | `.project-toolkit/architecture/` detection |
...
```powershell
Reason = "Session log mentions adr-review, but no debate log artifact found in .project-toolkit/analysis/"
```

**Recommended fix:**

```powershell
$AnalysisDir = if ($env:AWESOME_AI_ANALYSIS_DIR) { $env:AWESOME_AI_ANALYSIS_DIR } else { ".project-toolkit/analysis" }
...
|----------|-----------|-------------|--------------|----------------|-----------|
| ai-session-protocol.yml | 4 | 0 | 0 | Hybrid | `.project-toolkit/sessions/` validation |
...
| validate-planning-artifacts.yml | 2 | 2 | 0 | Hybrid | `.project-toolkit/planning/` + `ai-agents` |
...
| invoke_security_retrospective.py | 4 | Hybrid | `.project-toolkit/security/` output |
| Fix-PR964-Validation.ps1 | 2 | Hybrid | `.agents/` validation |
| Validate-Traceability.ps1 | 4 | Hybrid | `.agents/` traceability checks |
| Invoke-SessionStartGate.ps1 | 7 | Hybrid | Heavy `.project-toolkit/sessions/` coupling |
| Validate-SessionJson.ps1 | 11 | Hybrid | Heavy `.project-toolkit/sessions/` validation |
...
```powershell
$SessionDir = ".project-toolkit/sessions"
```

**Recommended fix:**

```powershell
$SessionDir = if ($env:AWESOME_AI_SESSIONS_DIR) { $env:AWESOME_AI_SESSIONS_DIR } else { ".project-toolkit/sessions" }
```
...
- File path
- Hardcoded path to replace (e.g., `.project-toolkit/sessions`)
- Environment variable name (e.g., `AWESOME_AI_SESSIONS_DIR`)
- Default value (e.g., `.project-toolkit/sessions`)
