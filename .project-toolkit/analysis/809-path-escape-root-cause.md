...
When committing `.project-toolkit/sessions/2026-01-08-session-809.md`, the pre-commit hook failed with:

```
E_PATH_ESCAPE: Session log must be under .project-toolkit/sessions/: <project-root>.feat-Worktrunk-support/.project-toolkit/sessions/2026-01-08-session-809.md
```

The path shown IS under `.project-toolkit/sessions/`, yet validation claimed it was not. This indicates a bug in the validation logic itself, not a user error.
...
- Pre-commit hook line 792: Extracts relative path `.project-toolkit/sessions/2026-01-08-session-809.md` from git diff
...
**Example scenario**:
```
expectedDirNormalized = "/path/to/.project-toolkit/sessions"  (no trailing /)
expectedDirWithSep    = "/path/to/.project-toolkit/sessions/" (added /)
sessionFullPathNormalized = "/path/to/.project-toolkit/sessions" (no trailing /)
```
