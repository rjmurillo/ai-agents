...
4. `.project-toolkit/analysis/session-log-enforcement-bypass-vectors.md` - NOT in acceptance criteria (research artifact)
5. `.project-toolkit/security/SR-session-log-write-guard.md` - NOT in acceptance criteria (security report)
6. `.project-toolkit/sessions/2026-02-07-session-1183.json` - NOT in acceptance criteria (session log from PR work)
...
if [ -n "$STAGED_SESSION_LOG" ]; then
    ...
    if [[ "$STAGED_SESSION_LOG" =~ session-([0-9]+) ]]; then
        ...
        DUPLICATES=$(find .project-toolkit/sessions -name "*session-${SESSION_NUM}*" ! -path "$STAGED_SESSION_LOG" | wc -l)
