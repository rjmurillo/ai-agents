...
| `agent-workflow/agentskills-io-standard-integration.md` | keep | external standard reference; cited as a source by `.project-toolkit/architecture/SKILL-STANDARDS-RECONCILED.md` |
...
| `protocol/protocol-012-branch-handoffs.md` | workaround for a removed validator | `.project-toolkit/architecture/ADR-014-distributed-handoff-architecture.md` |
...
| `protocol/protocol-continuation-session-gap.md` | proposals for a retired protocol | `.project-toolkit/retrospective/2026-01-09-session-protocol-violation-analysis.md` holds the incident |
...
- `.project-toolkit/architecture/SKILL-STANDARDS-RECONCILED.md`: two memory paths
...
`.project-toolkit/specs/agent-orchestration-mcp-spec.md` carried a link to a memory path
...
Left unchanged: `.project-toolkit/sessions/`, `.project-toolkit/qa/`, `.project-toolkit/critique/`,
`.project-toolkit/retrospective/`, `.agents/archive/`, `.project-toolkit/memory/episodes/`, and
`.claude-mem/` backups. Those
record what was true at the time, which the issue says not to rewrite.
`.project-toolkit/analysis/5393-serena-workflow-inventory.md` also stands as the upstream
...
6. **Two ADRs with dangling memory links.**
   `.project-toolkit/architecture/ADR-011-session-state-mcp.md` and
