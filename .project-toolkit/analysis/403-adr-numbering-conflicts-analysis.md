...
**Scope**: All ADR files in `.project-toolkit/architecture/` directory. Excludes ADR-TEMPLATE.md and non-architecture ADR references in other directories.
...
**Background**:
- ADRs stored in `.project-toolkit/architecture/ADR-NNN-title.md` format
...
check_adr_uniqueness() {
    ...
    local adr_files=$(git diff --cached --name-only --diff-filter=A | grep -E "^\.project-toolkit/architecture/ADR-[0-9]+-.*\.md$" | grep -v "ADR-TEMPLATE.md")
    ...
    for new_adr in $adr_files; do
        ...
        local existing_count=$(find .project-toolkit/architecture -name "ADR-${new_number}-*.md" -o -name "ADR-0*${new_number}-*.md" | grep -v "ADR-TEMPLATE.md" | wc -l)
        ...
        if [ "$existing_count" -gt 1 ]; then
            ...
            find .project-toolkit/architecture -name "ADR-${new_number}-*.md" -o -name "ADR-0*${new_number}-*.md" | grep -v "ADR-TEMPLATE.md" | while read existing; do
                echo_error "    - $(basename "$existing")"
            done
            echo_info "  To fix:"
            echo_info "    1. Check last ADR number: ls .project-toolkit/architecture/ADR-*.md | sort -t- -k2 -n | tail -5"
    ...
    steps:
      ...
      - name: Check for duplicate ADR numbers
        run: |
          ...
          numbers=$(ls .project-toolkit/architecture/ADR-*.md | grep -v TEMPLATE | sed -E 's|.*ADR-0*([0-9]+)-.*|\1|' | sort -n)
          ...
          if [ -n "$duplicates" ]; then
            ...
            for dup in $duplicates; do
              ...
              ls .project-toolkit/architecture/ADR-*${dup}-*.md | while read file; do
      ...
      - name: Check for format consistency (3-digit zero-padded)
        run: |
          ...
          bad_format=$(ls .project-toolkit/architecture/ADR-*.md | grep -v TEMPLATE | grep -vE "ADR-[0-9]{3}-")
...
**Location**: `.project-toolkit/architecture/ADR-RESERVATIONS.md`
...
- File system enumeration: `.project-toolkit/architecture/ADR-*.md`
...
- Session logs: `.project-toolkit/sessions/2025-12-24-session-92-adr-renumbering.md`
