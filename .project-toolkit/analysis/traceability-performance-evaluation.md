...
- Cache storage: JSON files in `.project-toolkit/.cache/traceability/`
...
2. **Disk Cache** (cross-session): JSON files in `.project-toolkit/.cache/traceability/`
...
$cacheDir = ".project-toolkit/.cache/traceability/"
...
| `.project-toolkit/.cache/` directory | ✅ No (documented location) | ✅ Current |
...
**Recommendation**: Continue using `.project-toolkit/.cache/traceability/` (current approach).
...
- **Code**:
  ...
  $changedSpecs = git diff --name-only --diff-filter=M $lastRun |
      Where-Object { $_ -match '\.project-toolkit/specs/.*\.md$' }
...
```markdown
# .project-toolkit/metrics/traceability-performance.md
...
Add-Content ".project-toolkit/metrics/traceability-performance.md" $metrics
```
