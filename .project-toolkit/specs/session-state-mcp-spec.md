...
| SESSION_LOG | COMPLETE | .project-toolkit/sessions/2025-12-21-session-01.md |
...
async function session_start(params: SessionStartParams): Promise<SessionStartResult> {
  ...
  const existingSessions = await glob(`.project-toolkit/sessions/${today}-session-*.md`);
...
async function validate_gate(params: ValidateGateParams): Promise<ValidateGateResult> {
  ...
  switch (phase) {
    ...
    case "SESSION_LOG":
      ...
      const logPath = `.project-toolkit/sessions/${state.session_id}.md`;
...
async function saveState(state: SessionState): Promise<void> {
  await withSerenaFallback(
    ...
    () => writeFile(".project-toolkit/sessions/.session-state.json", JSON.stringify(state))
  );
}
```
