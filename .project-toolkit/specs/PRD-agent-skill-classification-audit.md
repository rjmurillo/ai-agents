...
- Lifecycle: audit table is **point-in-time**. The committed file at `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md` includes the date. Re-runs produce a new dated file; old files preserved for diff.
...
| `.project-toolkit/audits/` filesystem | Disk full, permission denied | Write-then-rename; partial writes detected via missing rename |
...
- **Metrics**: audit emits one row to `.project-toolkit/metrics/agent-skill-audit-runs.tsv` with 7 tab-separated columns: `timestamp | agent_count | verdict_skill | verdict_context_fork_skill | verdict_keep | verdict_merge | fm_bytes_saved_pct`. The 7th column quantifies load-budget impact (% of always-loaded frontmatter bytes saved if all skill+context-fork-skill rows migrate). Re-runs append rows; old rows preserved for diff.
...
1. **AC-1**: WHEN the audit is run THE SYSTEM SHALL produce a markdown file at `.project-toolkit/audits/YYYY-MM-DD-agent-skill-classification-audit.md` SO THAT the table is durable, version-controlled, and reviewable in a PR.
...
9. **AC-9**: THE SYSTEM SHALL include a summary footer with: total agents, count by verdict, total `c5_frontmatter_bytes` saved if all `verdict = skill` AND `verdict = context-fork-skill` rows were migrated (both verdicts represent shape-mismatch refactor candidates and contribute equally to load-budget reduction), and citations to ADR-030, ADR-036, and `.agents/governance/agent-consolidation-process.md` SO THAT readers see the audit's authority and impact at a glance.
...
| Artifact | Location |
|----------|----------|
| Requirement | `.project-toolkit/specs/requirements/REQ-011-agent-skill-classification-audit.md` |
| Design | `.project-toolkit/specs/design/DESIGN-011-agent-skill-classification-audit.md` |
| Task | `.project-toolkit/specs/tasks/TASK-011-agent-skill-classification-audit.md` |
| Audit output (Phase 1 deliverable) | `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md` (created by TASK-011) |
