...
(`.project-toolkit/architecture/ADR-100-retire-pr-size-ceilings.md`), an accepted
...
No committed baseline exists. `.project-toolkit/metrics/baseline-report.md` and
...
doc pair (`.project-toolkit/metrics/control-plane-baseline-v0.7.0.md` and `.json`)
...
See `.project-toolkit/specs/ontology/control-plane-subtraction-cohort-1.md`. This
...
- Reads (does not modify): `.claude/agents/`, `.claude/skills/`,
  ...
  `.project-toolkit/architecture/`, `.agents/governance/`, `.serena/memories/`,
  `.project-toolkit/memory/episodes/`, `.project-toolkit/sessions/`, `.agents/archive/`,
  `.project-toolkit/eval-results/`, `src/copilot-cli/`, `scripts/eval/examples/harness-capability-matrix.json`.
...
- **Scenario**: `.project-toolkit/metrics/` becomes a write target another in-flight
  effort (#5420) wants to relocate, creating a conflicting PR.
  **Category**: organizational. **Early warning**: a second open PR touches
  `.project-toolkit/metrics/` paths. **Detection**: PR conflict on push.
...
`.project-toolkit/metrics/`, and no new authentication surface.
...
- [ ] .project-toolkit/metrics/control-plane-baseline-v0.7.0.md:"new file" -- committed
      baseline doc, pinned SHA `cd0f9561d` (or later pinned SHA)
- [ ] .project-toolkit/metrics/control-plane-baseline-v0.7.0.json:"new file" --
...
- Relocating `.project-toolkit/metrics/` if #5420 lands a generated-state separation
...
- **OQ1**: Should `.project-toolkit/metrics/control-plane-baseline-v0.7.0.json`
