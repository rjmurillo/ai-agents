...
Entities from `.project-toolkit/specs/ontology/skill-guidance-excerpts.md` (revised for D1): rule file, always-on rule set, skill template, partial, template-owned skill file, skill mirror, compile step, drift gate, pilot skill, include line. Implements decision rules DR1 through DR6.
...
16. [ ] ADR-108 SHALL exist under `.project-toolkit/architecture/` with a recorded `adr-review` round, and `.project-toolkit/architecture/README.md` SHALL be regenerated to list it.
