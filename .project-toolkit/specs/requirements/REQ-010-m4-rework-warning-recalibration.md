...
issues:
  - 1990
predecessor: REQ-009
retrospective: .project-toolkit/retrospective/2026-05-10-pr-1989-recursive-failure.md
...
2. `.project-toolkit/retrospective/2026-05-10-pr-1989-recursive-failure.md` (PR #1989 bug history #7: threshold-6 returned 0 on test bed)
...
1. ~15 min: add `.project-toolkit/memory/episodes/` to `_REWORK_EXCLUDED_PREFIXES`
...
- Connected entity: `.project-toolkit/memory/episodes/episode-*.json` (in-scope addition to exclusion list)
...
WHEN `rework_warning.py` is invoked against a development branch that contains episode log files under `.project-toolkit/memory/episodes/` alongside edited source files,
THE SYSTEM SHALL exclude all files matching `_REWORK_EXCLUDED_PREFIXES` (including `.project-toolkit/memory/episodes/`) from the rework-candidate list and SHALL emit a warning for every source file whose edit count meets or exceeds `REWORK_THRESHOLD`,
...
1. The exclusion list omitted `.project-toolkit/memory/episodes/`, a path that accumulates high edit counts on every branch that touches agent memory. Episode files are generated artifacts, not rework candidates. Their presence inflated the noise floor and crowded out real signal.
...
- [ ] REQ-010-02: WHEN `_REWORK_EXCLUDED_PREFIXES` is evaluated, THE SYSTEM SHALL include `.project-toolkit/memory/episodes/`, SO THAT episode logs are filtered alongside session logs and src/claude/.
