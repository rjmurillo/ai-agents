...
WHEN an agent invokes `git push` AND the changeset contains any `.project-toolkit/sessions/*.json` file
...
- [ ] AC-6: WHEN an agent invokes `git push` AND the changeset contains any `.project-toolkit/sessions/*.json` file THE SYSTEM SHALL parse each session log and check:
...
- [ ] AC-8 (partial): WHEN no `.project-toolkit/sessions/*.json` files are in the changeset THE SYSTEM SHALL exit zero immediately.
...
- `.project-toolkit/sessions/` directory and JSON session log format (existing)
- `git diff --name-only @{push}..HEAD` for change detection
...
None. All scope questions resolved during the interview at `.project-toolkit/specs/interviews/INTERVIEW-1884-pr-iteration-cost.md` and during gap-analysis, pre-mortem, and decision-critic review.
