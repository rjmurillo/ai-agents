...
This design is derived from `.project-toolkit/analysis/command-skill-bundling-2026-05-03.md` (2026-05-03), which identified all gaps and proposed the fix for each command.
...
| Idempotency | `session-init` checks for existing log | Handled inside the skills; command edits do not need idempotency guards. <!-- orphan-ref-ignore --> |
| Idempotency | `reflect` skips when no HIGH-confidence learnings | Handled inside the skills; command edits do not need idempotency guards. |
...
|---|---|---|
| Static parse | `tests/test_command_bundles.py` | Each command file contains the `Skill(skill="...")` calls from the BundleRegistry. Parses markdown; does not execute commands. Tests use `xfail` marks for not-yet-edited rows so CI stays green during M1/M2. <!-- orphan-ref-ignore --> |
...
| CWE-78 check | `tests/test_command_bundles.py` | All git log commands in build.md and review.md quote file paths (independent regex grep, NOT shared with the bundle parser). <!-- orphan-ref-ignore --> |
...
- Requirements: `.project-toolkit/specs/requirements/REQ-005-command-skill-bundling.md`
- Tasks: `.project-toolkit/specs/tasks/TASK-005-command-skill-bundling.md`
- Analysis: `.project-toolkit/analysis/command-skill-bundling-2026-05-03.md`
