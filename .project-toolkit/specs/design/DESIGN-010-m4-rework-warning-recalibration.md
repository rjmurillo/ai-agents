...
The M4 rework detector's threshold of 6 is empirically correct. The exclusion list is incomplete. Adding `.project-toolkit/memory/episodes/` to `_REWORK_EXCLUDED_PREFIXES` and pinning the contract with a real-fixture test closes both defects in two targeted edits totalling fewer than 10 lines of production change.
...
| REQ-010-02: episodes prefix added | `.claude/skills/session-end/scripts/rework_warning.py:_REWORK_EXCLUDED_PREFIXES` | Append `.project-toolkit/memory/episodes/` <!-- orphan-ref-ignore --> |
...
After TASK-010-03, the tuple gains `.project-toolkit/memory/episodes/` as an additional entry. No other lines in the file change.
...
2. No path matching `.project-toolkit/memory/episodes/` appears in the rework warning list.
