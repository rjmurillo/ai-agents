...
REQ-026-001 through REQ-026-014, per the numbering in `.project-toolkit/specs/requirements/REQ-026-template-first-plugin-distribution.md`.
...
|---|---|---|
| DR1 (plugin-tree file equals canonical-source render) | `.project-toolkit/specs/ontology/template-first-distribution.md` O5 | Each new compile module's `compile_all`, mirroring `skill_templates.compile_all` |
...
2026-09-15 follow-up: B3 as landed only ever populated `src/claude/skills/<name>/SKILL.md`; `generate_skills.sync_claude_plugin_skill_support` now also mirrors each skill's non-`SKILL.md` support files (`.claude/skills/<name>/` stays their canonical source) into the same plugin tree, ahead of the binplace step in `build_all.py`'s generator sequence. The `skills` row's owned set (`skill_templates.owned_targets`) is unchanged, SKILL.md only: binplace's generic `plugin_tree.rglob("*")` walk (Per-class compile modules, above) does traverse the now-present support files too, but by the time it runs they already match `.claude/skills/` exactly (the mirror step ran first, from the same canonical source), so it never has a mismatch to write and the narrower allowlist is never exercised for them in practice.
