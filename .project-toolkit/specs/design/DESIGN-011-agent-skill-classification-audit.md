...
- Output 1: `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md`. Schema:
...
| Storage | `.project-toolkit/audits/` (new directory) | Convention matches `.project-toolkit/analysis/`, `.project-toolkit/critique/`, etc. |
...
- **Read-only check (NFR-2)**: `git status` after audit run shows changes only to `.project-toolkit/audits/`, `.project-toolkit/specs/`, and (optionally) `.project-toolkit/metrics/`.
...
This audit is the first artifact under `.project-toolkit/audits/`. Future audits (skill-side per #1950, periodic re-runs) follow the same convention.
