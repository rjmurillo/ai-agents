...
# TASK-028: Build and commit the control-plane baseline for v0.7.0

## Objective

Implement `scripts/metrics/control_plane_baseline.py`, its test suite, and
the committed baseline doc pair, satisfying REQ-024's twelve acceptance
criteria and DESIGN-023's component architecture.
...
Out of scope: anything REQ-024's Out of Scope section names (reduced-config
comparison, final release report, mechanism deletions, gate p50/p95
sampling, fan-out routing changes).

## Acceptance Criteria

- [ ] TASK-028-AC1: `scripts/metrics/control_plane_baseline.py` exists,
...
- [ ] TASK-028-AC2: Each of the seven dimension functions (`fanout_residue`
      removed post-implementation per independent review F2, see
      DESIGN-023's Data model Amendment note) in `control_plane_baseline.py`
      (not a `dimensions/*.py` package; see DESIGN-023's Component
      Architecture revision) exists and returns its dict or
      `None`-with-reason, independently testable.
- [ ] TASK-028-AC3: `always_loaded.py` imports `instruction_budget`'s token
      estimator; no second token-counting implementation exists in this
      script (REQ-024 AC-04, DR4).
- [ ] TASK-028-AC4: `gate_budget.py` imports (or, after extraction,
      imports from a shared module) the same summation function
      `tests/ci/test_lefthook_declared_budget.py` uses (REQ-024 AC-06,
      DR4). Resolve OQ3 first: confirm whether extraction is required.
- [ ] TASK-028-AC5: `tests/metrics/test_control_plane_baseline.py` covers
      positive, negative, edge, CLI exit-code, parity, reproducibility, and
      no-content-leak cases per DESIGN-023's Testing Strategy.
- [ ] TASK-028-AC6: A synthetic-value test matrix asserts exit 0 across
      metric values deliberately exceeding every release target (REQ-024
      AC-08); this is the test that makes DR1 verifiable, not aspirational.
- [ ] TASK-028-AC7: `.project-toolkit/metrics/control-plane-baseline-v0.7.0.md` and
      `.json` are committed at one pinned `main` SHA, with the exact
      command, every exclusion and its reason, and the release targets
      recorded (REQ-024 AC-12).
- [ ] TASK-028-AC8: `uv run pytest tests/metrics -x`, `uv run ruff check
...
**Revised at implementation time (design review, 2026-09-11)**: DESIGN-023's
...
| `tests/metrics/test_control_plane_baseline.py` | create | full test suite per DESIGN-023 |
| `scripts/ci/lefthook_budget_model.py` | move (from `tests/ci/`) | make the declared-budget summation importable from production code, not test code (REQ-024 OQ3, AC-04) |
...
| `.project-toolkit/critique/ADR-104-debate-log.md` | modify | update the one path mention of the moved module |
| `.project-toolkit/metrics/control-plane-baseline-v0.7.0.md` | create | committed baseline, pinned SHA |
| `.project-toolkit/metrics/control-plane-baseline-v0.7.0.json` | create | machine-readable twin |

## Implementation Notes

1. Resolve OQ3 (gate-budget summation location) and the DESIGN-023 open
...
5. Pin the baseline doc's commit SHA to the actual `HEAD` at commit time,
   not `cd0f9561d` if `main` has advanced; update REQ-024 AC-12's cited SHA
   in the PR description if it differs.

## Testing Requirements

Per DESIGN-023's Testing Strategy: positive, negative, edge, CLI exit-code,
