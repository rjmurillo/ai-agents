...
- [x] `ls templates/skills/*.tmpl | wc -l` equals `find .claude/skills -maxdepth 2 -name SKILL.md | wc -l` (both 111) once every batch lands.
- [x] `git diff --exit-code -- .claude/skills` exits 0 after `uv run python build/scripts/build_all.py` runs on a clean checkout, for every batch's PR. Verified for this PR's render-target-move commit.
...
- [x] The existing `Skill Template Drift` gate stays green after every batch (no new gate needed; the class's gate already exists from ADR-108).
- [x] `tests/build_scripts/test_skill_templates_pilot_scope.py`'s `PILOT` constant is deleted (and the file itself deleted, since its sole purpose was pinning the pilot boundary) once `discover(repo_root)` on the real tree returns all 111 names with no further owner-approval step required for a new one. Batch 3 (PR #5781, 20 skills) merged after the render-target-move PR opened; this branch merged `origin/main`, confirmed `discover(repo_root)` returns all 111 names with `discover_errors()` empty, deleted the pilot-scope test file, and added `test_skill_templates_full_migration.py` asserting `discover()` equals the on-disk `.claude/skills/*/SKILL.md` name set, so a skill with no template can no longer land silently.
- [x] `uv run pytest tests/build_scripts/test_generate_skills_template_compile.py tests/skills -v` passes for every migrated skill.
- [x] `ls src/claude/skills/*/SKILL.md | wc -l` returns 111 once the last batch lands; `git diff --exit-code -- .claude/skills` exits 0 after `uv run python build/scripts/build_all.py` runs on a clean checkout, confirming the binplace copy from `src/claude/skills/` reproduces today's `.claude/skills/` content exactly.
- [x] `grep -n "class: skills" -A 4 templates/platforms/binplace.yaml` shows `plugin_tree: src/claude/skills`, not `null`; `tests/build_scripts/test_binplace_manifest.py` passes with the updated row.
- [x] `grep -n "sourceDir" build/scripts/generate_skills.py` (or the equivalent read site) shows the Copilot-mirror copy step reading `src/claude/skills`, not `.claude/skills`; `git diff --exit-code -- src/copilot-cli/skills` exits 0 after a clean run. Corrected during implementation: `sourceDir` in the platform config stays `.claude/skills` (`_iter_skill_sources` still enumerates skill directories and copies non-`SKILL.md` files from there, since only `SKILL.md` is template-owned); the copy step's `SKILL.md` CONTENT read is what moved, to `src/claude/skills/<name>/SKILL.md` via the new `plugin_skill_md` parameter on `_copy_skill_tree`, so there is no literal `sourceDir` grep hit for `src/claude/skills`.
- [x] `grep -n "OWNED_PREFIXES" -A 8 build/scripts/build_all.py` lists `.claude/skills/` in the tuple; symlinking `.claude/skills/` and running `uv run python build/scripts/build_all.py --check` exits 2. Corrected during implementation (PR that lands this box): a blanket `.claude/skills/` prefix would also claim every skill's hand-maintained scripts, references, and tests as staleness-checked, so `build_all.py` widens `OWNED_PREFIXES` at call time with one `.claude/skills/<name>/SKILL.md` entry per name `skill_templates.owned_targets()` returns, not a bare `.claude/skills/` prefix in the tuple literal. A hand edit to any `.claude/skills/<name>/SKILL.md` still fails `--check` with exit 2 (verified). Verified end to end: replacing `.claude/skills` with a symlink to a directory outside the repository and running `build_all.py --check` exits 2, caught by `binplace_manifest.load()`'s `install_tree` containment check before the compile step runs.
- [x] `grep -n "class: prompts" -A 4 templates/platforms/binplace.yaml` shows the row (`source: .claude/skills/review/references`, `install_tree: .github/prompts`, `compile: generate_pr_quality_prompts`); `git diff --exit-code -- .github/prompts` exits 0 after a clean run, and only the twelve `pr-quality-gate-*.md` files are covered by the row (the other 21 stay hand-maintained, per REQ-026 Deferred). The row's `plugin_tree: null` means `binplace_manifest.binplace()` performs no copy for it; the twelve files are still generated the same way they already were, by `build/scripts/generate_pr_quality_prompts.py`, outside `build_all.py`'s `GENERATORS` pipeline. This row's only effect is documenting the class in the manifest; verified it does not change `.github/prompts/` content (`git diff --exit-code -- .github/prompts` is 0).
- [x] 2026-09-15 follow-up (found by the B6 install-verification e2e): this task's original scope moved only `SKILL.md`'s render target to `src/claude/skills/<name>/SKILL.md`; the 84 skills that also carry `scripts/`, `references/`, `tests/`, or other support files under `.claude/skills/<name>/` had nothing mirroring those into the same plugin tree, so `src/claude/skills/<name>/` was incomplete for every one of them. `build/scripts/generate_skills.py`'s `sync_claude_plugin_skill_support` mirrors every non-`SKILL.md` file from `.claude/skills/<name>/` (which stays canonical) into `src/claude/skills/<name>/`, the same way the Copilot mirror step already does for `src/copilot-cli/skills/<name>/`, with merge-resolver included (it is excluded only from the Copilot mirror). No `OWNED_PREFIXES` widening was needed: the tuple's blanket `src/` entry already covers `src/claude/skills/` support files.
