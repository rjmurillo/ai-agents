...
# TASK-029: Author and commit the v0.7.0 control-plane disposition ledger

## Objective

Write `.project-toolkit/metrics/control-plane-dispositions-v0.7.0.md`, classifying
every epic-named candidate plus the duplicate-gate finding, satisfying
REQ-022's seven acceptance criteria.

## In/Out of Scope

In scope: the ledger document itself, citing REQ-024's committed baseline.
...
- [ ] TASK-029-AC1: One row for each of: #5394, #5395, #5396, #5404,
      #5420/#5421, #5436, #5241, and redundant hooks/review-axes/
      validators/workflows/mirrors the baseline surfaces.
- [ ] TASK-029-AC2: The duplicate-gate row is present, classified `KEEP`.
      Citations: `pre_pr_sequence.py:543-556`, `lefthook.yml:590`, and
      `test_pre_pr_sequence_registry.py:133-266`.
- [ ] TASK-029-AC3: Every `KEEP` row's five required fields are non-empty
      (spot-checked at PR review, per DESIGN-021).
- [ ] TASK-029-AC4: ADR-100 item 5 recorded as `DELETE` (the construct
      #5241 describes is absent from `qa_report.py`) and item 6 recorded
      as `EXPERIMENT` (owned by #5238/#5239), each with a one-line
      reason.
- [ ] TASK-029-AC5: No new GitHub child issue created as a byproduct of
      writing this ledger.
- [ ] TASK-029-AC6: `uv run python scripts/validation/pre_pr.py` passes.

## Files Affected

| File | Action | Description |
|---|---|---|
| `.project-toolkit/metrics/control-plane-dispositions-v0.7.0.md` | create | disposition ledger |

## Implementation Notes

1. Read REQ-024's committed baseline doc first; every numeric evidence
...
4. Resolve REQ-022 OQ1 (row order) and OQ2 (redundancy scope) per the
   assumptions recorded there, revisiting only if TASK-028's baseline
