...
- **PRD**: `.project-toolkit/specs/PRD-agent-skill-classification-audit.md`.

## Objective

Produce the Phase 1 audit deliverable for issue #2003: a 23-row table classifying every agent in `.claude/agents/` against five criteria, with verdicts and Phase 2 backlog, committed at `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md` and linked from a comment on issue #2003.
...
- [ ] **TAC-1** (covers AC-1, AC-3, AC-4, AC-7, AC-8): file `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md` exists with exactly 23 data rows, every cell populated, every score has an evidence citation, `has_shared_template` and `c5_frontmatter_bytes` columns present.
...
- [ ] **TAC-7** (NFR-1, NFR-2): the audit-execution step writes only documentation and session-protocol artifacts: `.project-toolkit/audits/`, `.project-toolkit/specs/`, `.project-toolkit/metrics/`, plus the session log and episode the work session necessarily produces (`.project-toolkit/sessions/`, `.project-toolkit/memory/episodes/`). The audit itself makes no changes under `.claude/agents/`, `templates/agents/`, or `src/` (any `src/` delta in the PR is merge propagation from `main`, not the audit): it identifies skill candidates but does not migrate them.
...
- Write file `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md`.
- Append metrics line to `.project-toolkit/metrics/agent-skill-audit-runs.tsv`.
...
| Action | Path | Purpose |
|--------|------|---------|
| Create | `.project-toolkit/audits/2026-05-10-agent-skill-classification-audit.md` | Phase 1 deliverable |
| Append | `.project-toolkit/metrics/agent-skill-audit-runs.tsv` | Re-run diff support |
