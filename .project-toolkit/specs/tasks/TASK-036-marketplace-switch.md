...
- [x] `.project-toolkit/architecture/README.md` needs no change from this task alone (ADR-109's acceptance-time edits to ADR-052's status and the README's Accepted-to-Retired row move are governed by the ADR lifecycle process, not this implementation task).
