...
| File | Action | Description |
|---|---|---|
| `.project-toolkit/architecture/ADR-NNN-agent-eval-discipline.md` | create | Full ADR covering corpus, scoring, baseline, threshold, cadence |

**In Scope** (all required per AC-6):
- ADR number reservation: at the START of the T4-6 working session, run `ls .project-toolkit/architecture/ADR-0*.md | tail -1` to determine the next available number. Claim it locally by creating the file with frontmatter (status: proposed) and the full body in a single working session. The placeholder and full content land in **one commit**, not two: the budget table reflects this. This prevents collision with concurrent ADR PRs while staying within the file budget.
...
**Acceptance Criteria**:
- [ ] ADR follows `.project-toolkit/architecture/ADR-*.md` convention (frontmatter, status: proposed)
...
| `.project-toolkit/architecture/ADR-NNN-agent-eval-discipline.md` | T4-6 | create |

---

## Related Documents

- Requirements: `.project-toolkit/specs/requirements/REQ-004-agent-eval-harness-spike.md`
