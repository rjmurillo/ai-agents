...
| `AWESOME_AI_SESSIONS_DIR` | Consumer session log directory | `.project-toolkit/sessions` |
| `AWESOME_AI_HANDOFFS_DIR` | Consumer per-issue handoff directory | `.project-toolkit/sessions/handoffs` |
| `AWESOME_AI_ARCHITECTURE_DIR` | Consumer ADR directory | `.project-toolkit/architecture` |
...
```markdown
Save to: .project-toolkit/analysis/NNN-[topic]-analysis.md
```
...
```python
# Before (hard-coded):
SESSION_DIR = ".project-toolkit/sessions"

# After (parameterized with validation):
SESSION_DIR = get_safe_path("AWESOME_AI_SESSIONS_DIR", ".project-toolkit/sessions", PROJECT_ROOT)
```
...
| PR quality prompts | `.project-toolkit/prompts/` or inline | 27 |
...
| File | Role in Plan |
|------|-------------|
| `.project-toolkit/analysis/claude-code-plugin-marketplaces.md` | Plugin format reference (session 1180 research) |
| `.project-toolkit/analysis/adr-045-inventory-audit.md` | File-by-file inventory verification |
