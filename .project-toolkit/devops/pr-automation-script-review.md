...
$metrics | ConvertTo-Json | Out-File '.project-toolkit/logs/pr-maintenance-metrics.json'
```
...
- name: Parse metrics
  ...
  shell: pwsh
  run: |
    $metrics = Get-Content .project-toolkit/logs/pr-maintenance-metrics.json | ConvertFrom-Json
...
```powershell
# State file: .project-toolkit/logs/pr-maintenance-state.json
$stateFile = '.project-toolkit/logs/pr-maintenance-state.json'
...
```powershell
$lockFile = '.project-toolkit/logs/pr-maintenance.lock'
    ...
    steps:
      ...
      - name: Run PR maintenance
        ...
        run: |
          ...
          $params = @{
            MaxPRs = $maxPRs
            LogPath = '.project-toolkit/logs/pr-maintenance.log'
          }
      ...
      - name: Parse results
        ...
        run: |
          ...
          $log = Get-Content .project-toolkit/logs/pr-maintenance.log -Raw
      ...
      - name: Upload logs
        ...
        with:
          ...
          path: .project-toolkit/logs/pr-maintenance*.log
          retention-days: 30
...
3. Review logs for errors (`.project-toolkit/logs/pr-maintenance.log`)
