...
**Session 106 Reference**: `.project-toolkit/sessions/2025-12-30-session-106-pr-593-ci-fix.md`
...
$investigationAllowlist = @(
    ...
    '^\.project-toolkit/memory/',             # Memory-graph updates (Amendment 2026-07-08, issue #831)
    '^\.project-toolkit/architecture/REVIEW-', # ADR-review REVIEW- artifacts only (Amendment 2026-07-08, issue #732)
    '^\.project-toolkit/critique/'            # ADR-review critique artifacts (Amendment 2026-07-08, issue #732)
)
```
...
|--------------|-----------|
| `.project-toolkit/planning/` | Implementation plans produce testable artifacts |
| `.project-toolkit/architecture/ADR-*` | ADRs are design decisions affecting code. Exception: `.project-toolkit/architecture/REVIEW-*` review artifacts are investigation-only (see Amendment 2026-07-08) |
| `.project-toolkit/qa/` | QA reports are validation artifacts |
...
|---------------|-----------|--------|
| `^\.project-toolkit/memory/` | Memory-graph updates are investigation artifacts, not implementation code. They record cross-session context and produce no testable behavior. | Issue #831 |
| `^\.project-toolkit/architecture/REVIEW-` | ADR-review REVIEW- artifacts are investigation outputs. This is narrowly scoped to the `REVIEW-` filename prefix; ADR-* design files remain Not Allowed. | Issue #732 |
| `^\.project-toolkit/critique/` | ADR-review and plan critique artifacts triggered false `E_INVESTIGATION_HAS_IMPL` failures. They are review outputs, not implementation code. | Issue #732 |

A redundant 9th pattern (`^\.project-toolkit/memory/episodes/`) was removed because
`^\.project-toolkit/memory/` already subsumes it. The Not Allowed table above is updated in
the same change: the `.project-toolkit/critique/` row is removed, and `.project-toolkit/architecture/`
...
3. Integration test: commit with staged `.project-toolkit/sessions/` only, verify PASS
...
|----------|-----------------|
| Investigation session with only `.project-toolkit/sessions/` staged | PASS |
| Investigation session with `.serena/memories/` + session log | PASS |
| Investigation session with `.project-toolkit/security/SA-*.md` staged | PASS |
| Investigation session with `.project-toolkit/planning/PRD.md` staged | FAIL (implementation artifact) |
| Investigation session with `.project-toolkit/critique/` staged | PASS (review artifact, see Amendment 2026-07-08) |
...
- `.project-toolkit/analysis/pre-commit-qa-investigation-sessions-gap.md` - Problem analysis
- `.project-toolkit/analysis/investigation-session-patterns-analyst-report.md` - Analyst findings
- `.project-toolkit/architecture/ASSESSMENT-session-qa-validation-options.md` - Architect assessment
- `.project-toolkit/security/SA-pre-commit-qa-skip-options.md` - Security assessment
- `.project-toolkit/critique/investigation-qa-exemption-proposal-critique.md` - Critic review
...
| Security | CONDITIONAL | Missing `.project-toolkit/security/` path |
...
|----------|--------|-----------|
| `.project-toolkit/critique/` in allowlist | EXCLUDE | 5/6 agents flagged as loophole; consequential artifacts |
| Missing `.project-toolkit/security/` path | ADD | Security assessments are investigation outputs |
| Missing `.project-toolkit/handoffs/` path | REJECT | Handoffs coordinate implementation, not investigation |
...
2. Removed `.project-toolkit/critique/` from allowlist (loophole)
3. Added `.project-toolkit/security/` to allowlist (investigation output)
