...
| **QA Validation** | `gh pr create` | `.project-toolkit/qa/` report exists | Retired 2026-07-19 (see Amendment) |
...
if ($RequiresSessionLog) {
    ...
    if (-not $SessionLog) {
        $Output = @{
            decision = "deny"
            reason = "SESSION PROTOCOL GATE: Create session log first.`nPath: .project-toolkit/sessions/YYYY-MM-DD-session-NN.json"
        }
...
**Reference**: [SKILL-CREATION-CRITERIA.md](../../.agents/governance/SKILL-CREATION-CRITERIA.md)
...
2. Define `.project-toolkit/qa/` report format requirement
...
2. **Evidence Requirements** (any satisfies):
   ...
   - Retrospective file in `.project-toolkit/retrospective/` for today
