...
ADRs in this repository record lifecycle state as prose inside the `## Status` section. The canonical template (`.project-toolkit/architecture/ADR-TEMPLATE.md`) offers `[Proposed | Accepted | Deprecated | Superseded by ADR-XXX]`, and `adr-best-practices.md` instructs authors to "Mark the old ADR as `Superseded by ADR-NNN`". This is the stock MADR convention, and for a human-read decision log it is fine.
...
- **When introduced**: Convention inherited from the project canonical template `.project-toolkit/architecture/ADR-TEMPLATE.md`; reinforced by `adr-generator/references/adr-best-practices.md`.
...
- Current state is one query, no body parsing. Using the repo's existing Python frontmatter dependency (`python-frontmatter`, per ADR-042 Python-first; `yq` is not in the toolchain):
  ...
  for p in pathlib.Path(".project-toolkit/architecture").glob("ADR-*.md"):
...
| Existing 72 ADRs under `.project-toolkit/architecture/` | Direct | Deferred backfill with a triage pass for conditional/inline status (ADR-072, ADR-055) | Medium |
