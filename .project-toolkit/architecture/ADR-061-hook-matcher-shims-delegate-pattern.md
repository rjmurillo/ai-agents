...
This ADR is preserved (not deleted) as institutional knowledge. The debate log at `.project-toolkit/critique/ADR-061-debate-log.md` records the full positions. The follow-up issue (#2112) tracks when the structural refactor should be revisited: when multi-matcher hook count exceeds 8, OR when the alternative B CI drift gate fires three or more times in a quarter (signal that the procedural fix is failing).
...
|-----------|----------------|-----------------|------|
| `.project-toolkit/specs/requirements/REQ-003-multi-tool-artifact-build.md` step 5 | Direct | Replace inline-body language with delegate-shim language; preserve sentinel and crash-policy clauses verbatim | Low |
