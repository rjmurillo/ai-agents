...
Claude Code provides a stable plugin marketplace system (researched in session 1180, documented in `.project-toolkit/analysis/claude-code-plugin-marketplaces.md`) that supports:
...
|--------|----------|---------------------|
| `core-agents` | 26 agent definitions, 18 shared templates, build/generation system, governance templates | Low coupling. 14 of 18 templates contain output path directives (e.g., `Save to: .project-toolkit/analysis/...`) that require parameterization before extraction. Path parameterization is Phase 1 scope. |
...
Plugins are installed to a cache directory, not the project root. Framework code that references consumer paths (e.g., `.project-toolkit/sessions/`) must use environment variables with sensible defaults:
...
See `.project-toolkit/projects/v0.4.0/PLAN.md` for the detailed phased implementation plan and session estimates.
