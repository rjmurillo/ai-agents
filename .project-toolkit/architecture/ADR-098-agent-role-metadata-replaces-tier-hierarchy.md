...
`.project-toolkit/critique/`, which now exists, so the flip is mechanically available and
...
`.project-toolkit/retrospective/2026-08-17-governance-bureaucracy-critical-review.md`
...
- Four frozen issue #1738 measurement artifacts under `.project-toolkit/prototypes/` and
  `.project-toolkit/analysis/` keep `metadata.tier` deliberately, exempted by explicit
...
| `.project-toolkit/architecture/ADR-078` | Indirect | Seven phrases moved off the retired vocabulary; the option-C rejection clause re-grounded on the skill/agent boundary. Decision untouched | Low |
| `.project-toolkit/prototypes/agents/README.md` | Indirect | Frontmatter-parity instruction named `metadata.tier`; updated to `metadata.role` | Low |
...
rounds are in `.project-toolkit/critique/5130-tier-hierarchy-removal-debate-log.md`.
...
- `.project-toolkit/architecture/ADR-009-parallel-safe-multi-agent-design.md`. The
  canonical aggregation and escalation source. Unchanged by this decision and
  now quoted verbatim rather than paraphrased.
- `.project-toolkit/architecture/ADR-078-autoplan-orchestrator-router-boundary.md`.
...
- Issue #1769. `.project-toolkit/analysis/1769-monolith-section-classification.md`, the
