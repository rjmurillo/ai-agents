...
Router Architecture). Debate evidence: `.project-toolkit/critique/ADR-106-debate-log.md`,
...
`.project-toolkit/memory/episodes`. Episodes are an append-only session record governed
...
ADR-007:202-206 named three mechanisms that verified the memory-first rule. Two
are gone; the third is live under a different filename. "SESSION-PROTOCOL Phase
2" no longer exists as a blocking gate, and session-log creation is discontinued
per the session-logs rule. The pre-commit hook, however, still runs on every
commit: `lefthook.yml:101-103` defines the `session-policy` job as
`uv run --frozen python scripts/validation/git_hook_policy.py session
{staged_files}`, and that subcommand invokes
`scripts/validate_session_json.py`, which exists. Only the PowerShell name
ADR-007 cited is absent, because ADR-042 ported it; `git ls-files` matches no
`Validate-SessionJson`.

An earlier draft of this record, and of its debate log, said "none is live". That
was inferred from the missing `.ps1` filename instead of from the call path, and
it is the same failure this change's own retrospective is about. It was caught in
pre-merge audit; Round 3 of the debate log records it.
...
sensitive user data, or security vulnerabilities (use `.project-toolkit/security/`).
...
|---|---|---|---|
| Agent prompt trees (six of them; `.claude/agents` and `.github/agents` are hand-maintained, not generated, per `build/scripts/detect_agent_drift.py:33-34`) | Direct. 110 lines name the withdrawn Memory Router: 38 in `src/claude`, 35 in `.claude/agents`, 10 in `.github/agents`, 9 each in `templates/agents`, `src/copilot-cli/agents` and `src/vs-code-agents` | Repoint or remove; not done here | Medium |
...
| `.serena/memories/adr/adr-037-accepted.md` | Direct. Its `**Status**: Accepted` line and its "Forgetful augmentation" decision would become false on merge, and memory is re-injected ahead of reasoning | Marked superseded here, with current truth stated inline and the episode tier carved out | Low |
| `.project-toolkit/architecture/README.md` | Indirect. Index moves both records to Retired | Regenerated by `build_all.py` | Low |

The unrepaired rows above are prose and frontmatter edits outside the ADR bodies
#5574 protects. They do not belong to Stage 2: that stage merged as #5578 on
2026-09-05, and its scope was the `forgetful` token, which 7 of the 8 SKILL.md
files never contained, so no Stage 2 grep could have reached them. They need
their own follow-up under #5574, which remains open. The missing guard, that no
validator resolves a declared `metadata.adr` against that ADR's status, is filed
separately as #5665.
...
- ADR-063 cites ADR-037 six times, at :63, :104, :117, :122, :145 and :284, and
  every one is about the router pattern, which is a skill-surface router that is
  alive and untouched. Its store-topology clause is a separate sentence that
  cites ADR-007, not ADR-037: "Serena remains the / canonical store and
  Forgetful the supplementary store (ADR-007)" at :118-119. That clause is
  withdrawn by Decision item 2; the six ADR-037 citations are inherited intact.
  What ADR-063 actually decided survives whole, by its own words at :123-124:
...
- ADR-038's four-tier schema is untouched. Its Tier 2 episode store holds 750
  episode files under `.project-toolkit/memory/episodes/`, and its Tier 3 was already
  removed by ADR-089.
