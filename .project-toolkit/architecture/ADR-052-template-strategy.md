...
A 2025-12-15 analysis (`.project-toolkit/analysis/drift-analysis-claude-vs-templates.md`) measured similarity between Claude agents and shared templates:
...
- Pro: Eliminates the template layer. Targets reducing maintenance surfaces from 3 to 2 (Claude source + generation script), unmeasured (see Migration Plan Phase 2 step 8). Claude is already the most-used platform and receives the most iteration.
...
- Targets reducing maintenance surfaces from 3 (templates, Claude, derived) to 2 (Claude, derived); unmeasured, not a settled reduction, since `src/claude/` (~33 files) is not a clean superset of `templates/agents/` (31 files) (see Migration Plan Phase 2 step 8)
...
3. **Migrate `build/generate_agent_catalog.py` off `templates/agents/*.shared.md`.** As of 2026-08-25 that generator treats the template tree as `docs/agent-catalog.md`'s system of record and is wired into `build/scripts/build_all.py` as its own `agent-catalog` step, independent of the two agent-output generators (Copilot, PR #5286). **Correction (2026-08-25, Round 3 adr-review): the target this step originally prescribed, "point it at `src/claude/*.md` and verify `docs/agent-catalog.md` regenerates unchanged," is not reachable as written** (independent-thinker and critic, Round 3): `src/claude/*.md` carries `role:` nested under a `metadata:` key rather than at top level for most files (only 6 of 33 have a top-level `role:`; verified via `grep -l "^role:" src/claude/*.md`); `generate_agent_catalog.py:159-172` raises `CatalogError` on a missing top-level `role`. **Correction (2026-08-25, Copilot review): `name:` is not missing.** 32 of 33 files carry a top-level `name:` field (verified via `grep -L "^name:" src/claude/*.md`; the one exception, `claude-instructions.template.md`, is a template file, not an agent definition). The source-shape mismatch is `role:`'s nesting alone, not a missing `name:`. The catalog renderer also hardcodes `../templates/agents/{name}.shared.md` links that would point at the deleted tree. Whoever executes this phase must resolve the source-shape mismatch first (either add a compatible frontmatter shim or adjust the generator to read `src/claude/`'s actual schema) and expect a reviewed diff, not byte-for-byte parity.
...
- `.project-toolkit/critique/ADR-052-debate-log.md`: 6-agent adr-review debate recording this acceptance
- `.project-toolkit/analysis/drift-analysis-claude-vs-templates.md`
- `.project-toolkit/analysis/template-strategy-debate.md` (architect review findings)
- `.project-toolkit/retrospective/2025-12-15-accountability-analysis.md`
