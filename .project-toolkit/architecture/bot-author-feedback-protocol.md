...
**Verification**: Check for session log at `.project-toolkit/sessions/YYYY-MM-DD-session-NN.json` (optional; see `.claude/rules/session-logs.md`)
...
- **Local runs**: `.project-toolkit/logs/pr-maintenance.log`

### Reading Logs

```bash
# Tail latest local run
tail -f .project-toolkit/logs/pr-maintenance.log

# Filter errors only
grep '\[ERROR\]' .project-toolkit/logs/pr-maintenance.log

# Search for specific PR
grep 'PR #123' .project-toolkit/logs/pr-maintenance.log
```
...
| 2 | Fatal error | Fail | Review logs at `.project-toolkit/logs/pr-maintenance.log` |
