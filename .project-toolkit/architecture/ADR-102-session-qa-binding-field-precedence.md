...
The equality raise ties `endingCommit` and `comparison.head` together, and the tie is visible in history, at a scale smaller than it first looks. **Correction**: an earlier version of this measurement walked `git log --all -- .project-toolkit/sessions/`, which reads every ref this clone happens to hold rather than the branch under review, a violation of `.claude/rules/ci-scripts.md` MUST 9 ("a claim about what the repository contains MUST come from a named ref"); that version reported 50 edits across 38 commits. Re-measured from `HEAD`: across the 1026 single-parent commits touching `.project-toolkit/sessions/` reachable from `HEAD`, `comparison.head` is edited 44 times (a session log can be edited more than once), across 32 distinct commits. Of those 44 edits, 38 land in a commit where `endingCommit` is also present; 36 of those 38 agree (SHA-prefix-normalized, since an abbreviated `endingCommit` can name the same commit as a full `comparison.head`). Reproduce by walking `git log HEAD -- .project-toolkit/sessions/` and diffing `episodeMetrics.comparison.head` against `endingCommit` per file, per single-parent commit.

Splitting the 36 by what kind of edit produced them matters, and an earlier draft of this section did not split them. **33 of the 36 are the session log's first commit**: both fields are written once, together, because they both describe the same commit at creation time. There is no prior value for either field to move away from; agreement here is the ordinary case of two fields that both name the session's own last commit, which is exactly the belief the original equality raise encoded. The remaining **3 are edits to an already-committed log**, and none of them is the pattern this section originally described from the `git log --all` sample (`endingCommit` moved to follow a `comparison.head` that had already advanced to a QA rebind target). All three instead backfill a previously-absent `comparison.head` field during a later documentation-correction commit unrelated to QA rebinding, for example `a98d42a636` ("docs(session): correct PR 4944 closure evidence"): two set the new field to a fresh value alongside a changed `endingCommit`, one sets it equal to the `endingCommit` already there.

The hand-sync pattern this ADR is about is real, but it is not measured in this corrected sample. A PR #4954 reviewer independently raised exactly this pattern against two other logs: `.project-toolkit/sessions/handoffs/2026-08-15-2840-handoff.md:207` records two findings against session-14695's and session-15001's `endingCommit` fields as "false historical session provenance," and records that the fix "requires relaxing `session_qa_binding()`'s enforced `endingCommit`/`comparison.head` equality." That session could not make the change: it was at commit 40 of a 40-commit ceiling. The findings were closed with a reply, not a fix, which is exactly why they do not show up as a "modification" in the `git log HEAD` walk above: no commit ever performed the hand-sync. The pattern is documented and real; it is corroborated qualitatively by an independent reviewer, not quantitatively by this corpus.

So the corrected corpus divides two ways: 33 creations where agreement is unremarkable, and 3 field-backfill modifications unrelated to QA rebinding. Neither category is the hand-sync pattern this ADR is about. Citing "36 of 38 agree" as evidence the check rarely engages is accurate; citing either the corrected or the original number as evidence of how often the hand-sync churn happens is not, since the churn's only committed evidence is the independently-documented, never-fixed PR #4954 finding above, not a count from this corpus. The check's design flaw (it forces a choice between two contracts that should not be coupled) is the real, narrow finding, and the PR #4954 citation is enough to establish it is a documented, reviewer-identified defect this ADR fixes, even though the corrected corpus measures its incidence at zero committed repairs rather than seven.
...
Measured against `git ls-tree -r HEAD .project-toolkit/sessions/`:
...
- `.project-toolkit/sessions/2026-08-05-session-2.json`: `afae287fa...` against `f1a805fc5...`
- `.project-toolkit/sessions/2026-08-06-session-10003-review-pending-checkout-changes-update.json`: `609b314e5a6ed63332fa193f4644683067740590` against `bb30860ac61d4b62654653b0f6b67658d9594653`
...
Do not read the low disagreement count as evidence the check is harmless. The corrected corpus does not show a repaired-before-commit pattern directly (see Measured Incidence above): the low count instead reflects that most agreeing edits (33 of 36) are first-commit creations that were never disagreements to repair. The hand-sync repair cost is real and independently documented (the PR #4954 finding above), just not visible in this corpus because it was never fixed by a commit. The issue's cited interactive cost is real but is not this check's: `.project-toolkit/sessions/handoffs/2026-08-15-2840-handoff.md` carries 23 occurrences of "rebind" across 15 review rounds on PR #4954 (reproduce with `grep -ci rebind` against the tracked file), and `handoff.md:227` attributes those rounds to a different mechanism, `QA_EVIDENCE_PREFIXES` boundary churn: "the analysis file alone has now triggered rebinds six separate times." That figure measures a check ADR-096 already relaxed at the other call site, not this one; citing it here overstated this raise's cost by roughly an order of magnitude, and it is corrected out of the Alternatives table below.
...
The exposure is bounded by how the fallback is reached. It requires live-`HEAD` resolution to fail, on the fresh-validation path, on a log whose two fields disagree with `comparison.head` older. `tests/test_validate_session_json.py::TestValidateQaReportEvidence::test_fallback_head_masks_a_real_change_between_the_two_fields` pins it: `validation_head=None`, `comparison.head` older than `endingCommit`, `report.commit` a genuine ancestor of `comparison.head`, and it asserts both that the report still passes and that the git ancestry query issued never names `endingCommit` at all, so the gap is demonstrated rather than assumed.
...
|-------------|------|------|----------------|
| Status quo (keep the raise) | Zero change; the check reads like a control | Rejects the state described under "`endingCommit` is carrying two contracts" above as normal; blocks the whole QA-evidence check to protect a value normally never read; forces the hand-sync pattern documented at PR #4954 (never fixed by a commit, so absent from the HEAD-scoped corpus) that pulls `endingCommit` off its own contract to satisfy an equality it should not have to | The check contradicts the invariant it validates, and the issue was filed to remove it |
...
The design trades a hard verdict for an accurate one. The raise asserted that a disagreement means the log is wrong; for the documented case (PR #4954) where an operator would have hand-synced `endingCommit` to a rebind target, it meant a rebind happened. Downgrading to a warning gives up the ability to block on that state, which was never a state worth blocking on, and removes the manual-sync cost on the narrow population where the check actually engages (see Measured Incidence). The one real cost is that a log corrupted by a hand-edit into disagreement now proceeds with a warning. That cost is bounded by `endingCommit` keeping its own reachability check and by `post_qa_code_changes()` failing closed on ancestry when the selected commit is used as a head.
...
- The residuals ADR-096 recorded and left open are untouched: `post_qa_code_changes()` still reports every path a catch-up merge picked up, and the `QA_EVIDENCE_PREFIXES` boundary gaps (instruction-prose coverage, the self-referential `.project-toolkit/qa/` entry, `AI_AGENTS_ARTIFACT_ROOT` desync) remain a separate follow-up.
...
4. `.project-toolkit/sessions/2026-08-06-session-10003-review-pending-checkout-changes-update.json` has carried two unresolvable, disagreeing commit fields since 2026-08-06 and is this ADR's central exhibit. Nobody owns cleaning it up.
...
- `.project-toolkit/critique/ADR-096-debate-log.md`. The review pattern this ADR follows.
- `.project-toolkit/critique/ADR-102-debate-log.md`. Round 3: the six-independent-agent re-review, the overruled Block, and the corpus and citation corrections this ADR incorporates.
- `.project-toolkit/sessions/handoffs/2026-08-15-2840-handoff.md`. The 23-mention, 15-round rebind evidence, attributed at `:227` to `QA_EVIDENCE_PREFIXES` churn rather than to this check; the round-15 findings against `endingCommit`'s "false historical session provenance" at `:207`; the round-15 recommended remedy at `:219`; and the rebind-preserves-`commitHead` observation at `:80`.
