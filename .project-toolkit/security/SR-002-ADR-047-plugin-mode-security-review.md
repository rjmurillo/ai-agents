...
**Description**: The ADR states "When a required directory (e.g., `.agents/`, `.project-toolkit/sessions/`) does not exist, create it with `os.makedirs(path, exist_ok=True)`." If `path` is derived from unvalidated `CLAUDE_PROJECT_DIR`, this could create directories outside the project scope.
