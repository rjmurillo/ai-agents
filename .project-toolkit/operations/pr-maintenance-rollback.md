...
**Location**: `.project-toolkit/logs/pr-maintenance.lock`

**Steps**:

```bash
# Check lock status
ls -la .project-toolkit/logs/pr-maintenance.lock

# Remove if stale (>15 min old)
rm .project-toolkit/logs/pr-maintenance.lock
```
...
2. Identify affected comments via log file:
   ```bash
   cat .project-toolkit/logs/pr-maintenance.log | grep "Acknowledging comment"
   ```
...
2. Identify affected PRs from log:
   ```bash
   cat .project-toolkit/logs/pr-maintenance.log | grep "Successfully resolved"
   ```
...
Primary log location: `.project-toolkit/logs/pr-maintenance.log`
...
2. Include:
   ...
   - Log files (`.project-toolkit/logs/pr-maintenance.log`)
