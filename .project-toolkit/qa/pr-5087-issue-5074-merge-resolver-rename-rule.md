---
qaVerdict: PASS
qaSessionLog: .project-toolkit/sessions/2026-08-15-session-14712-issue-5074-merge-resolver-rename-rule.json
...
- [x] merge-resolver guidance names the artifact classes and the rename
  ...
  content-merge" class listing session logs (`.project-toolkit/sessions/*`), QA reports
  (`.project-toolkit/qa/*`), and retrospectives (`.project-toolkit/retrospective/*`), with the
...
- [x] The anti-pattern (content-merging two session logs) is stated with the
  ...
  `.project-toolkit/retrospective/2026-08-10-pr-4856-session-log-collision.md` and
...
| Session log validates | `uv run python scripts/validate_session_json.py .project-toolkit/sessions/2026-08-15-session-14712-issue-5074-merge-resolver-rename-rule.json` | PASS after QA report landed |
...
- `resolve_pr_conflicts.py` `AUTO_RESOLVABLE_PATTERNS` still matches
  `.project-toolkit/sessions/*` and resolves by accept-theirs, which performs only half
