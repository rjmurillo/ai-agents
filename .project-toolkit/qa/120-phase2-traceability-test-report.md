...
**Data Strategy**: Production specs in `.project-toolkit/specs/`
...
**Verdict**: [PASS] - Script correctly scans `.project-toolkit/specs/` directory structure and finds all existing specs. The stats match expected counts exactly.
...
STAGED_SPEC_FILES=$(echo "$STAGED_FILES" | grep -E '^\.project-toolkit/specs/(requirements|design|tasks)/.*\.md$' || true)
...
if [ -n "$STAGED_SPEC_FILES" ]; then
    ...
    if [ -L "$TRACEABILITY_VALIDATE_SCRIPT" ]; then
        ...
        if command -v pwsh &> /dev/null; then
            ...
            if ! pwsh -NoProfile -ExecutionPolicy Bypass -File "$TRACEABILITY_VALIDATE_SCRIPT" -SpecsPath "$REPO_ROOT/.project-toolkit/specs" 2>&1; then
...
- Triggers on `.project-toolkit/specs/(requirements|design|tasks)/*.md` files
...
**Recommendation**: Create test fixtures with known violations to validate error reporting paths. Add to `.project-toolkit/specs/test-fixtures/` directory with broken refs, untraced tasks, and orphans.

## Recommendations

1. **Add test fixtures for error cases**: Create `.project-toolkit/specs/test-fixtures/` with intentionally broken specs to validate error reporting. This ensures error messages are clear and actionable when users hit actual violations.
...
- Trigger pattern: `^\.project-toolkit/specs/(requirements|design|tasks)/.*\.md$`
