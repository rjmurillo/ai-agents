---
qaVerdict: PASS
qaSessionLog: .project-toolkit/sessions/2026-08-21-session-99927-ruff-linting-cleanup.json
...
`per-file-ignores` entry for `.project-toolkit/security/benchmarks/**/*.py`, which
already waived `RUF100` with an answer-key rationale, was widened to `I001` and
`E501`. A later commit narrowed that glob to
`.project-toolkit/security/benchmarks/vulnerable_samples/**/*.py`, the only subtree
