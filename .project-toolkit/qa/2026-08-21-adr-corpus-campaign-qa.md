---
qaVerdict: PASS
qaSessionLog: .project-toolkit/sessions/2026-08-21-session-5189-54e494d-adr-corpus-evaluation-and-tooling.json
...
**Session log**: `.project-toolkit/sessions/2026-08-21-session-5189-54e494d-adr-corpus-evaluation-and-tooling.json`
...
check requires a path to a report under `.project-toolkit/qa/`. This file is that report.
...
- `.project-toolkit/critique/ADR-corpus-repair-5189-5201-debate-log.md`
- `.project-toolkit/critique/ADR-079-091-092-chain-debate-log.md`
- `.project-toolkit/critique/ADR-005-042-scripting-language-debate-log.md`
- `.project-toolkit/critique/ADR-023-032-033-link-repair-debate-log.md`
...
| ADR-042 carried the corpus's most load-bearing enum without citing its evidence. | `## Status` now names `.project-toolkit/critique/ADR-042-debate-log.md` and four supporting artifacts. |
...
`.project-toolkit/critique/ADR-005-status-duplication-debate-log.md`.
...
The colocated test added above spelled `.project-toolkit/architecture` literally. Those
...
`.project-toolkit/critique/ADR-063-frontmatter-transcription-debate-log.md`.
...
addendum 7 of `.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`. Not
...
of `.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`: a query recipe that
...
corpus measurements showing both fixes latent. `.project-toolkit/architecture/README.md`
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`. The headline: the
...
in Addendum 11 of `.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`:
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`. Summary: the only real
...
`.project-toolkit/architecture/README.md` needed regenerating after the Addendum 11
merge; see Addendum 13 of `.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`.
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`.
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`: committed the
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`. Summary: `AGENTS.md`
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`: this branch and PR
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`: a hidden HTML
...
`.project-toolkit/qa/session-5209-adr-review-fixes-stacked.md`: fixed by
...
- **`generate_adr_index.py`'s `_INTRO` carried two inaccurate claims about
  ...
  ValueError`). `.project-toolkit/architecture/README.md` regenerated;
...
directly: fixtures that create `.project-toolkit/architecture` empty to test
...
`build/scripts/build_all.py --check` flagged `.project-toolkit/architecture/README.md`
...
`.project-toolkit/retrospective/2026-08-25-pr5209-push-notification-false-completion.md`
...
`build/scripts/build_all.py --check` (run as part of `pre-pr-validation` on the push this addendum responds to) correctly flagged `.project-toolkit/architecture/README.md` as stale: the origin/main merge brought in PR #5291's ADR-073 frontmatter across 67 records, changing dates, statuses, and decision-makers the generated index renders. Regenerated via `scripts/sync_plugin_lib.py` then `build/scripts/build_all.py`; `--check` now reports zero staleness. No ADR-tooling script changed; `check_adr_lifecycle.py` and `check_adr_links.py` re-run unchanged from Addendum 37.
...
Copilot review comment on `.project-toolkit/critique/ADR-073-phase2-backfill-debate-log.md:545`,
...
means: `.project-toolkit/architecture/ADR-073-adr-lifecycle-frontmatter.md:49` reads
...
regenerated `.project-toolkit/architecture/README.md`. Commit `d331cba4f`.
...
`.project-toolkit/critique/ADR-005-status-duplication-debate-log.md` per ADR-073's
...
re-measures it), regenerated `.project-toolkit/architecture/README.md`
...
2. `dc22389f9`: ADR-055/063 frontmatter `date`, same class. Regenerated
   `.project-toolkit/architecture/README.md` for all five date changes.
...
3. `e80a79e06`: `check_adr_links.py`&#39;s empty-corpus guard (round 9) only
   ...
   anchored the check to `.project-toolkit/architecture` (mirroring
...
Converted to UTC: `cdf688a9f` landed at `2026-08-25T15:35:32Z`, roughly 8h40m *after* `d331cba4f` (`2026-08-25T06:55:31Z`, the commit Addendum 41/53 cited as authoritative). `cdf688a9f` is also already an ancestor of this branch's `HEAD`. Its diff on ADR-042 (`git show cdf688a9f -- .project-toolkit/architecture/ADR-042-python-migration-strategy.md`) is two hunks: the frontmatter `date: 2026-04-13` to `2026-08-25`, and a new `**Downstream**:` bullet under `## Related Decisions` stating ADR-031 was rejected and ADR-028 was superseded, both "2026-08-25". The bullet is still present in the current file (`grep -n Downstream .project-toolkit/architecture/ADR-042-python-migration-strategy.md` returns line 154).

**Why Addendum 53 got this wrong.** It reasoned that `d331cba4f` (mine, this session, earlier that same day) had "already settled the correct value" and that Addenda 46/51's `2026-08-25` was a repeated mistake. But `d331cba4f` came *first*; `cdf688a9f` landed nearly 9 hours later, already on `main`, already an ancestor, adding a real content change the debate log's own "content, not touch" rule (`.project-toolkit/critique/ADR-073-phase2-backfill-debate-log.md:101-125`) explicitly covers: a factual claim about another record's status, added to this record's own body. Addendum 53 verified only that `2026-04-13` matched Amendment 1 (true, as of `d331cba4f`) and never checked `git log` for anything landing after it. Addenda 46 and 51 had the reasoning right the first time.

**Fixed.** `.project-toolkit/architecture/ADR-042-python-migration-strategy.md` frontmatter restored to `date: 2026-08-25`; `.project-toolkit/architecture/README.md` regenerated (`uv run python build/scripts/build_all.py`) to match; `.project-toolkit/critique/ADR-073-phase2-backfill-debate-log.md` Batch 30 added, correcting Batch 29 with this same evidence. Not silently overwriting Batch 29 or Addendum 53's prose, for the same reason Addendum 53 gave for not overwriting Addenda 46/51: a report that erases its own repeated wrong call teaches the next reader the mistake never happened, when it happened three times now.
...
Neither round got its own addendum when it landed; this one covers both, since the PASS evidence above still named `_mask_inline_html_comments`, an implementation removed two rounds ago. A round-17 Copilot review caught the staleness directly (`.project-toolkit/qa/2026-08-21-adr-corpus-campaign-qa.md:2565` at the time): "No later addendum validates or mutation-tests the implementation being merged". Real, and now fixed by this addendum.
