---
qaVerdict: PASS
qaSessionLog: .project-toolkit/sessions/2026-08-06-session-10003-record-strict-policy-intentional-merge.json
qaCommit: 8d3af6409219f5b204b24e926726b71709dcef4e
---
...
- `uv run --frozen python scripts/validation/instruction_budget.py --ci --format table` reported PASS for `.py` at 98,982 bytes against the 99,000 byte ceiling.
- `uv run python scripts/ci/ruff_count_ratchet.py --base-ref origin/main` reported the measured count equals baseline 27.
- `uv run pytest -q tests/validation/test_instruction_budget.py tests/validation/test_instruction_ceiling_ratchet.py tests/validation/test_always_on_corpus_claims.py tests/validation/test_audit_procedure_claims.py` reported 160 passed.
