...
**Subject**: `.project-toolkit/architecture/ADR-096-relax-qa-evidence-commit-equality.md`, addressing issue #5164.
...
- **`QA_EVIDENCE_PREFIXES` has boundary problems the ADR does not examine**: it covers agent-instruction prose, not just data (security P1-1, memory-poisoning-adjacent risk on `.project-toolkit/sessions/handoffs/*.md` and the 2180-file episode store); `.project-toolkit/qa/` is inside its own allowlist so a report can rewrite its own verdict (security P1-2); `AI_AGENTS_ARTIFACT_ROOT` can desync the containment check from the hardcoded prefixes (security P1-3); a second, disagreeing definition of "bookkeeping path" already exists in `git_hook_policy.py`'s `GENERATED_PATHS`/`GENERATED_GLOBS` (independent-thinker P1-2). None of these are reachable in the measured history (independent-thinker: 0/50 commits), so they are named as an explicit, separate residual and follow-up rather than folded into this already-large change.
