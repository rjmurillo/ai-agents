...
**Critic**: Critic Agent
**Reviewed Documents**:
- `.project-toolkit/analysis/investigation-session-patterns-analyst-report.md`
- `.project-toolkit/architecture/ASSESSMENT-session-qa-validation-options.md`
- `.project-toolkit/security/SA-pre-commit-qa-skip-options.md`
...
- [ ] **Undefined Guardrail Logic for `.agents/` Files** (Location: All reports, Line: Security 209-214, Architect 96-104)
  ...
  **Issue**: Security agent's Guardrail 1 excludes files matching `'^\.agents/'` from code file detection. This means sessions that ONLY update `.project-toolkit/sessions/` and `.serena/memories/` would pass the staged-file check and be allowed to use investigation-only skip. However, the logic is inconsistent:

  - Security report (Line 210): `$codeFiles = $stagedFiles | Where-Object { $_ -notmatch '^\.agents/' }`
  - Implication: ANY file under `.agents/` is exempt, including `.project-toolkit/planning/`, `.project-toolkit/architecture/ADR-*.md`, `.project-toolkit/qa/`, etc.

  **Problem**: This is too permissive. A session that creates a new ADR (`.project-toolkit/architecture/ADR-008.md`) or updates a planning document (`.project-toolkit/planning/PRD-feature.md`) could claim investigation-only skip even though it produced implementation artifacts.
  ...
  **NOT exempt**:
  ...
  - `.project-toolkit/architecture/` (ADRs are design decisions)
  - `.project-toolkit/qa/` (QA reports)
  - `.project-toolkit/critique/` (plan reviews)
...
- [ ] **Missing Edge Case: Session Log + Code Change Scenario** (Location: All reports, implicit)
  ...
  **Issue**: All three reports assume investigation sessions will ONLY have `.project-toolkit/sessions/*.md` and `.serena/memories/*.md` staged. None address the scenario where an agent:
...
$investigationPaths = @(
    ...
    '^\.serena/memories/',      # Cross-session context
    '^\.project-toolkit/critique/'       # Plan reviews (critic sessions are investigation)
)
...
**Scenario**: Agent attempts investigation-only skip but has `.project-toolkit/planning/PRD-feature.md` staged.

**Current Proposed Error** (Security Line 212): "Investigation skip claimed but staged files contain code: .project-toolkit/planning/PRD-feature.md"
...
```text
Investigation skip claimed but staged files contain implementation artifacts (not investigation outputs): .project-toolkit/planning/PRD-feature.md
...
| Stage `.project-toolkit/planning/PRD.md`, claim investigation | Current logic: PASS (`.agents/` exempt) | [FAIL] Too permissive |
