...
1. `.project-toolkit/critique/ADR-042-debate-log.md` records a real six-role debate dated 2026-01-17, `**Final Verdict**: ACCEPT`, 5 Concerns plus 1 Accept resolved Disagree-and-Commit, all P0 findings marked RESOLVED. Four supporting artifacts sit beside it: `ADR-042-independent-review.md`, `ADR-042-security-review.md`, `ADR-042-strategic-advisory.md`, `ADR-042-python-migration-critique.md`.
...
- **The debate-log gate is forgeable.** `check_adr_review_policy` tests only that a staged file sits in `.project-toolkit/critique/`, ends in `.md`, has "debate" in its name, and matches `ADR-\d+` anywhere in its bytes. A 7-byte file cleared it. There is no verdict field, no roster, no round count, no tie to the record's content.
