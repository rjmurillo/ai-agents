...
Record under review: `.project-toolkit/architecture/ADR-109-template-first-plugin-distribution.md`
...
- 2026-09-11, independent review, finding 1 (blocker, B1 loses Claude-only agent content). Measured across the 31 `templates/agents/*.shared.md` / `src/claude/*.md` pairs: 1,414 Claude-only body lines and 622 template-only body lines, 18 of 31 pairs with Claude-only content, worst `pr-comment-responder` (303), `devops` (253), `merge-resolver` (209), `retrospective` (175), `architect` (83, sections `## Claude Code Tools`, `## Reversibility Assessment`, `## Vendor Lock-in Assessment`, `## Degraded Mode Protocol`); 4 `src/claude/*.md` carry `isolation_required: true` against 2 templates. Section 1's agent row and prose now specify per-stem `<stem>.claude.md.tmpl` and `<stem>.copilot.md.tmpl` composed from `templates/agents/partials/` under ADR-108's grammar, replacing ADR-052's `platform-overrides/` idea rather than dropping it without a replacement; B1's exit criterion is now byte-identical first render, pinned by a fixture test, with content reconciliation deferred to per-agent PRs; ADR-036 §Intentional Divergence is named as retired as a reading under Related Decisions. Evidence: for each pair, `comm -13`/`comm -23` on sorted, frontmatter-stripped body lines; `grep -l 'isolation_required: true' templates/agents/*.shared.md src/claude/*.md`; `grep -n 'convert_frontmatter_for_platform\|MEMORY_PREFIX' build/generate_agents_common.py`.
- 2026-09-11, independent review, finding 2 (should-fix, hooks count). Context's `.claude/hooks/` clause and the B4 row now cite `git ls-files .claude/hooks | wc -l` (23) instead of `find -type f` (32, includes `__pycache__`), and name the hooks class as 16 files (13 executable scripts, `hooks.json`, `dispatch_groups.json`, `PreToolUse/markdownlint-safe-config.yaml`) against 7 docs files B4 does not template. Evidence: `find .claude/hooks -type f | wc -l`; `git ls-files .claude/hooks | wc -l`; `git ls-files .claude/hooks` classified by extension and directory role.
- 2026-09-11, independent review, finding 3 (should-fix, generator-order rule timing). The Impact row for `.claude/rules/generated-artifacts.md` and the B1 row now say the section's REQ-003-010 premise goes stale at B1, the first binplace write to `.claude/`, not at B5; B5's row keeps only the lib-specific sync-ordering hazard rewrite. Evidence: `sed -n '159,197p' .claude/rules/generated-artifacts.md` read against Decision section 3's binplace description.
- 2026-09-11, independent review, finding 4 (should-fix, plugin-self-containment.md impact widened). The Impact row now cites the rule's frontmatter `paths:` (`src/claude/**`, `src/copilot-cli/**`), its statement that `templates/agents/**` is canonical source, and its upstream-only list naming `templates/agents/`, `templates/platforms/`, and says B2 and B4 add `templates/rules/` and `templates/hooks/` with the same property. Evidence: `sed -n '1,10p;17,30p;94,105p' .claude/rules/plugin-self-containment.md`.
- 2026-09-11, independent review, finding 5 (should-fix, binplace manifest is a new file). Section 3's binplace-manifest paragraph now states the manifest is a new file with its own schema, not a repurposing of the three existing `templates/platforms/*.yaml` files, and that only `copilot-cli.yaml` currently declares `sourceDir` values (`.claude/agents`, `.claude/skills`, `.claude/rules`, `.claude/lib`) that B1 rewrites; `visual-studio.yaml` and `vscode.yaml` carry no `sourceDir` yet. Evidence: `cat templates/platforms/copilot-cli.yaml templates/platforms/visual-studio.yaml templates/platforms/vscode.yaml`.
...
- 2026-09-15: B3 follow-up (found by the B6 install-verification e2e, `RUN_CLI_E2E=1 uv run pytest tests/e2e/test_plugin_load_smoke.py -k claude -q`). B3's render-target move (Implementation Notes B3 row) only ever wrote `SKILL.md` into `src/claude/skills/<name>/`; 84 skills' `scripts/`, `references/`, `tests/`, and other support files under `.claude/skills/<name>/` were never mirrored there, so an installed Claude plugin's `SKILL.md` could route to a script that never shipped. Decision section 6's Amendments gained a dated paragraph: these files stay hand-maintained at `.claude/skills/<name>/` (the lib exception's shape, section 1 Rationale/Trade-offs) and `generate_skills.sync_claude_plugin_skill_support` (`build/scripts/generate_skills.py:281`) mirrors them, unlike the Copilot mirror never excluding merge-resolver. No decision text changed; no `OWNED_PREFIXES` widening was needed (the tuple's blanket `src/` entry already covers these files). Self-review: PR opened with unit, integration, and always-on e2e regression coverage; `build_all.py --check` verified exit 0 clean and exit 2 with a full restore after a hand-edited, committed support file under `src/claude/skills/`.
  Re-anchoring note: mirroring 525 files in one commit crossed `generate_skills.py`'s taste-lint 500-line ceiling; `_copy_skill_tree` and `sync_claude_plugin_skill_support` split into `build/scripts/skill_support_mirror.py` (same shape as `skill_templates.py`/`skill_template_grammar.py`). The four citations above re-anchored from `generate_skills.py:281/122/200/241` to `skill_support_mirror.py:246/87/165/206`; no decision text changed.
  Re-anchoring note: the citation-freshness gate requires an anchor (a backtick span, here the function name) to sit on the cited line itself; the :87/:165/:206 sentence reworded so each function name follows its own citation instead of sharing one anchor list before all three. No decision text changed.
  Escape-hatch note: the reworded sentence still did not satisfy the gate's anchor heuristic for a run-on sentence citing three functions in sequence. `def _copy_skill_tree` at line 87, `def _iter_support_relpaths` at line 165, and `def _prune_stale_support_files` at line 206 were each verified correct by direct grep; marked `citation-freshness: ignore` rather than continuing to restructure already-accurate prose around a heuristic's parsing limit. No decision text changed.
