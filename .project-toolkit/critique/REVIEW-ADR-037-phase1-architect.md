...
**Recommendation**: Extract Implementation Plan (lines 151-177) to `.project-toolkit/planning/PLAN-memory-router-*.md`. Keep ADR focused on the architectural decision:
...
3. **Phase 2 Escalation**: Once blocking issues resolved, invoke adr-review skill:
   ```bash
   Skill(skill="adr-review", args=".project-toolkit/architecture/ADR-037-memory-router-architecture.md")
   ```
