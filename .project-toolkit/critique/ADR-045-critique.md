...
**Evidence**:
- Path contract defines env vars with defaults (e.g., `AWESOME_AI_SESSIONS_DIR` defaults to `.project-toolkit/sessions`)
...
**Impact**: Skills that write to consumer paths (e.g., session-log-fixer) may fail silently or write to wrong locations. CI environments may not have `.project-toolkit/sessions/` directory.
...
1. Add path validation to framework scripts:
   ...
   if not os.path.exists(SESSION_DIR):
       raise ValueError(f"Session directory not found: {SESSION_DIR}. Set AWESOME_AI_SESSIONS_DIR or create .project-toolkit/sessions/")
   ```
2. Document consumer setup requirements: "Projects using session-protocol must create `.project-toolkit/sessions/` or set AWESOME_AI_SESSIONS_DIR"
