...
**Test**: `find .project-toolkit/architecture -name "ADR-*.md" | sed 's/.*ADR-//' | sed 's/-.*//' | sort | uniq -c`
...
**Test**: `ls -1 .project-toolkit/architecture/ADR-* | grep -E "ADR-[0-9]{1,2}-"`
...
- References in `.project-toolkit/critique/474-adr-numbering-second-review.md` are historical (documenting the problem that was fixed)
...
find .project-toolkit/architecture -name "ADR-*.md" | \
  sed 's/.*ADR-/ADR-/' | sed 's/-.*//' | \
  sort | uniq -c | awk '$1 > 1'

# Verify no a/b notation in active files
grep -rn "ADR-[0-9]\{3\}[ab]" .project-toolkit/architecture/ .serena/memories/adr-reference-index.md
```
