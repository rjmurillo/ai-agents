...
`.project-toolkit/analysis/1769-monolith-section-classification.md`. Relocation lost on
evidence:

1. The relocation target does not exist in the tree.
   `.project-toolkit/analysis/1769-monolith-section-classification.md:87` classifies
...
3. Nothing reads a rank. The hierarchy claimed that a lower tier cannot
   ...
   review in `.project-toolkit/retrospective/2026-08-17-governance-bureaucracy-critical-review.md`
   was filed to catch.
...
adr = Path(".project-toolkit/architecture/ADR-009-parallel-safe-multi-agent-design.md").read_text()
...
- Issue #1769. `.project-toolkit/analysis/1769-monolith-section-classification.md`.
- PR #5127. The reverted attempt whose critic review produced #5130.
- `.project-toolkit/architecture/ADR-009-parallel-safe-multi-agent-design.md`. Canonical
...
- **The `security: 2` misattribution** was found by architect and critic
  separately. Verified: `.project-toolkit/architecture/ADR-009-parallel-safe-multi-agent-design.md:90`
  ...
  0.
- **`.project-toolkit/prototypes/agents/README.md:32`** was found by architect and
...
| `.project-toolkit/prototypes/agents/README.md:32` mandates the deleted field | P2 | architect, independent-thinker | Updated to `metadata.role`, with the frozen-prototype exemption explained so a new prototype does not inherit it. |
...
- **Duplicate section number `2.5`** at `.agents/AGENT-SYSTEM.md:572` (`### 2.5
  ...
  `.project-toolkit/analysis/1769-monolith-section-classification.md` plus the section
...
origin/main...HEAD -- .project-toolkit/architecture/` shows only the ADR-078
...
requires only a staged file under `.project-toolkit/critique/` naming the ADR ID, and is a
...
`.project-toolkit/architecture/ADR-098-agent-role-metadata-replaces-tier-hierarchy.md`
