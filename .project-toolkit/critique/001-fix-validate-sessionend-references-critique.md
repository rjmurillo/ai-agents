...
- 19 session logs in `.project-toolkit/sessions/` correctly reference the script name that existed when sessions occurred
...
- `.project-toolkit/sessions/`: 19 files (HISTORICAL logs - preserve)
- `.project-toolkit/analysis/`: 15+ files (MIXED - evaluate by date)
- `.project-toolkit/planning/`: 10+ files (HISTORICAL - preserve)
- `.project-toolkit/architecture/`: 3 files (CURRENT - must fix):
  - ADR-007-memory-first-architecture.md
  - ADR-011-session-state-mcp.md
  - ADR-019-script-organization.md
- `.project-toolkit/security/`: Files (MIXED - evaluate by date)
...
grep -r "Validate-SessionEnd" \
  --include="*.md" \
  --exclude-dir=".project-toolkit/sessions" \
...
| Session log refs | 19 files | grep -r "Validate-SessionEnd" .project-toolkit/sessions/ |
| Memory refs | 23 files | grep -r "Validate-SessionEnd" .serena/memories/ |
| ADR refs | 3 files | grep -r "Validate-SessionEnd" .project-toolkit/architecture/ |
...
1. Architecture Decision Records (3 files):
   - `.project-toolkit/architecture/ADR-007-memory-first-architecture.md`
   - `.project-toolkit/architecture/ADR-011-session-state-mcp.md`
   - `.project-toolkit/architecture/ADR-019-script-organization.md`
...
4. **Verification**:
   ...
   grep -r "Validate-SessionEnd" \
     ...
     --exclude-dir=".project-toolkit/sessions" \
     --exclude-dir=".serena/memories" \
     --exclude-dir=".project-toolkit/planning" \
     .project-toolkit/architecture/ \
     .claude/skills/ \
     AGENTS.md
   # Expected: 0 results
