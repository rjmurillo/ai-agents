...
and left an inline comment on `.project-toolkit/architecture/ADR-005-powershell-only-scripting.md`
line 16:

> Duplicative. Already in frontmatter

That comment is the decision. It is recorded here because the change touches an
ADR and the commit gate requires an artifact under `.project-toolkit/critique/`, not
