...
Record under review: `.project-toolkit/architecture/ADR-108-template-owned-skill-files.md`
...
`.project-toolkit/specs/requirements/REQ-003-multi-tool-artifact-build.md:359` and appends a
...
converting any other skill waits on the owner's word. `.project-toolkit/specs/design/DESIGN-024-skill-guidance-excerpt-sync.md:72-73`
...
REQ-025. Line 27 says `templates/` holds `agents/`, `platforms/`, and `toolsets.yaml` only, and it
...
- **High-level-advisor: the epic's byte reduction has no funded follow-up, so the record is
  ...
  `.project-toolkit/plans/active/5706-skill-guidance-excerpts.md:109` lists the deferred cut against issues
...
orchestrator, not by a seat. ADR-108:160 cites `.project-toolkit/architecture/ADR-107-...md:61-62` with a
...
`REQ-025-skill-guidance-excerpts.md:96`, which calls it an estimate. The trailing "which" at `:59`
...
`DESIGN-024-skill-guidance-excerpt-sync.md:154`, inside its "Declined alternative" section that
...
`.project-toolkit/plans/active/5706-skill-guidance-excerpts.md:109`. The Context constraint table at `:33`,
...
- **Independent-thinker: the declined design specifies a partials tree in five places.**
  Corrected. Four lines of `DESIGN-024-skill-guidance-excerpt-sync.md` contain
...
- **Round-1 item on ADR-108:111 ("a template layer skills do not have").** The orchestrator had
  ...
  The independent-thinker seat's evidence at `DESIGN-024:154` restores it as a P2: the sentence
...
`.project-toolkit/architecture/README.md:191` carries "amend the two texts that forbid it", and
`uv run python build/scripts/generate_adr_index.py --check` reports
`DRIFT: .project-toolkit/architecture/README.md differs from generated output`. Two seats reported the index
drift; the Status sentence is its root cause, because the index blocker cell is generated from that
paragraph. `TASK-030-skill-excerpt-parity-gate.md:24` names a green `build_all.py --check` as A0's
...
`REQ-025-skill-guidance-excerpts.md:96`. The record names the byte-growth regret at `:144` but ties
...
- **Critic ran `generate_adr_index.py` against the working tree and restored it.** Verified: `git
  diff` is empty and `.project-toolkit/architecture/README.md` remains staged with no unstaged change. The
...
- `:17` now names three texts, ADR-107 property 1, REQ-003-010, and REQ-003 decision D4.
  `.project-toolkit/architecture/README.md:191` carries the matching three-text summary,
...
literal-brace scan, both pass. The partials in DESIGN-024 are specified as substrings of rule
files, which carry no trailing newline of their own, and a repository-wide search of ADR-108,
DESIGN-024, TASK-030, TASK-025 and TASK-027 returns no trailing-newline requirement. Because the
...
DESIGN-024 test matrix, not in this record's decision.
...
`.project-toolkit/specs/design/DESIGN-024-skill-guidance-excerpt-sync.md`" and `:111` says "The
no-amendment alternative exists (DESIGN-024)". That file's frontmatter title is "Mustache-compiled
...
- **Critic: the DESIGN-024 mischaracterization is P1.** Downgraded to P2. DESIGN-024 does record
...
DESIGN-024 test matrix rather than in this record's decision, and it is cheap to close before
...
- Second CodeRabbit round on #5724: section 1 adds the non-symlink and resolved-containment rule for `.claude/skills/<name>/`; REQ-003-005 names the template as the regeneration trigger and the two drift gates; REQ-003-008 gains a template-owned verification fixture; TASK-030 lists task ids explicitly. No decision changed.
- 2026-09-11, after merge: PR #5725 merged with its own REQ-021, DESIGN-020, and TASK-024, so the three artifacts this record cites were renumbered to REQ-025, DESIGN-024, and TASK-030; references in this log were updated in place. No decision changed.
- 2026-09-11, second renumber: PR #5727 renumbered the control-plane set to the same REQ-024, DESIGN-023, and TASK-028 concurrently, so this record's artifacts move again to REQ-025, DESIGN-024, and TASK-030. No decision changed.
- 2026-09-11: status moved from proposed to accepted by the repository owner after PRs #5726 and #5731 merged and issue #5706 closed; implemented: true was set by #5726.
- 2026-09-14: ADR-109 B2 added the `\{{` literal-brace escape to the grammar (Decision section 1 table). Reason: `testing.md` and two skills carry GitHub Actions `${{ }}` text the grammar could not express, which would have left one rule and two skills permanently hand-maintained. Tests in `tests/build_scripts/test_skill_template_grammar.py` cover the escape in a template, in a partial, and the unchanged refusal of an unescaped tag. Review of PR #5775 found the amendment had landed as a third table cell; folded into the Value cell the same day. A symlinked partial is refused at validation, and a NUL byte in a source is refused, so the marker cannot be forged.
