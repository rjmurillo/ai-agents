...
`.project-toolkit/architecture/` plus `.agents/SESSION-PROTOCOL.md`, both of which fire the
...
110 stale planning artifacts moved from `.project-toolkit/planning/` to
`.agents/archive/planning/` by `git mv`. Nothing deleted. The change also rewrites
`.project-toolkit/planning/INDEX.md`, adds an archive index, and repoints inbound links.
...
| F10 | P2 | advisor | Issue #3426 is scoped to `.project-toolkit/plans/`, so citing it as tracking this directory overclaims | Fixed. Scope note added to #3426, claims softened in both indexes |
...
- A link scanner over 977 files across `.project-toolkit/architecture/`, the protocol and
...
- `.project-toolkit/planning/INDEX.md`, the rewritten live index
