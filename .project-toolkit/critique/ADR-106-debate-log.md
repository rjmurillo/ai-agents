...
Record under review: `.project-toolkit/architecture/ADR-106-serena-only-memory-architecture.md`,
...
`.project-toolkit/memory/episodes/` contains a `CLAUDE.md` alongside 750 episode files.
...
`.project-toolkit/memory/episodes`. Item 2 also contradicted item 3 of the same ADR,
...
validator models amendments, and `.project-toolkit/architecture/README.md` regenerates
...
**One finding was fixed rather than deferred.** The audit noted that
`.serena/memories/adr/adr-037-accepted.md` asserts `**Status**: Accepted` and
carries "Forgetful augmentation - Enhances but never replaces Serena results" as
a live decision, and that this merge is what makes both false. Serena memory is
re-injected ahead of reasoning, so a stale record there is read before the ADR
that retired it. Deferring a falsehood into the surface agents consult first
would have reproduced, in the memory tree, exactly the condition ADR-106 exists
to end. That file is now marked superseded in place per the `curating-memories`
convention: the historical decisions are kept and labelled, current truth is
stated above them, and the ADR-038 episode tier is carved out explicitly so the
correction does not overreach. `.serena/memories/adr-reference-index.md` no
longer advertises the record as accepted, and `memory-index.md` token counts were
regenerated by `scripts/update_memory_index_tokens.py`.
