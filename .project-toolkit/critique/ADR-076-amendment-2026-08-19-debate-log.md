...
**Subject**: "Amendment 2026-08-19: PR-live-state check on the renew path" appended to `.project-toolkit/architecture/ADR-076-pr-autofix-branch-ownership-lease.md`, addressing issue #5165.
...
The implemented placement (widened read after the `RENEW_SKIP_MARGIN` noop fast path, per finding 5 above and the Phase-3 resolution) was found incomplete during CI on PR #5167: the AI Spec Validator workflow (`Validate Spec Coverage` check) traced the acceptance criterion "renew against a merged/closed PR returns SKIP/pr-closed" against the actual code path and found it false for a self-renew with more than `RENEW_SKIP_MARGIN` (5 min) of its 15-minute TTL remaining, because the noop fast path returns before the widened read ever executes. Verified directly by reading `acquire()`: the noop check at the time sat textually before the widened-read block, so any call satisfying `already_holds and current.expires_at - now > RENEW_SKIP_MARGIN` returned `ACT`/`self-renew-noop` without ever reaching the merged/closed check.

This is not a new disagreement between reviewers; it is evidence that finding 5's optimization (place the read after the noop check "so it only fires when a write is actually imminent") was itself wrong, because the noop check can return before the write path is ever entered. No agent in round 1 traced that specific control-flow interaction. Fixed by moving the widened read and the pr-closed check to run unconditionally on every `renewing=True` call, before the noop fast path, so the noop path can only be reached once the PR is confirmed open. This costs one REST read on every renew call rather than only on the write-imminent subset; it adds no new write, since the pr-closed check returns before the noop fast path and the noop fast path itself still exists unchanged for confirmed-open PRs.

Applied to `.claude/skills/github/scripts/pr/pr_autofix_lease.py` and mirrored to `src/copilot-cli/`; a regression test (`test_renew_on_merged_pr_with_ample_ttl_returns_skip_pr_closed`) pins the exact scenario that was missed: same ample-TTL fixture as the noop-path test, with the PR merged, asserting `SKIP`/`pr-closed` rather than `ACT`/`self-renew-noop`. The ADR amendment's Decision, Placement, Fail-open, Alternatives, and Impact sections are corrected in place to describe the new ordering rather than left contradicting the shipped code.

No further disagreement is open; this is a mechanical correction of an implementation defect the debate did not catch, not a reopened design question.
