...
|----------|------------|------------|------------|-----------|
| Staged every importer fixture as an absolute `tmp_path` file, hiding a whole input class from the entire suite | Unreachable coverage, shipped from commit `176e68873` and found in round 13 | A configured importer may be RELATIVE, and `tsx` reads a leading dash as a flag, so a legal file named `--experimental-foo` was handed over as a flag: the importer never ran and the run still exited 0, the same silent-failure shape as the original defect. Thirteen rounds of review and my own single-decision mutations on that exact function never surfaced it, because a mutation can only probe code paths the fixtures can reach | Vary fixture SHAPE, not just fixture values: relative and absolute, dash-leading and plain. Negative controls test the code; nothing tests the input space the fixtures admit, so that space has to be chosen deliberately | 95% |
| Fixed the "no Copilot plugin" overclaim by editing the occurrences visible in the diff | Incomplete correction, recurred in round 13 | The round-6 fix corrected what the diff showed and left one table cell asserting the plugin does not exist at all, contradicting three corrected statements in the same file. A claim is corrected when no instance of it survives in the artifact, not when the diff looks right | Correct a false claim with a full-artifact search for the claim itself, then re-search after editing; a diff-scoped fix leaves the instances the diff never touched | 90% |
