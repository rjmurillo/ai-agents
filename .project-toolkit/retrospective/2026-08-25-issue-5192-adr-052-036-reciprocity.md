...
| Push-time gates (`retrospective-policy`) have no committed worked example in `.project-toolkit/retrospective/` for a session to learn the contract from, forcing a source-read of the enforcing script | First observed this session. **Correction (2026-08-25, Copilot review): the root cause named here originally contradicted this same file's own Finding 1 correction above**, which established the 2026-08-21 retrospective WAS committed. The real root cause is narrower: `_today_retrospective_exists` only accepts a retrospective dated today or yesterday, so a real, committed, four-day-stale example still fails to satisfy a later session's gate. The gap is a date-window limitation, not an absence of any worked example. | Low-medium: cost about two tool calls to resolve by reading `git_hook_policy.py` directly, but a documented recipe would have made it a lookup instead of an investigation | Documentation gap |
...
**Finding 1** (recurring em/en-dash self-violation): **Correction (2026-08-25, review): does not fit FM #9.** `.agents/governance/FAILURE-MODES.md:288` defines FM #9's shape as "partial signal, premature conclusion, confident delivery, multi-round correction," and its Detection section names a PR correcting the same mistake across three or more commits. This finding's actual shape is a single-shot catch: the em-dash violation was caught and fixed at the first pre-commit gate, with no multi-round correction. It also is not true that the 2026-08-21 retrospective "assigned" FM #9 to its own instance of this pattern; that retrospective's own Finding 2 explicitly says the event "does not cleanly fit an existing `FAILURE-MODES.md` class" and calls FM #9 only the closest analogy in general shape, the same hedge this retrospective's Finding 1 (above, this same file) independently reached before this addendum over-firmed it. Neither existing class in `FAILURE-MODES.md` cleanly covers "high-confidence prose generation that violates a rule already loaded in context, caught on the first downstream gate before any multi-round correction is needed." Per `.claude/rules/retros.md` MUST 2, a class that does not fit needs a new class proposed in a linked ADR, not a forced fit to the nearest existing one. **Correction (2026-08-25, second review): proposed as issue #5300**, since MUST 2 requires the retro to route to a proposal, not leave the finding permanently unclassified. Issue #5300 scopes the proposed class (single-shot rule violations caught at the first gate, distinct from FM #9's multi-round shape) and cites both retrospectives' instances as evidence; the ADR itself, with its own adr-review debate per ADR-073, is that issue's deliverable, not this correction's.

**Rejected alternative (2026-08-25, third review): FM #3 does not fit either.** A separate autofix agent independently proposed classifying this finding as FM #3 (Ambiguous Instruction Inversion) instead of routing to issue #5300. FM #3's own description (`.agents/governance/FAILURE-MODES.md:96-100`) requires an ambiguous instruction that the agent resolves permissively ("SHOULD" decaying into "MAY"); the em/en-dash prohibition is an unambiguous MUST NOT with no permissive reading available, so there is no ambiguity to invert. The proposal's own text conceded this ("the rule was unambiguous but simply not actively self-checked"), which describes a missing self-check, not an ambiguous-instruction failure, and forcing the label onto a class it admits doesn't fit repeats the exact anti-pattern this correction exists to stop. The proposal's own prose also contained an em dash, which the dash-prohibition finding this classification is about would have caught had it been staged through this repository's own gates.
