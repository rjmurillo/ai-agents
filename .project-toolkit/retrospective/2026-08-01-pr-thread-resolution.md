...
The push hook checks for retrospective patterns (heading `## Retrospective`, reference to `.project-toolkit/retrospective/`, or `learnings? captured`). A session log without those patterns is not enough. Creating a retro file under `.project-toolkit/retrospective/` (dated today) satisfies the gate.
