...
**Target artifact**: `.project-toolkit/architecture/` - consider an ADR documenting the empirically verified Copilot CLI and Claude Code hook runtime contracts
...
| 9 | Create ADR-063 for plugin hook runtime contract | `.project-toolkit/architecture/ADR-063-plugin-hook-runtime-contract.md` | Document the empirically verified Copilot CLI + Claude Code hook invocation contract | ADR review BLOCKING - do not create without adr-review | PENDING |
...
Those files remain in `.project-toolkit/retrospective/` as session artifacts. For the authoritative post-incident analysis, reference this document.
