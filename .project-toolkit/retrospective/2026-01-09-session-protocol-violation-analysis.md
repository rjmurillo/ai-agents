...
3. **Reduce**: Context compaction resets state
   - Write session-initialized flag to `.project-toolkit/sessions/.current`
...
- **File lock mechanism**: `.project-toolkit/sessions/.lock` blocks work until session initialized
- **Pre-work validation gate**: Check initialized state before tools execute
- **Persistent session flag**: `.project-toolkit/sessions/.current` survives context compaction
...
**Statement**: "Block tool execution until session lock file exists at `.project-toolkit/sessions/.lock` confirming initialization complete"
...
**Statement**: Block tool execution until session lock file exists at `.project-toolkit/sessions/.lock`
...
3. Create session log at `.project-toolkit/sessions/YYYY-MM-DD-session-NN.json`
...
$sessionNumber = (Get-ChildItem .project-toolkit/sessions/*.json |
...
1. Create lock file at `.project-toolkit/sessions/.lock` after initialization complete
...
function Invoke-ToolWithSessionGate {
    ...
    if (-not (Test-Path .project-toolkit/sessions/.lock)) {
...
1. Write persistent state flag to `.project-toolkit/sessions/.current` with session metadata
...
```powershell
# Add to conversation start hook
if (-not (Test-Path .project-toolkit/sessions/.current)) {
...
SESSION_LOG=$(find .project-toolkit/sessions -name "$(date +%Y-%m-%d)-session-*.json" -type f)
...
|-----------|------|--------|
| git add | `.project-toolkit/retrospective/2026-01-09-session-protocol-violation-analysis.md` | Retrospective artifact |
