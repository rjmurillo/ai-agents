...
5. `yagni:` 291-line metrics reader over `.project-toolkit/sessions/`, an artifact type
...
7. `delete:` v0.3.0 orchestrator prototype: 1,339-line bash driver, its
   ...
   [`.project-toolkit/projects/v0.3.0/`, `.project-toolkit/projects/v0.3.1/`]
...
5. `.claude/rules/session-logs.md:9`: "Session log creation is discontinued: do
   not create a new `.project-toolkit/sessions/*.json` file." The 1,467 existing logs are
   frozen history. An earlier draft said 1,538, which is every tracked file under
   `.project-toolkit/sessions/`: it counts the 15 files in `handoffs/` and the non-JSON
...
7. `.project-toolkit/projects/v0.3.0/scripts/orchestrate.sh` is 1,339 lines of bash in a
   ...
   `.project-toolkit/projects/v0.3.1/PowerShell-migration.md` plans a migration that is
