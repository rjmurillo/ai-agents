...
|------|-----------|-------------|
| **Spec** | [`.project-toolkit/specs/requirements/REQ-003-multi-tool-artifact-build.md`](.project-toolkit/specs/requirements/REQ-003-multi-tool-artifact-build.md) | EARS requirements with verified Copilot CLI facts |
| **Plan** | [`.project-toolkit/plans/active/req-003-multi-tool-artifact-build.md`](.project-toolkit/plans/active/req-003-multi-tool-artifact-build.md) | 6 milestones, 30 tasks, ~23 person-days |
| **ADR Amendment** | [`.project-toolkit/architecture/ADR-006-thin-workflows-testable-modules.md`](.project-toolkit/architecture/ADR-006-thin-workflows-testable-modules.md) (Amendment 2026-04-28) | Config-data exception |
| **Debate log** | [`.project-toolkit/critique/ADR-006-amendment-2026-04-28-debate-log.md`](.project-toolkit/critique/ADR-006-amendment-2026-04-28-debate-log.md) | Round 1 + Round 2 multi-agent consensus |
| **Triggering incident** | [`.project-toolkit/incidents/2026-04-27-pir-plugin-manifest-schema-1773.md`](.project-toolkit/incidents/2026-04-27-pir-plugin-manifest-schema-1773.md) | PR #1773 PIR (motivates schema gates) |
...
- **`.project-toolkit/specs/requirements/REQ-003-multi-tool-artifact-build.md`** (428 lines): EARS-format spec. 12 acceptance criteria (REQ-003-001 through -012), 11 locked architectural decisions (D1-D11), CVA matrix, verified-facts table with citations, 4 residual open questions tagged for empirical post-merge testing, 7 risks pre-flagged.
- **`.project-toolkit/plans/active/req-003-multi-tool-artifact-build.md`** (149 lines): 7-milestone execution plan (M0 ADR gate + M1-M6 implementation). 30 atomic tasks, 14S/10M/3L sizing, ~23 person-days. Risk register with R1-R10 (matcher shim whitespace bypass, applyTo unknown CLI consumption, etc.). M5 kill criteria documented. Single critical path; no inter-milestone parallelism.
- **`.project-toolkit/architecture/ADR-006-thin-workflows-testable-modules.md`**: Amendment 2026-04-28 (165 added lines). 7 conditions gate the config-data exception. Anchors original ADR-006 rationale verbatim. Forward-looking policy with grandfathering for existing `templates/platforms/*.yaml` files. Reversibility assessment + confirmation method.
- **`.project-toolkit/critique/ADR-006-amendment-2026-04-28-debate-log.md`**: Multi-agent debate log. Round 1: 6 agents (architect APPROVE_WITH_CHANGES, critic NEEDS_REVISION, independent-thinker/security/analyst D&C, advisor ACCEPT). Round 2: all blocking findings addressed; 6/6 ACCEPT consensus.
- **`.project-toolkit/sessions/2026-04-28-session-1761-...json`**: Protocol-compliant session log.

## Verification

```text
$ python3 scripts/validate_session_json.py .project-toolkit/sessions/2026-04-28-session-1761-*.json
[PASS] (after session-end)

$ ls .project-toolkit/critique/ADR-006-amendment-2026-04-28-debate-log.md
exists  # ADR architect-gate hook satisfied

$ wc -l .project-toolkit/specs/requirements/REQ-003-multi-tool-artifact-build.md \
        .project-toolkit/plans/active/req-003-multi-tool-artifact-build.md \
        .project-toolkit/architecture/ADR-006-thin-workflows-testable-modules.md
...
- [x] Debate log artifact exists at `.project-toolkit/critique/` (satisfies architect-gate hook)
