...
5. **Add the index entry by hand when writing a Serena memory**. A new memory under `.serena/memories/` MUST get a keyword line in `.serena/memories/memory-index.md` in the same change. The index is a curated retrieval map, not a manifest: 305 of the 897 memory files on disk carry an entry, and a memory with no entry is unreachable through the keyword path even though the file is committed and readable by name. Nothing catches the omission. `scripts/validation/memory_index.py --ci` walks index entries and checks each one resolves to a file on disk, so it reports `Files: 305 indexed, 0 missing` whether or not the new memory is listed. Verified by deleting a just-written memory and re-running: the count and the verdict were byte-identical with the file present and absent. Add the line, then let the `memory-token-update` hook fill the token count; write `(0)` as the placeholder rather than guessing a number.
