...
- **Parallel teams carry a context-duplication tax.** An UpGPT study across 52 Next.js/TypeScript/Supabase benchmarks (Sonnet 4.6) measured parallel subagent teams at 73 to 124 percent higher token cost than sequential execution, with equivalent output quality. Each spawn loaded full context independently and missed cache, so the cache-fill was paid once per worker instead of once per run. Source: [UpGPT benchmarks](https://upgpt.ai/blog/upcommander-benchmarks).
