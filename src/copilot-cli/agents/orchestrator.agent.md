...
- **Parallel teams carry a context-duplication tax.** UpGPT measured agent teams at 73 to 124 percent higher token cost than sequential execution with no quality gain (N=5 for that comparison). Each agent loaded the full codebase context independently, so three agents meant three copies of an 80,000-token context and the cache burn dominated. The study ran one codebase of greenfield TypeScript tasks and varied the model across Haiku, Sonnet, and Opus, so treat it as directional. Source: [UpGPT benchmarks](https://upgpt.ai/blog/upcommander-benchmarks).
