...
| Exit 2 | Block the action. stderr is the agent-visible channel for PreToolUse blocks; hooks emit the reason on stderr, a JSON decision payload on stdout, or both | CODE | `invoke_branch_context_guard.py:96,43` (JSON block on stdout, reason on stderr); `invoke_skill_first_guard.py:368` (stderr-only reason) |
