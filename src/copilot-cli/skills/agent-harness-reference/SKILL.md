...
| `CLAUDE_CODE_ENTRYPOINT` | Set by Claude Code, absent from the Copilot CLI build measured. This is the only confirmed host signal; do not rely on `COPILOT_CLI`, which is unverified | EMPIRICAL (Copilot CLI 1.0.80 and Claude Code, issue #4727) | `references/probe-evidence.md` section 8b |
