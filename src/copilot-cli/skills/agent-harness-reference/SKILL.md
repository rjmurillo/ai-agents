...
| Exit 2 | Block the action via exit 2. The block reason is surfaced on stderr (the agent-visible channel for PreToolUse blocks) and/or printed to stdout | CODE | `invoke_skill_first_guard.py:368` (block reason on stderr, exit 2); `invoke_branch_context_guard.py:187,191` (block reason on stdout, exit 2) |
