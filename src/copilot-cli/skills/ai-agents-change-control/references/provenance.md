...
| Commit count is advisory only, no block or bypass label (ADR-099) | `scripts/validation/pr_commit_count.py` (`WARNING_THRESHOLD`/`ALERT_THRESHOLD`, `classify_count`) | `grep -n "THRESHOLD = " scripts/validation/pr_commit_count.py; grep -n "BLOCK_THRESHOLD\|BYPASS_LABEL" scripts/ci/enforce_pr_validation.py` (second command should print nothing) |
