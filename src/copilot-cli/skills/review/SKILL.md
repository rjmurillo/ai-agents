...
**Self-audit round cap (hard stop).** `/review` runs each axis once per invocation; no internal fix-and-re-review loop. The cap binds the caller: any skill, command, or agent chaining "fix, re-invoke `/review` on the same diff base, repeat" with no human checkpoint MUST NOT exceed 3 rounds. The caller tracks its own count; `/review` holds no shared state. A diff-base change starts a new chain. Hard stop, not a guideline. Round 3: `PASS` ships; `WARN`, `CRITICAL_FAIL`, or `UNKNOWN` escalates to the operator with every open finding (axis, verdict, location, recommendation); no self-acknowledging a `WARN` to ship. Never loop past round 3 silently.

Modeled on the PR-loop cap at `.claude/skills/github/scripts/pr/check_pr_round_cap.py` (issue #5056, after PR #1887 ran 11+ rounds over 46h uncapped). Quoted (lines 40-41, 188-189): "0 = round recorded, under both caps (ACT); 1 = a cap is exceeded (ESCALATE)"; `_DEFAULT_MAX_ROUNDS = 5`, `_DEFAULT_MAX_HOURS = 4.0`.

**Stricter/looser/different than canonical**: looser on round count (3 vs. default 5) and time (canonical also caps elapsed hours; this has none). Looser on persistence (canonical persists state across restarts via a PR-comment marker; this relies on the caller's in-session tracking). Different in kind: canonical only gates `ACT`/`ESCALATE` for pr-autofix's loop, never ships; round 3 here can ship `PASS`, which canonical lacks.
