...
**Self-audit round cap (hard stop).** `/review` itself runs each axis once per invocation and returns; it has no internal fix-and-re-review loop. The cap below binds the caller: any skill, command, or agent that chains "fix findings, re-invoke `/review` against the same diff base, repeat" without a human checkpoint in between MUST NOT run more than 3 such `/review` rounds. The caller owns and tracks its own round count for that chain; `/review` carries no shared round-count state of its own. A round counts against the cap only while the diff base is unchanged: if the caller moves to reviewing a different branch, PR, or unrelated diff, that starts a new chain with its own count. This is a hard stop, not a guideline, modeled on the PR-loop cap in `check_pr_round_cap.py` (issue #5056), built after PR #1887 ran 11+ review rounds over 46 hours with no cap. On the 3rd round: if the FINAL VERDICT is PASS (or a WARN the caller has explicitly acknowledged, matching the acknowledged-WARN handling in "Write the SHA-bound PASS marker" below), ship. Otherwise stop and escalate to the operator, listing every open finding from that round (axis, verdict, location, recommendation). Never continue looping past the 3rd round silently.
