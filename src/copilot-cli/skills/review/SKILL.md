...
1. Read the diff with three-dot range syntax (`git diff "origin/$BASE_BRANCH"...HEAD`, where `BASE_BRANCH` is the detected base branch and the remote-tracking ref is used because the local base branch may not exist in fresh clones) so every evaluation step uses the same change set as the diff-scoped gates in step 5.
   ...
   - **Python version floor**: when the diff contains any `.py` files, read `pyproject.toml` and extract `project.requires-python` (e.g. `>=3.14`). Include this floor in the context block sent to every axis. Axis prompts MUST NOT report a version-sensitive finding (deprecated syntax, removed built-in, runtime behavior change) for a Python version older than the declared floor. Example: `isinstance(x, A | B)` is valid from Python 3.10 (PEP 604); if `requires-python = ">=3.10"` or higher, no finding is permitted. If `pyproject.toml` is absent or `requires-python` is missing, record `Python floor: unknown` and treat it as unresolved context, not as a ground for a finding.
...
4a. **Validate scope of each axis output against the PR diff.** After each axis (steps 2, 3, and every axis in step 4) returns its findings text, run `validate_findings_scope.py` (resolved via the "Path resolution" section, under `scripts/validate_findings_scope.py`) to detect findings whose `location:` fields name files outside the three-dot diff collected in step 1. Invocation: `python3 <validate_findings_scope.py> --worktree <WORKTREE_PATH> --base-branch <BASE_BRANCH> --text <AXIS_TEXT>`. When the script exits 1 (out-of-scope locations found), prepend `[pre-existing - not in this PR diff]` to each flagged finding before recording the axis output and before extracting the verdict. Do NOT suppress or skip the finding; preserve it with the label so the reviewer can see it was noted but does not belong to this change. When the script exits 0 (all in scope, or diff empty/unavailable), record the axis output unmodified. A scope-check failure never changes an axis verdict; it only annotates findings.
