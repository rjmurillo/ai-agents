...
| Adding to always-on context | Passive context is billed on every request, whether or not the task needs it | Use progressive disclosure by default. Read `context-optimizer/references/model-context-doctrine.md` and `rule-audit-procedure.md` before proposing always-on text. |
