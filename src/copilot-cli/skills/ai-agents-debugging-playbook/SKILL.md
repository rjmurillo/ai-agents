...
| Fix a NON_COMPLIANT session log | Hand-edit against `.project-toolkit/schemas/session-log.schema.json`, re-run `scripts/validate_session_json.py` |
...
| Coverage pin trips at ~63% (or another surprise number) | Re-run the pinned step's EXACT file list locally, e.g. `pytest tests/test_ai_review.py tests/test_verdict.py tests/test_quality_gate.py --cov --cov-branch --cov-report=` then `coverage report --include=scripts/ai_review_common/verdict.py --fail-under=100` | Does the number change when you add/remove test files? The pin is file-set sensitive (issue #1963): running one file alone reports 63% | Run all files that exercise the module; add tests for genuinely uncovered branches (`.github/workflows/pytest.yml:424-437`, `ai_review_common.verdict`) | The pin runs as two steps: broad collection (`--cov --cov-branch --cov-report=`), then a separate `coverage report --include=` isolation gate. Do not collapse them into one module-name `--cov=` call; that combination is what issue #2063 found broken, for the REQ-009 pin the next Provenance row covers |
...
4. Commit with the discipline gates expect: 5 files or fewer per commit. Commit count itself is advisory only (`needs-split` label plus a WARNING/ALERT notice at 10/15 commits, via `git rev-list --count HEAD ^origin/main`; no PR can be blocked on commit count alone, ADR-099).
...
| install-parity checks co-change, not content; skips `RULE` and hand-maintained-only diffs | `build/scripts/validate_install_parity.py:346-360` | `sed -n '346,360p' build/scripts/validate_install_parity.py` |
| Agent drift compares a `SECTIONS_TO_COMPARE` (23-heading) allowlist at 80 percent similarity, reported not enforced | `build/scripts/detect_agent_drift.py:77-101,984`, `scripts/ci/drift_run_detection.py:53-60` (`drift_detected`) | `sed -n '77,101p;984p' build/scripts/detect_agent_drift.py; sed -n '53,60p' scripts/ci/drift_run_detection.py` |
| testpaths exclude skill tests | `pyproject.toml [tool.pytest.ini_options].testpaths` | `grep -n testpaths pyproject.toml` |
| FAILURE-MODES.md 12 sections | `.agents/governance/FAILURE-MODES.md:34-524` (`Post-Completion Continuation`) | `grep -c "^## [0-9]" .agents/governance/FAILURE-MODES.md` |
