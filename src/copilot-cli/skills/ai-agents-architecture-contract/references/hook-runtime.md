...
The dispatcher originated in Copilot CLI 1.0.57-era matcher and timeout behavior. Copilot CLI 1.0.72-1 honors supported matchers and fails open on timeout, but one dispatcher per event remains the compatibility fallback and single-producer boundary for PermissionRequest translation. Harness details are owned by `agent-harness-reference`. Issue #3218 closed after verifying the expansion seam and matcher union remain on the live generation path, while parity tests cover the generated surfaces. Removing them now requires a new architecture decision, not technical-debt cleanup.
