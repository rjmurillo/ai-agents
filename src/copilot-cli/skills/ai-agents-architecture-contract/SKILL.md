...
|---|---|---|
| Hook sources serve different consumers | `.claude/settings.json` has 3 events and 4 groups; `.claude/hooks/hooks.json` has 2 events and 3 groups | Do not force parity. Verify whether a hook is repository-only or vendored before editing either source |
