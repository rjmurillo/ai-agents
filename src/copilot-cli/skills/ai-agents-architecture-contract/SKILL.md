...
|---|---|---|
| Generators never write `.claude/` (except the ADR-108 template-owned skill files) | `assert_no_claude_writes` at `build_all.py:841`, applied at lines 2318 to 2328, exit 2 | Canonical tree gets silently overwritten by its own mirror; source of truth inverts |
