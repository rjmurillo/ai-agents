...
| Shared Python libs | `scripts/hook_utilities`, `scripts/github_core`, `scripts/ai_review_common` | `.claude/lib/{hook_utilities,github_core,ai_review_common}` (imports rewritten to relative) | `build_all.py`'s lib step (`build/scripts/lib_mirror.py`, ADR-109 B5); `--check` is the CI dry-run |
