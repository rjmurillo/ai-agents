...
| Trap | Symptom | Fix |
|------|---------|-----|
| CONTRIBUTING.md build commands are DEAD | `pwsh build/Generate-Agents.ps1` (CONTRIBUTING.md:155) and `pwsh build/scripts/Invoke-PesterTests.ps1` (CONTRIBUTING.md:741) fail: zero `.ps1` files exist in the repo (ADR-042) | Real commands: `python3 build/generate_agents.py` and `python3 build/scripts/build_all.py`. `templates/README.md` is similarly stale |
