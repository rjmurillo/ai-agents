...
| `SKIP_TESTS` no longer defaults a flag | `grep -n "SKIP_TESTS" scripts/validation/pre_pr.py` (expect no matches; `pre_pr_sequence.py` still names `--skip-tests` in the `_Gate` docstring, on purpose, to say why `skip_flag` is gone) |
| every `pre_pr.py` argument has a consumer | `uv run --frozen python scripts/validation/pre_pr.py --help` lists only `--quick` and `--markdown-lint-only` |
