...
Three classes of path are recorded and not compared, because a PR cannot deliver content through them: option flags, option values, and bare interpreter names that are not paths (nothing to compare); files outside the git work tree, such as an installed-plugin script, which a PR to the consumer repository cannot rewrite and which has no trusted-ref copy; and untracked work-tree files, such as the `--dispositions-file` JSON a reviewer writes during the review, since PR content arrives through a checkout and is therefore always tracked. A repo-local path that resolves outside the work tree (a PR-committed symlink) fails closed as untrusted.

Scope: a `trusted` verdict on both fields covers the config and every tracked work-tree file the commands name. It does NOT cover transitive imports: a verified verifier script that imports a sibling module from the PR working tree still executes that module's PR-controlled code. Whole-directory verification was considered and deliberately not adopted, because the named-file boundary is the one that can be stated precisely without blocking unrelated PRs. It also does not vouch for the dispatcher script itself, which lives in the same PR tree; nothing a script asserts about itself can close that. Both residuals are the same exposure as running tests or lint on a PR branch.
