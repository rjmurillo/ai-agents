...
<!-- vendor-portability: declared. Step 1 cites scripts/validation/instruction_budget_constants.py as the file that holds the byte ceilings, so a reader can check the number the gate enforces today rather than trusting the one quoted here. It is a citation in a narrative, not a path the skill reads or writes. A vendored install loses the ability to open that file locally; the procedure still runs, it reads its verdict from whatever the installed harness enforces. The archive citation this marker used to declare now lives in rule-audit-instrument.md and is declared there. Issue #2050. -->
