...
76 bytes larger in total (50,988 always-on) because `generate_rules.py`
...
50,988-byte always-on corpus measured at source. `code-quality` and
...
| `paths: ["**"]` | `builder-ethos`, `search-before-building`, `universal`, `voice` |
...
`src/copilot-cli/instructions` carries 4 rules and 50,912 bytes, matching
...
The always-on corpus is 6.2x that threshold and a Python edit sees 11.3x,
