...
**always-on corpus is 8 rules, 72,784 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,535 bytes
...
135 bytes larger in total (72,919 always-on) because `generate_rules.py`
...
That leaves 14,152 always-on bytes of book-derived rule, 19.4% of the
72,919-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 72,784 bytes, matching
