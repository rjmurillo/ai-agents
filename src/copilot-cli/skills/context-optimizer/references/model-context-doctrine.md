...
**always-on corpus is 6 rules, 56,321 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,318 bytes
...
112 bytes larger in total (56,433 always-on) because `generate_rules.py`
...
corpus has taken. The biggest always-on file is `voice.md` at 18,142 bytes.
...
56,433-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 6 rules and 56,321 bytes, matching
