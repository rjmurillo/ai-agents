...
**always-on corpus is 8 rules, 74,197 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 99,948 bytes
...
135 bytes larger in total (74,332 always-on) because `generate_rules.py`
...
largest rule either: `voice.md` at 19,072 bytes is the single biggest
always-on file.
...
That leaves 14,152 always-on bytes of book-derived rule, 19.3% of the
74,332-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 74,197 bytes, matching
...
The always-on corpus is 9.1x that threshold and a Python edit sees 12.2x,
