...
**always-on corpus is 3 rules, 36,847 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 78,205 bytes
...
57 bytes larger in total (36,904 always-on) because `generate_rules.py`
...
36,904-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 3 rules and 36,847 bytes, matching
...
The always-on corpus is 4.5x that threshold and a Python edit sees 9.6x,
