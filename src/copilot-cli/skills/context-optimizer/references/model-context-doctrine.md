...
**always-on corpus is 7 rules, 70,297 bytes**: the ones that load regardless
...
116 bytes larger in total (70,413 always-on) because `generate_rules.py`
...
70,413-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 7 rules and 70,297 bytes, matching
