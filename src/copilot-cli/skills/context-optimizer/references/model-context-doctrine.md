...
**always-on corpus is 8 rules, 73,909 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 99,660 bytes
...
135 bytes larger in total (74,044 always-on) because `generate_rules.py`
...
74,044-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 73,909 bytes, matching
...
The always-on corpus is 9.0x that threshold and a Python edit sees 12.2x,
