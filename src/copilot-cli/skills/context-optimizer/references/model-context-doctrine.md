...
**always-on corpus is 8 rules, 71,505 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 97,790 bytes
...
135 bytes larger in total (71,640 always-on) because `generate_rules.py`
...
| `unified-software-engineering.md` | 8,776 | code files only | 3 positive, 1 negative | yes |

That leaves 14,152 always-on bytes of book-derived rule, 19.8% of the
71,640-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 71,505 bytes, matching
...
The always-on corpus is 8.7x that threshold and a Python edit sees 12.0x,
