...
72KB, with a Python edit pulling in roughly 98KB.
...
Measured at `98526f68b`. Two numbers, and they are not interchangeable. The
**always-on corpus is 8 rules, 71,901 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 97,652 bytes
...
135 bytes larger in total (72,036 always-on) because `generate_rules.py`
...
That leaves 14,152 always-on bytes of book-derived rule, 19.6% of the
72,036-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 71,901 bytes, matching
...
The always-on corpus is 8.8x that threshold and a Python edit sees 12.0x,
