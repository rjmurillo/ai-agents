...
Measured on this branch after `voice` dropped its jargon gloss list. Two numbers, and they are not interchangeable. The
**always-on corpus is 7 rules, 69,790 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 97,717 bytes
...
116 bytes larger in total (69,906 always-on) because `generate_rules.py`
...
largest rule either: `voice.md` at 17,463 bytes is the single biggest
always-on file.
...
That leaves 14,152 always-on bytes of book-derived rule, 20.2% of the
69,906-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 7 rules and 69,790 bytes, matching
...
The always-on corpus is 8.5x that threshold and a Python edit sees 12.0x,
