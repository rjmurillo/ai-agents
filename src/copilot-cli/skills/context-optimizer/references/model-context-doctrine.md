...
Measured on this branch after `lsp-first` was scoped to code files instead of
`**` (it gives code-navigation advice that has nothing to say about a markdown
edit). Two numbers, and they are not interchangeable. The
**always-on corpus is 7 rules, 68,531 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 97,693 bytes
...
116 bytes larger in total (68,647 always-on) because `generate_rules.py`
...
That leaves 14,152 always-on bytes of book-derived rule, 20.6% of the
68,647-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 7 rules and 68,531 bytes, matching
...
The always-on corpus is 8.4x that threshold and a Python edit sees 12.0x,
