...
Written 2026-07-02; every baseline in this skill re-measured 2026-07-29 by running each instrument on this checkout. Retro-cited short SHAs are not reachable from `main`: every PR squash-merges into a new commit, so the PR-branch SHA a retro recorded never lands on `main`. Do not use `git log` to re-derive any of this.
