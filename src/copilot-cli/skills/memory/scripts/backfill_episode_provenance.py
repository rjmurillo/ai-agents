...
mark them as hand-seeded rather than derived: they carry a ``last_used`` field,
which the generator writes nowhere, and their ids are sequential, where every
generated id is a 12-character sha256 prefix. The generator does now write
pattern ids (#3353), so the presence of an ``id`` no longer separates them; the
shape of it does.
