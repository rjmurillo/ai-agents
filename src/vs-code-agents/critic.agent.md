...
- **Mirror-obligation drift**: a behavior change that touches no old-behavior test is a FAIL, not a pass. A broadened detector with no no-over-fire test, a guard with an untouched sibling branch, or a contract change with no flipped old-contract test means the inverse was never closed (see implementer "Mirror Obligation" and `.agents/governance/TESTING-RIGOR.md`). Demand the inverse test or fail the diff. Distilled from samples #1091 (false positive on an unread valid case), #1010 (open sibling branch), #994 (PASS only after old-contract tests flipped).
- **Generated-file drift**: a change to a canonical source (a `.claude/rules/*` rule, a `templates/agents/*` prompt, a skill `SKILL.md`) that does not move its generated mirror (`.github/`, `src/copilot-cli/`, `src/claude/`) in the same diff ships drift. Confirm the mirror changed; an out-of-sync copy is a silent regression.
