...
**always-on corpus is 3 rules, 37,153 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 79,089 bytes
...
57 bytes larger in total (37,210 always-on) because `generate_rules.py`
...
37,210-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 3 rules and 37,153 bytes, matching
...
The always-on corpus is 4.5x that threshold and a Python edit sees 9.7x,
