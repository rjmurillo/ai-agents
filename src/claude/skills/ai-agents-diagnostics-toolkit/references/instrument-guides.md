...
- Current baseline (as of 2026-09-06): 223 files scanned, 673 refs checked, 0 findings, `VERDICT: PASS`, exit 0. The instrument reads green only because 238 refs are directive-suppressed: the historical specs that once produced findings carry `orphan-ref-ignore` markers, so read the suppression count alongside the verdict. Suppression carries almost the entire load, and it grew: issue #5581 cleared 44 findings, every one by directive rather than by correcting a reference or restoring an entity.
- Reconciling that number, because subtracting it is the first thing a reader does: the previous baseline was 191 suppressed, and 191 + 44 is 235, not 238. The extra 3 are references that share a line with a suppressed finding. A directive is line-scope, so it silences every reference on its line, not only the one that was a finding. All 3 are the token `argv` in prose (`.claude/rules/testing.md` and its two generated mirrors), which names no artifact and costs nothing to stop watching. A fourth, the live `reflect` skill, was collateral in the first pass and was recovered by splitting its table row so the directive covers only the dead reference. Expect this gap on any line-scope suppression, and account for it rather than letting a reader find it.
- Treat a rising suppression count as the number to challenge in review.
...
- Trap: do not "fix" the red baseline by mass-adding ignore directives to historical specs; that destroys the instrument. Measure your delta instead. The one sanctioned exception, exercised in issue #5581, is narrow: a dated record in the specs tree whose target entity is verifiably gone from the whole tree, suppressed one line at a time, each with its reason in the commit body, never with a file-scope or directory-scope directive. Judged per finding it is outcome 1 of three; applied in bulk to make a count go green it is the trap. If a pass clears most findings by directive, say so with the count where a reviewer will see it, so the choice is reviewed rather than absorbed.
