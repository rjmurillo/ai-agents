...
<!-- vendor-portability: contributor-facing pre-push gate for the rjmurillo/ai-agents
     repo itself. It intentionally reads .github/prompts/pr-quality-gate-security.md
...
Run the security axis of the quality gate against your working changes, before you
push, using the same criteria CI will apply.

Migrated from the pr-quality/security command under ADR-064, which makes skills the
single user-invocable surface and renames the namespaced sub-command
pr-quality/security to pr-quality-security. The command file is gone, so its path is named
...
Then emit a fenced JSON block conforming to `.project-toolkit/schemas/pr-quality-gate-output.schema.json` with `"agent": "security"`.
...
- [ ] The JSON block validates against the schema and carries `"agent": "security"`
...
- **New criterion.** Add it to `.github/prompts/pr-quality-gate-security.md`, which
