...
|------|--------|-----------|
| Investigation allowlist (8 patterns as of 2026-07-30; ADR-034 text reconciled to the same 8 in #2958) | `scripts/modules/investigation_allowlist.py`; `.project-toolkit/architecture/ADR-034-investigation-session-qa-exemption.md:78-87` | `uv run python -c "from scripts.modules.investigation_allowlist import get_investigation_allowlist_display as g; print(len(g()), g())"; sed -n '78,87p' .project-toolkit/architecture/ADR-034-investigation-session-qa-exemption.md` |
| build_all no-claude-writes invariant (`CLAUDE_GUARD_PREFIX` snapshot, then `assert_no_claude_writes`; ADR-108 exempts template-owned skill files; re-verified 2026-09-23) | `build/scripts/build_all.py:2092-2093,2240-2251` | `grep -n 'claude_baseline = _snapshot_owned_prefixes\|REQ-003-010: enforce' build/scripts/build_all.py` (prints the snapshot line and the enforcement comment) |
| Commit count has no gate at all, advisory or blocking (ADR-099 demoted it, ADR-100 deleted it; re-verified 2026-09-22) | `scripts/validation/pr_commit_count.py` (deleted), `pr-validation.yml` (the three steps that called it, removed) | `git grep -n pr_commit_count -- scripts lefthook.yml .github/workflows` (prints nothing) |
