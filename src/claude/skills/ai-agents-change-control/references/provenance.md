...
| Commit count has no gate at all, advisory or blocking (ADR-099 demoted it, ADR-100 deleted it) | `scripts/validation/pr_commit_count.py` (deleted), `pr-validation.yml` (the three steps that called it, removed) | `git grep -n pr_commit_count -- ':!.agents' ':!.project-toolkit' ':!.serena'` (should print nothing outside historical records) |
