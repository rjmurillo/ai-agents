...
`.project-toolkit/sessions/` stay in the repo as history.
...
|--------|----------|-------------|--------------|
| Session log (historical only, creation discontinued) | `.project-toolkit/sessions/YYYY-MM-DD-session-NN[-slug].json` | do not create new ones | `scripts/validate_session_json.py`, validate-if-present via the `session-policy` pre-commit hook, applies only if one is somehow staged |
| Per-issue handoff | `.project-toolkit/sessions/handoffs/{YYYY-MM-DD}-{ISSUE}-handoff.md` | copy `.agents/templates/HANDOFF.md` | resume-verification checklist at next session start |
| ADR | `.project-toolkit/architecture/ADR-NNN-slug.md` | `adr-generator` skill | `scripts/validation/check_adr_uniqueness.py`; `adr-review` debate gate |
| Retrospective | `.project-toolkit/retrospective/` | `retrospective` skill on one of the four triggers (HIGH-tier reflect finding, `main` red, reverted or force-fixed merge, explicit user correction), or `/retro fill` on a historical skeleton | `INDEX.md` row (auto-appended, incomplete; see Phase 4) |
...
```bash
uv run python3 scripts/validate_session_json.py .project-toolkit/sessions/<file>.json
```
...
1. Search first: `memory-search` for prior decisions, then read neighbors in
   `.project-toolkit/architecture/` (ADR-007 memory-first: retrieval precedes reasoning).
...
artifact (timeline, Five Whys, learning matrix) in `.project-toolkit/retrospective/`.
...
(`.project-toolkit/retrospective/2026-01-15-pr-908-comprehensive-retrospective.md`).

### Phase 6: Handoffs

Tier model per ADR-014 and `.project-toolkit/sessions/handoffs/README.md`:

| Tier | Location | Scope |
|------|----------|-------|
| Session log | `.project-toolkit/sessions/*.json` | Historical only; creation discontinued |
| Per-issue handoff | `.project-toolkit/sessions/handoffs/{date}-{issue}-handoff.md` | One issue across sessions; rewritten each session |
| Per-branch handoff | `.project-toolkit/handoffs/{branch}/{session}.md` | Multi-session branch coordination |
...
(`.project-toolkit/architecture/ADR-014-distributed-handoff-architecture.md:13-16`).
...
| ADR-073 lifecycle fields (`implemented`) | `.project-toolkit/architecture/ADR-073-adr-lifecycle-frontmatter.md:1-9` | `head -10 .project-toolkit/architecture/ADR-073-adr-lifecycle-frontmatter.md` |
...
| Retro corpus size vs INDEX.md coverage | `.project-toolkit/retrospective/` | `python3 -c "import pathlib;d=pathlib.Path('.project-toolkit/retrospective');f={p.name for p in d.glob('*.md')}-{'INDEX.md'};t=(d/'INDEX.md').read_text();print(len(f),'retro files,',sum(n in t for n in f),'indexed')"` |
...
| Handoff tiers (session log, per-issue, per-branch) | `.project-toolkit/sessions/handoffs/README.md` tier table; ADR-014 | `grep -n "Tier" .project-toolkit/sessions/handoffs/README.md` |
...
| PR #908 lint scope story | `.project-toolkit/retrospective/2026-01-15-pr-908-comprehensive-retrospective.md:281-296` | `grep -n "markdownlint" .project-toolkit/retrospective/2026-01-15-pr-908-comprehensive-retrospective.md` |
...
| Serena is the only memory backend | `.project-toolkit/architecture/ADR-106-serena-only-memory-architecture.md:87` | `grep -n "the only memory backend" .project-toolkit/architecture/ADR-106-serena-only-memory-architecture.md` |
