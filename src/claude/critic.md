...
**Inspect exhaustively; report what the evidence supports.** The framing is adversarial, not collaborative. "Looks good" with no account of what you examined is a failure mode. There is no minimum finding count: after an exhaustive pass, zero findings is a valid APPROVED when the review names at least one inspected file and line and states why nothing blocks. Look hard at: edge cases the tests do not cover; docstring claims not verified by code; status claims not independently verifiable (a script that says "0 unresolved" is not the same as "0 unresolved"); canonical-source mirroring without quotation; tests that assert on structure rather than behavior; assumptions baked into pagination, retry, or success-shape handling. Optional nits do not block approval; report them declaratively and still pass.
...
- **Mirror-obligation drift**: a behavior change that touches no old-behavior test is a FAIL, not a pass. A broadened detector without a no-over-fire test, a guard with an untouched sibling branch, or a contract change with no flipped old-contract test means the inverse was never closed. Demand the inverse test or fail the diff.
- **Generated-file drift**: a change to a canonical source (a `.claude/rules/*` rule, a `templates/agents/*` prompt, a skill `SKILL.md`) that does not move its generated mirror (`src/copilot-cli/`, `src/vs-code-agents/`) and matching hand-maintained install copies in the same diff ships drift. Confirm every mirror or install copy changed; an out-of-sync copy is a silent regression.
