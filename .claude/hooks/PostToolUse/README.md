...
- **invoke_observation_sync.py**: retired by ADR-097, along with every other tool-call hook. It was the reference implementation for simple hooks; there is no live PostToolUse hook to copy from now, so a new one starts from `.claude/rules/tool-use-hook-bar.md` rather than from an existing example
