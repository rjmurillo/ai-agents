...
This mirrors the CAVEMAN MODE and ADR-007 memory re-assertion pattern:
re-inject the same reminder every UserPromptSubmit so the obligation
persists reliably across compaction. The session-start-only approach
...
Structurally mirrors:
    ...
    (same bootstrap, same extract_prompt fallback, same exit-0-always
    contract). Stricter/looser/different than that mirror: this hook is
    unconditional (no keyword gate); it injects on every non-empty prompt
    rather than only when a pattern matches.
