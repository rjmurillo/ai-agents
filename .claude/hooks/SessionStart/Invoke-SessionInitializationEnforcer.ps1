...
try {
    ...
    if (Test-IsMainOrMasterBranch -Branch $currentBranch) {
        Write-Output "`n## WARNING: On Protected Branch`n`n**Current Branch**: ``$currentBranch`` - Switch to feature branch. Commits blocked by pre-commit hooks.`n`n``````bash`ngit checkout -b feat/your-feature-name`n``````"
        exit 0
    }
