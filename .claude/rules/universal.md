...
3. **Verify every closing keyword against the diff before merging.** `Fixes #<n>` is an instruction to GitHub, not a note to the reader: merging executes it whether or not the diff does the work. Confirm each claimed issue is actually closed by the diff, and downgrade the rest to `Refs #<n>` with the reason in the body. A claim is supported only when you can name the hunk. Measured 2026-08-03 across five PRs: 15 keywords failed this check, including two already-closed issues and one whose fix lived in another PR. The failure is silent and inverts the signal, since the backlog shrinks exactly as it becomes less accurate. The inverse is also a defect. A PR carrying no closing keyword is not necessarily missing one: agent-authored PR bodies in this repository SHOULD carry an explicit `These references do not close their linked items:` section (this convention is established by this rule, not by the PR template), so a bare `Refs #<n>` with a stated reason is correct authorship, not an omission. Measured 2026-08-03, of seven keyword-free PRs repaired by adding `Fixes` from a title match, three had bodies that explicitly declined to close the issue named and a fourth named an issue a different open PR was already closing. Never add a closing keyword to a PR you did not write without reading its body first. Adding one after the PR merges is inert: GitHub evaluates the linkage at merge time and never backfills `closingIssuesReferences`, so a merged PR that orphaned an issue is remediated by closing that issue with a comment citing the merge commit, never by editing the body.
