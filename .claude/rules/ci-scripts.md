...
22. **A job `name:` is a branch-protection identifier. Land the rename on `main` first, then update the ruleset, never the reverse.** GitHub matches a required status check against the check-run name, which for a workflow job is its `name:`, defaulting to the job id when `name:` is absent. So editing a `name:` under `.github/workflows/` or `.github/actions/` silently renames a contract the ruleset depends on, and nothing in the diff says so. Before changing one, check whether the current name is required: `gh api repos/OWNER/REPO/rulesets/ID --jq '[.rules[]|select(.type=="required_status_checks")|.parameters.required_status_checks[].context]'`. If it is, the rename and the ruleset edit are one change executed in a fixed order, because they cannot be atomic: the rename must be merged to `main` first, and only then may the ruleset require the new name. Updating the ruleset first blocks every pull request on a context nothing produces, including the pull request carrying the rename, and that deadlock is breakable only by an administrative bypass merge. Verify after landing by confirming the required context appears in `gh pr checks` on a fresh pull request, not by reading the workflow file, since the file proves what will be emitted next run rather than what the ruleset is waiting for. Measured 2026-08-09: commit `45f07f346` renamed `Aggregate Results` to `AI Quality Gate Results` in `ai-pr-quality-gate.yml` and renamed the session workflow's aggregate job to `Session Protocol Results`, ruleset `11104075` was updated to require both, and the commit stayed on the unmerged branch of PR #4784 while `main` continued emitting `Aggregate Results`. Every open pull request then waited on two names that did not exist, and `main` took no commits for roughly five hours until #4784 was merged with `--admin`. A repository-role bypass on the ruleset hid the fault for the whole window, because human merges through `merge_pr.py` bypassed the unsatisfiable requirement while reporting success; the deadlock became visible only when a merge queue, whose app holds no bypass, tried to honour the same requirement. Treat a bypass as a reason to check the requirement is satisfiable rather than as evidence that it is.
