...
22. **A job `name:` is a branch-protection identifier. Land the rename on `main` first, then update the ruleset, never the reverse.** GitHub matches a required status check against the check-run name, which for a workflow job is its `name:`, defaulting to the job id. Editing a `name:` under `.github/workflows/` or `.github/actions/` silently renames a contract the ruleset depends on, and nothing in the diff says so. Check first: `gh api repos/OWNER/REPO/rulesets/ID --jq '[.rules[]|select(.type=="required_status_checks")|.parameters.required_status_checks[].context]'`. If the name is required, the rename and the ruleset edit are one change in a fixed order, because they cannot be atomic: merge the rename to `main`, then require the new name. Reversing that blocks every pull request on a context nothing produces, including the pull request carrying the rename, and only an administrative bypass merge breaks it. Verify with `gh pr checks` on a fresh pull request, not by reading the workflow file, which proves what will be emitted next run rather than what the ruleset awaits. Measured 2026-08-09: `45f07f346` renamed two aggregate jobs, ruleset `11104075` required both new names, the commit stayed on the unmerged branch of PR #4784, and `main` took no commits for five hours. A repository-role bypass hid it, because human merges bypassed the unsatisfiable requirement while reporting success; a merge queue, whose app holds no bypass, exposed it. Treat a bypass as a reason to check a requirement is satisfiable, not as evidence that it is.
