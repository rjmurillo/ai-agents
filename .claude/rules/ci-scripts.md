...
22. **A job `name:` is a branch-protection identifier. Land the rename on `main` first, then require the new name, never the reverse.** GitHub matches a required status check against the check-run name, which for a workflow job is its `name:`, defaulting to the job id. Renaming a job in `.github/workflows/` silently renames a contract the ruleset depends on, and nothing in the diff says so. Check before renaming: `gh api repos/OWNER/REPO/rulesets/ID --jq '[.rules[]|select(.type=="required_status_checks")|.parameters.required_status_checks[].context]'`. If the name is required, the rename and the ruleset edit are one change in a fixed order, because they cannot be atomic: merge the rename to `main`, then require the new name. Reversing it blocks every pull request on a context nothing produces, including the pull request carrying the rename. Verify with `gh pr checks` on a fresh pull request, not by reading the workflow file, which proves what will be emitted next run rather than what the ruleset awaits. Measured 2026-08-09: `8565eb6d3` (01:19:31) and `45f07f346` (03:13:53) renamed jobs on the unmerged branch of PR #4784, while ruleset `11104075` was edited to require `Session Protocol Results` at 01:26:07 and `AI Quality Gate Results` at 03:12:10. The second edit preceded its commit by 1m43s, so the ruleset required a name no commit had yet created. `main` then took no commits for five hours, and the outage ended by dropping both requirements at 15:55:30, not by the rename landing. A repository-role bypass hid it, because human merges bypassed the unsatisfiable requirement while reporting success; a merge queue, whose app holds no bypass, exposed it. Treat a bypass as a reason to check a requirement is satisfiable, not as evidence that it is.
