...
22. **A job `name:` is a branch-protection identifier. Emit both names before requiring the new one.** GitHub matches a required status check against a workflow job's check-run name, which is the job `name:` or the job id when `name:` is absent. Composite-action metadata under `.github/actions/` does not create check runs by itself; only consuming workflow jobs do. Check the complete protected-branch requirement set, not one guessed ruleset: `gh api repos/OWNER/REPO/rules/branches/main --jq '[.[]|select(.type=="required_status_checks")|.parameters.required_status_checks[].context]'`. Compare that set with check-run names emitted on a fresh PR run, because `gh pr checks` cannot show a required context that never ran. To rename without an enforcement gap: add a temporary alias job that emits the new name while retaining the old named job, merge that to `main`, require the new context after both are visible, remove the old requirement, then remove the old alias job. The simpler sequence also works but creates a gap: remove the old requirement, merge the rename, observe the new check, then require it. Requiring the new name before any `main` commit emits it blocks every PR on a context nothing can satisfy. Measured 2026-08-09: ruleset changes required `Session Protocol Results` and `AI Quality Gate Results` while those names existed only on the unmerged PR branch, so `main` took no commits for five hours and the outage ended by dropping both requirements at 15:55:30.
