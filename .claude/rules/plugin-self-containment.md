...
The remaining body surface is large. Counting it is matcher-dependent, so use the gate's own pattern and say so: 2,556 references across 347 of 901 shipped Markdown files, measured at `30eaa85dde`. Reproduce with `OUTWARD_FILE` from `check_plugin_frontmatter_self_containment.py` applied to every `git ls-files` Markdown path under the three plugin roots. A count quoted without its matcher is not evidence; two reasonable matchers disagreed by a factor of nearly two on this same tree. Most of those references are legitimate under the three-kind table above, so the surface needs per-file classification rather than a pattern, and it stays on the reviewer.
