...
5. **Add the index entry by hand when writing a Serena memory**. A new memory under `.serena/memories/` MUST get a keyword line in `.serena/memories/memory-index.md` in the same change. That file is a curated retrieval map, not a manifest: it links 93 files out of the 897 `.md` files on disk, so a memory with no line in it is unreachable through the keyword path even though the file is committed and readable by name. Nothing catches the omission. `scripts/validation/memory_index.py --ci` walks index rows and checks each resolves to a file on disk, which is why its summary reads `Files: 305 indexed, 0 missing`: 305 counts rows across the 33 domain indexes, not memory files carrying an entry, and `0 missing` means no row points at an absent file. The check never runs the other direction. Verified by deleting an unindexed memory and re-running: the output was byte-identical with the file present and absent. Add the line, then let the `memory-token-update` hook fill the token count; write `(0)` as the placeholder rather than guessing a number.
