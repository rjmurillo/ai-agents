...
8. **Delete `__pycache__` between a mutation and its rerun.** MUST 7 governs how a harness detects that a patch applied; this is the separate failure that survives every one of those checks. CPython validates a cached `.pyc` against the source modification time in whole seconds and the source size, so a mutation that preserves size and is reverted inside the same second leaves the cache valid, and the restored file still executes as the mutant. The byte-level checks all pass because the bytes really did change; the interpreter simply never read them. Deleting the cache directory is the remedy. `PYTHONDONTWRITEBYTECODE` and `python -B` do not help, because they suppress writing new bytecode while still reading the stale entry, and pytest's `-p no:cacheprovider` governs `.pytest_cache` only. Applies equally to an ad-hoc control run by hand, where no harness is enforcing MUST 7 for you: a control that cannot be shown to have reached the interpreter is not evidence about the test.
