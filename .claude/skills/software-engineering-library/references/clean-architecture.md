...
- **Use cases**: the orchestrator agent and lifecycle skills (`/spec`, `/plan`, `/build`, `/test`, `/review`, `/ship`). Each is a named operation that loads domain state, calls domain methods, and commits a single change. New cross-cutting operations belong here, not inside hooks or wire-format scripts.
...
- **Frameworks and drivers**: GitHub Actions runners, the gh CLI, the Serena MCP server, the file system layout, Python and Node toolchains. Bootstrap them at the edge. Inner layers depend on adapter ports, not on these.
