...
- `.claude/skills/observability/references/event-logging-contract.md`: the producer-side event-logging contract. Use it to check whether a new path emits the MINIMUM required operational events (job start/completion, state transitions, external-call outcomes, retry attempts and exhaustion, rate-limit hits, behavior-changing skips, errors with operation context, cancellation/timeout) and avoids the prohibited ones (per-function entry/exit, per-layer duplication, secrets or full payloads, unjustified high-cardinality). Reference this contract in findings; do not restate its tables.
