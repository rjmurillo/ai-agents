...
- local axis 2: Skill(skill="doc-accuracy"), invoking `python3 <doc_accuracy.py> --target . --diff-base "origin/$BASE_BRANCH" --format json`. This is the axis `docs-and-instructions` selects: a docs or instruction change is reviewed for claims the code contradicts, not waved through.
