...
2. **Run the Stage-1 spec-compliance gate before the complexity classifier and all Stage-2 axes.** Load the canonical `spec-compliance` axis prompt via the "Path resolution" section above and invoke `Task(subagent_type="general-purpose")` with that prompt as the system instruction and the CONTEXT_MODE-prefixed diff plus any linked REQ/DESIGN/TASK docs (or PR-body acceptance criteria) as input. Extract its verdict with `extract_verdict`.
   - **CRITICAL_FAIL**: short-circuit. Do NOT run the complexity classifier, the 11 Stage-2 canonical axes, or the 3 chained skills. Mark all of them `SKIPPED` in the output table, set the FINAL VERDICT to `CRITICAL_FAIL`, and emit only the Stage-1 findings. A real spec violation blocks the merge; the author fixes the unmet criterion, then re-runs `/review`.
   - **UNKNOWN (INCONCLUSIVE, e.g. no linked spec)**: do NOT short-circuit. Record the Stage-1 verdict as `UNKNOWN` with the one-line reason no spec was linked, then continue to step 3 and run the complexity classifier and all Stage-2 axes. Pass the Stage-1 `UNKNOWN` into `merge_verdicts` (step 7) like any other axis: per UNKNOWN handling it only changes the final verdict when Stage 2 would otherwise be `PASS`. A missing spec MUST NOT suppress a real Stage-2 `CRITICAL_FAIL` or `WARN` (issue #2690).
