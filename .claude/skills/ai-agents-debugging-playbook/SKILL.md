...
2. Re-run the exact failing check locally, not a proxy for it:
   - Tests: `uv run pytest tests/ -x` (note: new skill tests belong in `tests/skills/<name>/` and ARE collected by default; legacy colocated tests at `.claude/skills/<name>/tests/` require explicit invocation: `uv run pytest .claude/skills/<name>/tests/ -q`)
