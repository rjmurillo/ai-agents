...
| `AI_AGENTS_PYTEST_FULL_SUITE_LOCALLY` | env var (`1` or unset) | Restores local execution of the pre-push full-suite fallback, which otherwise collects only (ADR-104). Unset: the fallback collects, so a broken import and a syntax error still block the push and everything else is CI's. `1`: the fallback executes the partitions locally. Any other value raises rather than quietly doing less, and the flag never changes a narrowed import-graph selection, which always executes what it selected | Production opt-in | Scoped to one job's fallback, not a bypass: it makes the gate stricter, never weaker, so there is no abuse direction. Validated before a path is chosen, so `true` is reported on both paths instead of being ignored on one | `scripts/validation/git_hook_policy.py:504` (name), `:6683` `_validated_full_suite_opt_in` (semantics) |
| `AI_AGENTS_PYTEST_WORKERS` | env var (positive integer or `auto`) | Overrides the xdist worker count pre-push pytest runs with. Unset: the policy picks the count. Any value it cannot parse raises rather than falling back to a default the caller did not ask for | Production | Same fail-loud contract as the flag above and validated at the same point, after a version that checked it only inside the executing partitions let an invalid value pass unreported on the collection path | `scripts/validation/git_hook_policy.py:512` (name), `:6494` `_pytest_parallel_flags` (semantics) |
