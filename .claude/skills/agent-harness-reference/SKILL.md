...
| `CLAUDE_PROJECT_DIR` under Copilot CLI | CONTESTED, do not rely on either presence or absence. A static byte search of the shipped 1.0.80 artifacts never finds the string, so Copilot does not set it by that name; the live environment listing in issue #4727 on 1.0.79-6 records it as present and "cannot distinguish harnesses". A static search cannot see inheritance from an ancestor shell, and the live read is the stronger evidence for what a hook receives, so the earlier "not exported" reading is withdrawn. Anchor defensively either way with `${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel)}`, because where the variable IS unset a bare `cd "$CLAUDE_PROJECT_DIR"` runs `cd ""`, which exits 0 and leaves cwd unchanged in dash and bash, so the `&&` chain continues and relative script paths resolve against the host cwd | EMPIRICAL, conflicting (static search 1.0.80 against live listing 1.0.79-6, issue #4727) | `references/probe-evidence.md` section 8b |
| `COPILOT_CLI` | Claimed set to `1` for subprocesses Copilot CLI spawns; the cited changelog entry does not exist in any published `@github/copilot` version, and the string appears nowhere as an environment variable in the shipped 1.0.80 binary | **RETRACTED** | Correction in `references/official-hook-contracts.md`, "Environment variables Copilot CLI sets" |
| `CLAUDE_CODE_ENTRYPOINT` | Set by Claude Code and never named in the Copilot CLI build measured. Not a host discriminator: no vendor document names it as one, and it evidences process ancestry rather than the program consuming the hook, because a Copilot CLI process launched from a Claude-spawned shell inherits it. That is the same inheritance that makes `COPILOT_CLI` unreliable in the other direction, so neither variable identifies the consuming host. Check it first only because that ordering fails safe for Claude | EMPIRICAL, static artifact search only (Copilot CLI 1.0.80 and Claude Code, issue #4727) | `references/probe-evidence.md` section 8b |
