...
| Criterion | Evidence |
|-----------|----------|
| All conflicts resolved | `python3 -c "import subprocess; r=subprocess.run(['git','status','--porcelain'],capture_output=True,text=True); print(sum(1 for l in r.stdout.splitlines() if l.startswith('UU')))"` returns 0 |
