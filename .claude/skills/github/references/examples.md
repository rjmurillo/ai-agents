...
result=$(python3 "$SCRIPTS_DIR/get_pr_review_comments.py" --pull-request 50 --only-unaddressed)
...
python3 "$SCRIPTS_DIR/get_pr_review_comments.py" --pull-request 50 --only-unaddressed --bot-only

# Get comments with full lifecycle state analysis
result=$(python3 "$SCRIPTS_DIR/get_unaddressed_comments.py" --pull-request 50)
...
python3 "$SCRIPTS_DIR/get_unaddressed_comments.py" --pull-request 50 --no-only-unaddressed

# Get comment counts by author
result=$(python3 "$SCRIPTS_DIR/get_pr_review_comments.py" --pull-request 50)
...
python3 "$SCRIPTS_DIR/get_pr_review_comments.py" --pull-request 50 --group-by-domain
```
...
python3 "$SCRIPTS_DIR/get_pull_requests.py"

# List all PRs with custom limit
python3 "$SCRIPTS_DIR/get_pull_requests.py" --state all --limit 100

# Filter PRs by label and state
python3 "$SCRIPTS_DIR/get_pull_requests.py" --label "bug,priority:P1" --state open

# Filter PRs by author and base branch
python3 "$SCRIPTS_DIR/get_pull_requests.py" --author rjmurillo --base main

# Search PRs by keyword or GitHub search syntax
python3 "$SCRIPTS_DIR/get_pull_requests.py" --search "fix auth is:open"

# Get PR with changed files
python3 "$SCRIPTS_DIR/get_pr_context.py" --pull-request 50 --include-changed-files

# Check if PR is merged before starting work
python3 "$SCRIPTS_DIR/test_pr_merged.py" --pull-request 50

# Get CI check status
python3 "$SCRIPTS_DIR/get_pr_checks.py" --pull-request 50

# Wait for CI checks to complete (timeout 10 minutes)
python3 "$SCRIPTS_DIR/get_pr_checks.py" --pull-request 50 --wait --timeout-seconds 600

# Get only required checks
python3 "$SCRIPTS_DIR/get_pr_checks.py" --pull-request 50 --required-only

# Detect Copilot follow-up PRs
python3 "$SCRIPTS_DIR/detect_copilot_followup_pr.py" --pr-number 50
```
...
python3 "$SCRIPTS_DIR/post_pr_comment_reply.py" --pull-request 50 --comment-id 123456 --body "Fixed."

# Resolve all unresolved review threads
python3 "$SCRIPTS_DIR/resolve_pr_review_thread.py" --pull-request 50 --all

# Reply to review thread by thread ID (GraphQL)
python3 "$SCRIPTS_DIR/add_pr_review_thread_reply.py" --thread-id "PRRT_kwDOQoWRls5m3L76" --body "Fixed."

# Reply to thread and resolve in one operation
python3 "$SCRIPTS_DIR/add_pr_review_thread_reply.py" --thread-id "PRRT_kwDOQoWRls5m3L76" --body "Fixed." --resolve

# Check if PR is ready to merge (threads resolved, CI passing)
python3 "$SCRIPTS_DIR/test_pr_merge_ready.py" --pull-request 50
```
...
python3 "$SCRIPTS_DIR/set_pr_auto_merge.py" --pull-request 50 --enable --merge-method SQUASH

# Disable auto-merge
python3 "$SCRIPTS_DIR/set_pr_auto_merge.py" --pull-request 50 --disable
```
...
python3 "$SCRIPTS_DIR/new_pr.py" --title "feat: Add feature" --body "Description"

# Close PR with comment
python3 "$SCRIPTS_DIR/close_pr.py" --pull-request 50 --comment "Superseded by #51"

# Merge PR with squash
python3 "$SCRIPTS_DIR/merge_pr.py" --pull-request 50 --strategy squash --delete-branch
...
ids=$(python3 "$SCRIPTS_DIR/get_pr_review_comments.py" --pull-request 42 | jq -r '.Comments[].id')
