...
5. Exhaust pagination before deriving any count or verdict. Every collection
   read above is paginated: `get_review_comments`, `get_reviews`,
   `get_comments`, `get_check_runs`, `list_pull_requests`, `search_*`, and
   `issue_read` method `get_comments`. Follow each response's pagination
   metadata to the end. A first-page read fails in the one direction that
   matters: it omits the later blocker and reads clean, so "0 unresolved", "no
   failing checks", and "no requested reviewers" all become false in exactly
   the case they are checked for. The gh scripts paginate internally, so this
   is work MCP mode adds rather than work it inherits. Measured on this PR,
   which reached 139 check runs against a 100-item page. When a read is
   truncated and you cannot page further, report the count as incomplete
   rather than as a verdict, per rule 3.
