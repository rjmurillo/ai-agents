...
**always-on corpus is 7 rules, 70,236 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,985 bytes
...
116 bytes larger in total (70,352 always-on) because `generate_rules.py`
...
70,352-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 7 rules and 70,236 bytes, matching
