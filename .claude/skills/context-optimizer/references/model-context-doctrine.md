...
**always-on corpus is 8 rules, 72,287 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,038 bytes
...
135 bytes larger in total (72,422 always-on) because `generate_rules.py`
...
72,422-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 72,287 bytes, matching
