...
Corpus measured at `856b51f9d`. Two numbers, and they
are not interchangeable. The
**always-on corpus is 8 rules, 72,197 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 97,948 bytes
...
135 bytes larger in total (72,332 always-on) because `generate_rules.py`
...
72,332-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 72,197 bytes, matching
