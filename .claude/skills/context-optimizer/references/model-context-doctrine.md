...
**always-on corpus is 8 rules, 72,652 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,937 bytes
...
135 bytes larger in total (72,787 always-on) because `generate_rules.py`
...
72,787-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 72,652 bytes, matching
