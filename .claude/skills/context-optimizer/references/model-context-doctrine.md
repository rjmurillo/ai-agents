...
**always-on corpus is 8 rules, 73,163 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,914 bytes
...
135 bytes larger in total (73,298 always-on) because `generate_rules.py`
...
73,298-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 73,163 bytes, matching
