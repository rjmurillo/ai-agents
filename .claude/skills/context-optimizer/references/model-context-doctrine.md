...
**always-on corpus is 8 rules, 73,230 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,981 bytes
...
135 bytes larger in total (73,365 always-on) because `generate_rules.py`
...
largest rule either: `voice.md` at 18,105 bytes is the single biggest
always-on file.
...
73,365-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 73,230 bytes, matching
