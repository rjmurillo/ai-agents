...
**always-on corpus is 8 rules, 73,216 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,967 bytes
...
135 bytes larger in total (73,351 always-on) because `generate_rules.py`
...
73,351-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 73,216 bytes, matching
