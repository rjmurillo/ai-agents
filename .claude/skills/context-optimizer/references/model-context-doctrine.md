...
Measured on this branch after PR #5433 (issue #5404) added terminal-state completion-contract prose to `builder-ethos` and `voice`. Two numbers, and they are not interchangeable. The
**always-on corpus is 7 rules, 70,917 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,844 bytes
...
116 bytes larger in total (71,033 always-on) because `generate_rules.py`
...
71,033-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 7 rules and 70,917 bytes, matching
