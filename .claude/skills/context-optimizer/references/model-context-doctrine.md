...
**always-on corpus is 7 rules, 70,313 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,242 bytes
...
116 bytes larger in total (70,429 always-on) because `generate_rules.py`
...
70,429-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 7 rules and 70,313 bytes, matching
