...
**always-on corpus is 3 rules, 38,086 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 79,444 bytes
...
57 bytes larger in total (38,143 always-on) because `generate_rules.py`
...
corpus has taken. The biggest always-on file is `builder-ethos.md` at 14,199 bytes: it overtook `voice.md` once epic #5456 M4 moved most of `voice.md`'s prose (Writing Style, Completeness Principle, Confusion Protocol) into the `spec`, `plan`, `review`, and `autoplan` skills.
...
38,143-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 3 rules and 38,086 bytes, matching
