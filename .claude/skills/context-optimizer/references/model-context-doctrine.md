...
**always-on corpus is 8 rules, 73,434 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 99,185 bytes
...
135 bytes larger in total (73,569 always-on) because `generate_rules.py`
...
That leaves 14,152 always-on bytes of book-derived rule, 19.2% of the
73,569-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 73,434 bytes, matching
