...
**always-on corpus is 8 rules, 71,484 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 97,235 bytes
...
135 bytes larger in total (71,619 always-on) because `generate_rules.py`
...
71,619-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 71,484 bytes, matching
