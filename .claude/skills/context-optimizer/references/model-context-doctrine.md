...
**always-on corpus is 8 rules, 72,669 bytes**: the ones that load regardless
of what you touch. The **effective context on a `.py` edit is 98,420 bytes
...
135 bytes larger in total (72,804 always-on) because `generate_rules.py`
...
72,804-byte always-on corpus measured at source. `code-quality` and
...
`src/copilot-cli/instructions` carries 8 rules and 72,669 bytes, matching
...
The always-on corpus is 8.9x that threshold and a Python edit sees 12.0x,
