...
- `build_all.py --check` exits 2 on staleness or when a file under `OWNED_PREFIXES` cannot be read, and exits 3 when it cannot read git state (docstring, `build/scripts/build_all.py:19-24`). Read the stderr line before reaching for the staleness remedy: regenerating and committing fixes only the staleness producer, not the exit-3 git or exit-2 unreadable-file causes. Its log legitimately says `Mode: Generate` mid-run; the snapshot/restore guard (#2440) makes the whole run read-only.
