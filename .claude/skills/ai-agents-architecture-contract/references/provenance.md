...
| Generator inventory, 7 generators | `build/scripts/build_all.py:550-558` `GENERATORS`; `.agents/governance/GENERATOR-FILES.md` | `grep -n -A8 "^GENERATORS" build/scripts/build_all.py` |
| REQ-003-010 no-write invariant | `build/scripts/build_all.py:712` (guard section), `:841` (`assert_no_claude_writes`), `:2204` (baseline snapshot), `:2318-2328` (enforcement, `REQ-003-010 VIOLATION`, exit 2); ADR-108 allowlists the template-owned skill files | `sed -n '712p;841p;2204p;2318,2328p' build/scripts/build_all.py` |
...
| Serena is the only memory backend | `.claude/skills/memory/memory_core/memory_router.py` module docstring | `head -12 .claude/skills/memory/memory_core/memory_router.py` |
