...
- Observation loop (issue #1345, Detect-Log-Graduate-Apply): the `reflect` skill and Stop hook detect corrections, observations land in Serena memories, the skillbook agent graduates patterns, and `.claude/hooks/PreToolUse/invoke_correction_applier.py` re-surfaces HIGH-confidence corrections at command time. This is why hook output injects "Self-Improving Agent" context into sessions.
...
| Stale docs contradict reality | `CONTRIBUTING.md:155` says "pwsh build/Generate-Agents.ps1"; zero `.ps1` files exist outside `.venv/` (ADR-042). `GENERATOR-FILES.md` lists `src/claude/` as a `generate_agents.py` output ("`src/claude/`, `src/copilot-cli/agents/`, `src/vs-code-agents/` (per platform YAML)"), but `generate_agents.py` contains no `src/claude` write and no claude platform YAML exists | Following docs verbatim fails; quote the canonical source when correcting (FM-9) |
...
| Two skill-test locations | `pyproject.toml:41` `testpaths = ["tests", "test"]`; `.claude/skills/<name>/tests/` NOT collected by default; `tests/skills/<name>/` is | Green CI does not prove skill tests ran; run them explicitly (`ai-agents-validation-and-qa`) |
...
| Correction-applier loop | `.claude/hooks/PreToolUse/invoke_correction_applier.py` docstring (issue #1345) | `head -12 .claude/hooks/PreToolUse/invoke_correction_applier.py` |
