...
|---|---|---|
| Generators never write `.claude/` (except the ADR-108 template-owned skill files) | `assert_no_claude_writes` at `build/scripts/build_all.py:841`, applied at `:2318-2328`, exit 2 | Canonical tree gets silently overwritten by its own mirror; source of truth inverts |
