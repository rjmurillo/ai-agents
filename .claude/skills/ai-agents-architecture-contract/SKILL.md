...
| Copilot dispatcher | Active `gate` handles two PreToolUse shims; active `observe` handles one PostToolUse shim; dormant `advise` translates a future PermissionRequest producer | Generated manifests; `build/scripts/generate_dispatcher.py` |
...
- **Hook sources serve different consumers**: `.claude/settings.json` has 6 events and 8 groups, `.claude/hooks/hooks.json` has 2 events and 3 groups; do not force parity; verify repository-only vs vendored before editing either source.
...
- [ ] Confirmed event counts still match: local settings print `6 8`, vendored source prints `2 3`, and generated Copilot config prints `2 2`
