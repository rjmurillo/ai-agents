...
def test_is_applicable_matches_toolkit_artifacts(tmp_path: Path) -> None:
    skill = _write(tmp_path, ".claude/skills/demo/SKILL.md", "---\nname: demo\n---\n")
    agent = _write(tmp_path, ".claude/agents/demo.md", "---\nname: demo\n---\n")
    workflow = _write(tmp_path, ".github/workflows/ci.yml", "on: push\n")
...
def test_is_applicable_rejects_non_toolkit_files(tmp_path: Path) -> None:
    ...
    claude_agent = _write(tmp_path, ".claude/agents/CLAUDE.md", "# claude\n")
