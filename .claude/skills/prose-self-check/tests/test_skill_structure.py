...
# .../.claude/skills/prose-self-check -> repo root is three parents up from SKILL_DIR
REPO_ROOT = SKILL_DIR.parents[2]
COPILOT_MIRROR = REPO_ROOT / "src" / "copilot-cli" / "skills" / "prose-self-check" / "SKILL.md"
...
def test_copilot_mirror_is_byte_identical() -> None:
    # Arrange / Act / Assert: dual-tree convention keeps the mirror in sync.
    assert COPILOT_MIRROR.is_file(), f"missing copilot mirror at {COPILOT_MIRROR}"
    assert COPILOT_MIRROR.read_bytes() == SKILL_MD.read_bytes(), (
