...
- **Branch behind main**: Re-check live state immediately before the base refresh (issue #4349): call `check_pr_live_state.py` and abort with a logged reason if the verdict is SKIP. The PR may have been merged, closed, or superseded in the time since the session-start gate ran; proceeding into a `git merge origin/main` on a closed branch produces unpushable commits and loses the work. If the re-check returns SKIP, preserve any unpushed local commits with `git format-patch -N origin/"$BRANCH"` (produces patch files named `000N-*.patch`), then report the merged/closed SHA and route the surviving fixes to a follow-up branch if still applicable. If the re-check returns ACT: Worktree + `git merge origin/main --no-edit` + push (no force needed).
