...
**Don't-do-this pattern:** try to dump the previous session's full state and reload it verbatim. Brittle, drifts every iteration, eventually breaks down. The literal analogue is naively pickling the entire prior context, and the substrate doesn't accept it.
