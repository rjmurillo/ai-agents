...
| Level | Step | Verification |
|-------|------|--------------|
| MUST | Complete session log (all MUST items `[x]`) | Validate-SessionJson.ps1 |
...
| MUST | Run `pwsh scripts/Validate-SessionJson.ps1 -SessionPath [log]` | Exit code 0 |
...
- Skip session validation (`Validate-SessionJson.ps1` must pass)
