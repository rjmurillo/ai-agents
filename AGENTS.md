...
**Never**: Commit secrets|Update HANDOFF.md|Use bash|Skip validation|Logic in YAML (ADR-006)|Raw gh when skills exist|Force push|Skip hooks|Internal refs in src/|Resolve security threads without fixing the underlying security vulnerability (CWE/OWASP/CVE) in code
